...
import su.terrafirmagreg.api.util.IVariant;
...
public class RegistryModel {
	...
	public static void registerBlockItemModels(Block... blocks) {
		...
		for (Block block : blocks) {
			...
			if (block instanceof IHasModel model) {
				...
				RegistryModel.registerBlockItemModel(block.getDefaultState());
			}
		}
	}
	...
	public static void registerBlockItemModel(IBlockState blockState) {
		...
		RegistryModel.registerItemModel(
				item,
	...
	public static <T extends IVariant & Comparable<T>> void registerVariantBlockItemModels(
		...
		RegistryModel.registerVariantBlockItemModels(baseState, property, IVariant::getMeta);
	}
	...
	public static <T extends Comparable<T>> void registerVariantBlockItemModels(
		...
		property.getAllowedValues()
		        .forEach(value -> RegistryModel.registerBlockItemModelForMeta(
	...
	public static void registerBlockItemModelForMeta(final IBlockState state, final int metadata) {
		...
		RegistryModel.registerItemModel(item, metadata, PROPERTY_STRING_MAPPER.getPropertyString(state.getProperties()));
	}
	...
	public static void registerItemModels(Item... items) {
		...
		for (Item item : items) {
			...
			RegistryModel.registerItemModel(item, registryName.toString());
		}
	}
	 ...
	 * @param items     the items
	 * @see RegistryModel#registerItemModels(Item...)
	 */
	public static void registerItemModels(String subfolder, Item... items) {
		...
		for (Item item : items) {
			...
			RegistryModel.registerItemModel(item, modelLocation);
		}
	}
	...
	public static void registerItemModel(Item item, String modelLocation) {
		...
		RegistryModel.registerItemModel(item, 0, resourceLocation);
	}
	...
	public static void registerItemModel(@NotNull final Item item, final int metadata, @NotNull final String variant) {
		...
		RegistryModel.registerItemModel(item, metadata, new ModelResourceLocation(registryName, variant));
	}
...
//    public static void registerItemModel(@Nonnull Item item, int meta, @Nonnull String modelName) {
...
//    }
	...
	public static <T extends IVariant> void registerVariantItemModels(
		...
		for (T value : values) {
			RegistryModel.registerItemModel(item, value.getMeta(), variantName + "=" + value.getName());
		}
	}
     ...
     */
    @SideOnly(Side.CLIENT)
    public static void registerInventoryModel(@Nonnull Block block) {
        if (block instanceof IHasModel model) model.onModelRegister();
        else registerBlockModel(block, new StateMap.Builder().build());
    }
     ...
     */
    @SideOnly(Side.CLIENT)
    public static void registerInventoryModel(@Nonnull Item item) {
        if (item instanceof IHasModel model) model.onModelRegister();
        else registerItemModel(item, 0, "normal");

        if (item instanceof ICustomMesh mesh) ModelLoader.setCustomMeshDefinition(item, mesh.getCustomMesh());
    }
}

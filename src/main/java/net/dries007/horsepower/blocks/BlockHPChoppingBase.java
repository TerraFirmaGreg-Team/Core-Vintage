...
public abstract class BlockHPChoppingBase extends BlockHPBase {
  ...
  public static IExtendedBlockState getExtendedState(TileEntityHPBase te, IExtendedBlockState state) {
    ...
    if (!side_texture.isEmpty()) {state = state.withProperty(SIDE_TEXTURE, side_texture);}
    if (!top_texture.isEmpty()) {state = state.withProperty(TOP_TEXTURE, top_texture);}
  ...
  public void getSubBlocks(CreativeTabs tab, NonNullList<ItemStack> list) {
    ...
    for (ItemStack stack : stacks) {
      if (!Configs.general.useDynamicDisplay && !"tfc".equals(stack.getItem().getRegistryName().getNamespace())) {continue;}

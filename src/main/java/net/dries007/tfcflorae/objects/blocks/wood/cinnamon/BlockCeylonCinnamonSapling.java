...
public class BlockCeylonCinnamonSapling extends BlockSaplingTFC {
  ...
  public void grow(World world, Random rand, BlockPos pos, IBlockState blockState) {
    ...
    for (int trunk = 0; trunk < height; trunk++) {
      ...
      for (EnumFacing d : EnumFacing.HORIZONTALS) {
        ...
        if (trunk < 0.3 * height && rand.nextFloat() < (1 - (float) trunk / (height)) / 3) {world.setBlockState(trunkPos.offset(d, 3), leaves);}
      }
    }

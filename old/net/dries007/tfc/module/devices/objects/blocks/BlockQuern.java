...
public class BlockQuern extends BlockBase implements IItemSizeAndWeight, IHighlightHandler, IHasModel {
    ...
    public void addCollisionBoxToList(IBlockState state, World world, BlockPos pos, AxisAlignedBB entityBox, List<AxisAlignedBB> collidingBoxes, @Nullable Entity entityIn, boolean isActualState) {
        Block.addCollisionBoxToList(pos, entityBox, collidingBoxes, BASE_AABB);
        TEQuern teQuern = Helpers.getTE(world, pos, TEQuern.class);
        if (teQuern != null && teQuern.hasHandstone()) {
            Block.addCollisionBoxToList(pos, entityBox, collidingBoxes, HANDSTONE_AABB);
        }
    }
            ...
            if (teQuern != null && !teQuern.isGrinding()) {
                ...
                if (inventory != null) {
                    if (selection == SelectionPlace.HANDLE) {
                        ...
                        return true;
                    } else if (selection == SelectionPlace.HANDSTONE && inventory.getStackInSlot(TEQuern.SLOT_HANDSTONE).isEmpty() && inventory.isItemValid(TEQuern.SLOT_HANDSTONE, heldStack)) {

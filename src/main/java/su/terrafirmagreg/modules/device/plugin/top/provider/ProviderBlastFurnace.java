    ...
    public void addProbeInfo(ProbeMode mode, IProbeInfo info, EntityPlayer player, World world, IBlockState state, IProbeHitData hitData) {
        ...
        if (block instanceof BlockBlastFurnace) {
            ...
            if (chinmey > 0) {
                ...
                currentTooltip.add(new TextComponentTranslation(ModUtils.localize("top", "device.bloomery.ores"), oreStacks, maxItems).getFormattedText());
                currentTooltip.add(new TextComponentTranslation(ModUtils.localize("top", "device.bloomery.fuel"), fuelStacks, maxItems).getFormattedText());
                if (heatTooltip != null) {
                    currentTooltip.add(heatTooltip);
                }
            } else {
                currentTooltip.add(new TextComponentTranslation(ModUtils.localize("top", "device.blast_furnace.not_formed")).getFormattedText());
            }

...
public class GeneratorRockMossyRaw implements IWorldGenerator {
    ...
    for (float r = rarity; r < (5 + rarity); r++) {
      ...
      if (data.isInitialized() && data.getRainfall() >= RAINFALL_SAND_SANDY_MIX) {
        ...
        for (int i = random.nextInt(Math.round(1 + floraDiversity));
          ...
          BlockPos blockPos = chunkBlockPos.add(random.nextInt(16) + 8,
                                                random.nextInt(16),
                                                random.nextInt(16) + 8);
          if (Variant.isVariant(world.getBlockState(blockPos), RAW) &&

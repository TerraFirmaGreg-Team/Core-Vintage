...
public final class AnnotationUtils {
  ...
  public static void of(FMLConstructionEvent event) {
   ...
   * @return A list of all classes annotated with the annotation, as instances.
   */
  public static <T, A extends Annotation> Map<T, A> getAnnotations(ASMDataTable table, Class<A> annotation, Class<T> instance, Function<A, Boolean> createInstance) {
    ...
    for (final ASMData asmData : getData(table, annotation)) {
      ...
      try {
        ...
        final A asmAnnotation = asmInstanceClass.getAnnotation(annotation);
        if (createInstance.apply(asmAnnotation)) {
          map.put(asmInstanceClass.getDeclaredConstructor().newInstance(), asmAnnotation);
        }

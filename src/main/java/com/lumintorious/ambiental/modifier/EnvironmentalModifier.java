...
import static com.lumintorious.ambiental.capability.TemperatureCapability.AVERAGE;
...
//		if(EnvironmentalTemperatureProvider.calculateEnclosure(player, 30) && temp < avg - 1) {
	...
	public static Optional<TempModifier> handleFood(EntityPlayer player) {
		if (getEnvironmentTemperatureWithTimeOfDay(player) < TFCAmbientalConfig.GENERAL.averageTemperature - 3 && player.getFoodStats()
		                                                                                                                .getFoodLevel() > 14) {

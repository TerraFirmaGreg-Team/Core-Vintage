...
public class BlockWaterGlowPlant extends BlockFluidTFC implements ICapabilitySize, IPlantable {
  ...
  public boolean canPlaceBlockAt(World worldIn, BlockPos pos) {
    for (EnumFacing face : EnumFacing.values()) {
      ...
      if ((blockState.getBlockFaceShape(worldIn, pos.offset(face), face.getOpposite()) == BlockFaceShape.SOLID || BlocksTFC.isGround(blockState)
  ...
  public AxisAlignedBB getSelectedBoundingBox(IBlockState state, World worldIn, BlockPos pos) {
    if (state.getBlock() instanceof BlockWaterGlowPlant) {return state.getBoundingBox(worldIn, pos).offset(pos);}
    return null;
  }

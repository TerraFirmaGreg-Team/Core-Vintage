...
public class BlockUrnLoot extends Block implements ICapabilitySize {
  ...
  private static ImmutableList<Item> getSeeds() {
    if (seedsList == null) {
      ...
      for (Item item : ItemsTFC.getAllSimpleItems()) {if (item.getRegistryName().getPath().contains("seeds/")) {buildSeeds.add(item);}}
      for (Item item : ItemsTFCF.getAllSimpleItems()) {if (item.getRegistryName().getPath().contains("seeds/")) {buildSeeds.add(item);}}

      if (TFCFlorae.FirmaLifeAdded) {
        for (Item item : ItemsFL.getAllEasyItems()) {if (item.getRegistryName().getPath().contains("seeds/")) {buildSeeds.add(item);}}
      }
      if (TFCFlorae.TFCPHCompatAdded) {
        for (Item item : ItemsTPC.getAllSimpleItems()) {if (item.getRegistryName().getPath().contains("seeds/")) {buildSeeds.add(item);}}
      }
  ...
  private static Metal getRandomMetal(Random rand) {
    if (metalList == null) {
      ...
      for (Metal metal : TFCRegistries.METALS.getValuesCollection()) {if (metal.getTier().isAtMost(Metal.Tier.TIER_III)) {buildMetal.add(metal);}}
...
//    } else {
    ...
    for (int i = 0; i < count; i++) {
      Item[] dropList = {ItemsTFCF.MADDER, ItemsTFCF.WELD, ItemsTFCF.WOAD, ItemsTFCF.INDIGO, ItemsTFCF.RAPE, ItemsTFCF.HOPS, ItemsTFCF.FLAX,
                         ...
                         ItemsCore.WOOL.get(), ItemsCore.WOOL_YARN.get(),

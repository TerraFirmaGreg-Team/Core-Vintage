...
public class BlockHangingPlantTFC extends BlockCreepingPlantTFC implements IGrowable {
  ...
  public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
    ...
    if (worldIn.getBlockState(pos.down()).getBlock() == this && actualState.getValue(UP) && !(Boolean) actualState.getValue(DOWN)
    ...
    if (worldIn.getBlockState(pos.up()).getBlock() == this && !(Boolean) actualState.getValue(UP) && !(Boolean) actualState.getValue(NORTH)
  ...
  public boolean canBlockStay(World worldIn, BlockPos pos, IBlockState state) {
    ...
    for (int var6 = 0; var6 < var5; ++var6) {
      ...
      if (material == Material.LEAVES || worldIn.getBlockState(pos.up()).getBlock() == this) {
        return this.plant.isValidTemp(Climate.getActualTemp(worldIn, pos)) && this.plant.isValidRain(ChunkDataTFC.getRainfall(worldIn, pos));
      }
    }
  ...
  protected BlockStateContainer createPlantBlockState() {
    return new BlockStateContainer(this, DOWN, UP, NORTH, EAST, WEST, SOUTH, this.growthStageProperty, DAYPERIOD, AGE, BOTTOM);
  }
  ...
  protected boolean canPlantConnectTo(IBlockAccess world, BlockPos pos, EnumFacing facing) {
    return
      !super.canPlantConnectTo(world, pos, facing) && world.getBlockState(pos.up()).getBlock() == this && facing != EnumFacing.DOWN && facing != EnumFacing.UP
      ? this.canPlantConnectTo(world, pos.up(), facing) : super.canPlantConnectTo(world, pos, facing);
  }
    ...
    if (!(Boolean) state.getValue(BOTTOM)) {
      ...
      for (int var7 = 0; var7 < var6; ++var7) {
        ...
        if (worldIn.isAirBlock(sidePos) && worldIn.isAirBlock(sidePos.down()) && (!sideMaterial.isSolid() || sideMaterial == Material.LEAVES)
            && this.canBlockStay(worldIn, sidePos.down(), state)) {
          flag = true;
        }
      }
    }

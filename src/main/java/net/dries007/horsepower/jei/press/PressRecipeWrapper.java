...
public class PressRecipeWrapper implements IRecipeWrapper {
  ...
  public void getIngredients(IIngredients ingredients) {
    ...
    if (isFluid) {ingredients.setOutput(VanillaTypes.FLUID, fluidOutput);} else {ingredients.setOutput(VanillaTypes.ITEM, output);}
  }
  ...
  public boolean equals(Object o) {
    ...
    for (ItemStack stack : inputs.get(0)) {
      for (ItemStack stack1 : that.inputs.get(0)) {
        if (stack1.getMetadata() == OreDictionary.WILDCARD_VALUE && !OreDictionary.itemMatches(stack, stack1, false)) {flag = false;}
      }
    }

    return flag && (output != null && that.output != null && output.equals(that.output) || (fluidOutput != null && that.fluidOutput != null
                                                                                            && fluidOutput.equals(that.fluidOutput)));
  }
}

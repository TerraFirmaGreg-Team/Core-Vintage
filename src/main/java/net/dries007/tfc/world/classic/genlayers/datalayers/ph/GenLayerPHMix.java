  ...
  public int[] getInts(int x, int z, int xSize, int zSize) {
    ...
    for (int var7 = 0; var7 < zSize; ++var7) {
      for (int var8 = 0; var8 < xSize; ++var8) {
        ...
        if (id0 >= thisID + 2 || id1 >= thisID + 2 || id2 >= thisID + 2 || id3 >= thisID + 2) {if (thisID + 1 < GenPHLayer.MAX) {thisID++;}}
        if (id0 <= thisID - 2 || id1 <= thisID - 2 || id2 <= thisID - 2 || id3 <= thisID - 2) {if (thisID - 1 > GenPHLayer.MIN) {thisID--;}}

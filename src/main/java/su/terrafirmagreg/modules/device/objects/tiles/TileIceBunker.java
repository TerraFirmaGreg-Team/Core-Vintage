...
import su.terrafirmagreg.modules.device.objects.containers.ContainerIceBunker;
...
public class TileIceBunker extends TileEntityLockableLoot
        implements IInventory, ITickable, IAmbientalTileProvider, IProviderContainer<ContainerIceBunker, GuiIceBunker> {
                ...
                if (lastUpdate < (int) Calendar.CALENDAR_TIME.getTotalDays()) {
                    if (outsideTemp > -10) {    //magic
                        ...
                        coolantAmount = coolantAmount -
                                (int) (ModConfig.coolantConsumptionMultiplier * (0.05 * volume * (1 + lossMult) * (outsideTemp + volume + 2)));
    ...
    private float doorsLossMult() {
        ...
        if (door == BlocksDevice.CELLAR_DOOR && BlockDoor.isOpen(world, new BlockPos(posX, posY, posZ))) {
        ...
        if (door == BlocksDevice.CELLAR_DOOR && BlockDoor.isOpen(world, new BlockPos(posX + entrance[2], posY, posZ + entrance[3]))) {

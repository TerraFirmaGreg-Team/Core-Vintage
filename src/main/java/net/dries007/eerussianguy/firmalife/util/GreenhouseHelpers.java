package net.dries007.eerussianguy.firmalife.util;
...
public class GreenhouseHelpers {
  ...
  private static boolean isGoodEndWallSide(World world, BlockPos pos, EnumFacing wallFacing, EnumFacing inward, boolean visual) {
    ...
    if (!(world.getBlockState(pos.offset(inward.getOpposite()).offset(wallFacing.getOpposite()))
               .getBlock() instanceof BlockGreenhouseWall)
        || !(world.getBlockState(pos.offset(inward, length).offset(wallFacing.getOpposite()))
                  .getBlock() instanceof BlockGreenhouseWall)
        || length < 3) {
      return false;
    }
    ...
    while (length > 0) {
      ...
      for (int i = 0; i < length; i++) {
        BlockPos newCheckPos = checkPos.offset(inward, i);
        checkState = world.getBlockState(newCheckPos);
        if (!validEndWallBlock(world, newCheckPos, checkState, wallFacing, visual)) {
          return false;
        }

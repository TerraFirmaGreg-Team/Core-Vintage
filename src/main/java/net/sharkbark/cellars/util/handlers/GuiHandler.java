...
public class GuiHandler implements IGuiHandler {
	...
	public Object getClientGuiElement(int ID, EntityPlayer entityPlayer, World world, int x, int y, int z) {
		...
		if (ID == Reference.GUI_CELLAR_SHELF) {
			...
			//te.isOpen += 1;
			return new GuiCellarShelf(container, entityPlayer.inventory, te, world.getBlockState(new BlockPos(x, y, z))
			                                                                      .getBlock()
			                                                                      .getTranslationKey());
		}
		if (ID == Reference.GUI_ICE_BUNKER) {
			TEIceBunker te = (TEIceBunker) world.getTileEntity(new BlockPos(x, y, z));
			return new GuiIceBunker(container, entityPlayer.inventory, te, world.getBlockState(new BlockPos(x, y, z))
			                                                                    .getBlock()
			                                                                    .getTranslationKey());
		}
		...
		if (ID == Reference.GUI_FREEZE_DRYER) {
			...
			return new GuiFreezeDryer(container, entityPlayer.inventory, te, world.getBlockState(new BlockPos(x, y, z))
			                                                                      .getBlock()
			                                                                      .getTranslationKey());
		}
		return null;

...
import su.terrafirmagreg.framework.manager.content.base.block.spi.BaseBlockLog;
...
public class BlockWoodLog extends BaseBlockLog implements IWoodEntry, IProviderBlockColor {
  ...
  public void harvestBlock(World worldIn, EntityPlayer player, BlockPos pos, IBlockState state, @Nullable TileEntity tile, ItemStack stack) {
    ...
    } else if (toolClasses.contains(ToolClasses.HARD_HAMMER) && ConfigWood.MISC.enableHammerSticks) {
  ...
  public boolean removedByPlayer(IBlockState state, World world, BlockPos pos, EntityPlayer player, boolean willHarvest) {
    ...
    if (toolClasses.contains(ToolClasses.AXE) && !toolClasses.contains(ToolClasses.SAW)) {
      ...
      if (!state.getValue(PLACED) && ConfigWood.MISC.enableFelling) {
        ...
        if (!removeTree(world, pos, player, stack, OreDictUtils.contains(stack, "axeStone") || OreDictUtils.contains(stack, "hammerStone"))) {
      ...
      return world.setBlockState(pos, Blocks.AIR.getDefaultState(), world.isRemote ? 11 : 3);
    } else if (!toolClasses.contains(ToolClasses.SAW) && ConfigWood.MISC.requiresAxe) {
  ...
  public boolean isToolEffective(String type, IBlockState state) {
    return (ToolClasses.HARD_HAMMER.equals(type) && ConfigWood.MISC.enableHammerSticks) || super.isToolEffective(type, state);
  }

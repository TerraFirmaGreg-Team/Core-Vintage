...
public class BlockSoilGrass extends BlockGrass implements ISoilBlock {
    ...
    public static void spreadGrass(World world, BlockPos pos, IBlockState us, Random rand) {
        ...
        if (up.getMaterial().isLiquid() || (neighborLight < 4 && up.getLightOpacity(world, upPos) > 2)) {
            ...
            if (usBlock instanceof BlockPeat) {
                ...
                world.setBlockState(pos, StorageSoil.getSoilBlock(soil.getBlockVariant().getNonGrassVersion(), soil.getType()).getDefaultState());
            }
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                if (!Helpers.isSoil(current) || Helpers.isGrass(current)) {
                    continue;
                }
                ...
                if (currentBlock instanceof BlockPeat) {
                    ...
                    world.setBlockState(pos, StorageSoil.getSoilBlock(block.getBlockVariant().getGrassVersion(spreader), block.getType()).getDefaultState());
                }
            }
        ...
        if (!worldIn.isRemote) {
            ...
            if (block instanceof ISoilBlock) {
                ...
                if (worldIn.getLightFromNeighbors(pos.up()) < 4 && worldIn.getBlockState(pos.up()).getLightOpacity(worldIn, pos.up()) > 2) {
                    worldIn.setBlockState(pos, StorageSoil.getSoilBlock(DIRT, soil).getDefaultState());
                } else {
                    if (worldIn.getLightFromNeighbors(pos.up()) >= 9) {
                        for (int i = 0; i < 4; ++i) {
                            ...
                            if (iblockstate1.getBlock() == StorageSoil.getSoilBlock(DIRT, soil) && worldIn.getLightFromNeighbors(blockpos.up()) >= 4 && iblockstate.getLightOpacity(worldIn, pos.up()) <= 2) {
                                worldIn.setBlockState(blockpos, StorageSoil.getSoilBlock(GRASS, soil).getDefaultState());
                            }
                        }
                    }
                }
    ...
    public Item getItemDropped(@Nonnull IBlockState state, @Nonnull Random rand, int fortune) {
        return StorageSoil.getSoilItem(PILE, type);
    }
    ...
    public boolean canSustainPlant(@Nonnull IBlockState state, @Nonnull IBlockAccess world, @Nonnull BlockPos pos, @Nonnull EnumFacing direction, @Nonnull IPlantable plantable) {
        ...
        if (plantable instanceof BlockPlant) {
            switch (((BlockPlant) plantable).getEnumType()) {
                ...
                case DESERT_CLAY -> {
                    return variant == CLAY || variant == CLAY_GRASS || Helpers.isSand(state);
                }
                case DRY_CLAY -> {
                    return variant == DIRT || variant == DRY_GRASS ||
                            variant == CLAY || variant == CLAY_GRASS || Helpers.isSand(state);
                }
                case DRY -> {
                    return variant == DIRT || variant == DRY_GRASS || Helpers.isSand(state);
                }
                case FRESH_WATER -> {
                    return variant == DIRT || variant == GRASS ||
                            variant == DRY_GRASS || Helpers.isGravel(state);
                }
                case SALT_WATER -> {
                    return variant == DIRT || variant == GRASS ||
                            variant == DRY_GRASS || Helpers.isSand(state) || Helpers.isGravel(state);
                }
                case FRESH_BEACH -> {
                    boolean flag = false;
                    ...
                    return (variant == DIRT || variant == GRASS ||
                            variant == DRY_GRASS || Helpers.isSand(state)) && flag;
                }
                case SALT_BEACH -> {
                    boolean flag = false;
                    for (EnumFacing facing : EnumFacing.HORIZONTALS) {

...
public class BlockThatchBed extends BlockBed implements IItemProvider {
    ...
    public void dropBlockAsItemWithChance(World worldIn, BlockPos pos, IBlockState state, float chance, int fortune) {
        if (state.getValue(BlockBed.PART) == BlockBed.EnumPartType.HEAD) {
            Block.spawnAsEntity(worldIn, pos, new ItemStack(ItemAnimalHide.get(ItemAnimalHide.HideType.RAW, ItemAnimalHide.HideSize.LARGE)));
            Block.spawnAsEntity(worldIn, pos, new ItemStack(BlocksCore.THATCH, 2));
        }
    }

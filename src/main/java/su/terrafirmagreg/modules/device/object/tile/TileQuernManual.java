...
import su.terrafirmagreg.modules.device.object.container.ContainerQuern;
...
import static net.minecraft.init.SoundEvents.ENTITY_ARMORSTAND_FALL;
import static net.minecraft.init.SoundEvents.ENTITY_ITEM_BREAK;

public class TileQuernManual extends BaseTileTickableInventory implements IProviderContainer<ContainerQuern, GuiQuern> {
    ...
    if (rotationTimer > 0) {
      ...
      if (rotationTimer == 0) {
        ...
        if (inventory.getStackInSlot(SLOT_HANDSTONE).isEmpty()) {
          for (int i = 0; i < 15; i++) {
            world.spawnParticle(EnumParticleTypes.ITEM_CRACK, pos.getX() + 0.5D,
                                pos.getY() + 0.875D, pos.getZ() + 0.5D,
                                (world.rand.nextDouble() - world.rand.nextDouble()) / 4,
                                world.rand.nextDouble() / 4,
                                (world.rand.nextDouble() - world.rand.nextDouble()) / 4,
                                Item.getIdFromItem(ItemsDevice.HANDSTONE));
          }
    ...
    if (!inputStack.isEmpty()) {
      ...
      if (recipe != null && !world.isRemote) {
        ...
        inventory.setStackInSlot(SLOT_OUTPUT,
                                 CapabilityFood.mergeItemStacksIgnoreCreationDate(inventory.getStackInSlot(SLOT_OUTPUT),
                                                                                  outputStack));
        if (!outputStack.isEmpty()) {
          // Still having leftover items, dumping in world
          StackUtils.spawnItemStack(world, pos, outputStack);
        }
      }
    }
  }
  ...
  @Override
  public ContainerQuern getContainer(InventoryPlayer inventoryPlayer, World world,
                                     IBlockState state, BlockPos pos) {
    return new ContainerQuern(inventoryPlayer, this);
  }

  @Override
  public GuiQuern getGuiContainer(InventoryPlayer inventoryPlayer, World world, IBlockState state,
                                  BlockPos pos) {

package su.terrafirmagreg.old.core.modules.ambiental.modifier;

import su.terrafirmagreg.old.core.config.TFGConfig;
import su.terrafirmagreg.old.core.modules.ambiental.api.AmbientalRegistry;
import su.terrafirmagreg.old.core.modules.ambiental.api.IEnvironmentalTemperatureProvider;
import su.terrafirmagreg.old.core.modules.ambiental.effects.TempEffect;
...
import static su.terrafirmagreg.old.core.modules.ambiental.capability.TemperatureCapability.AVERAGE;
...
public class EnvironmentalModifier {
  ...
  public static float getEnvironmentTemperature(EntityPlayer player) {
    ...
    if (TFGConfig.GENERAL.harsherTemperateAreas) {
      ...
      float mult0 = Math.max(0f, TFGConfig.GENERAL.harsherMultiplier - 1f);
  ...
  public static Optional<TempModifier> handleTimeOfDay(EntityPlayer player) {
    ...
    if (dayTicks < 6000) {return TempModifier.defined("morning", 2f, 0);} else if (dayTicks < 12000) {
...
//		if(EnvironmentalTemperatureProvider.calculateEnclosure(player, 30) && temp < avg - 1) {
  ...
  public static Optional<TempModifier> handleFood(EntityPlayer player) {
    if (getEnvironmentTemperatureWithTimeOfDay(player) < TFGConfig.GENERAL.averageTemperature - 3 && player.getFoodStats()
                                                                                                           .getFoodLevel() > 14) {
  ...
  public static Optional<TempModifier> handleDiet(EntityPlayer player) {
    if (player.getFoodStats() instanceof IFoodStatsTFC stats) {
      ...
      if (getEnvironmentTemperatureWithTimeOfDay(player) > TFGConfig.GENERAL.hotThreshold) {

...
public class BlockFirePit extends BlockBase implements IBellowsConsumerBlock, ILightableBlock, IHasModel {
    ...
    public BlockFirePit() {
        super(Material.WOOD);
        setDefaultState(blockState.getBaseState().withProperty(ILightableBlock.LIT, false).withProperty(ATTACHMENT, FirePitAttachment.NONE));
        disableStats();
    ...
    public IBlockState getStateFromMeta(int meta) {
        return this.getDefaultState().withProperty(ILightableBlock.LIT, (meta & 1) > 0).withProperty(ATTACHMENT, FirePitAttachment.valueOf(meta >> 1));
    }

    @Override
    public int getMetaFromState(IBlockState state) {
        return (state.getValue(ILightableBlock.LIT) ? 1 : 0) + (state.getValue(ATTACHMENT).ordinal() << 1);
    }
    ...
    public void addCollisionBoxToList(IBlockState state, World worldIn, BlockPos pos, AxisAlignedBB entityBox, List<AxisAlignedBB> collidingBoxes, @Nullable Entity entityIn, boolean isActualState) {
        Block.addCollisionBoxToList(pos, entityBox, collidingBoxes, FIREPIT_AABB);
        if (state.getValue(ATTACHMENT) != FirePitAttachment.NONE) {
            Block.addCollisionBoxToList(pos, entityBox, collidingBoxes, ATTACHMENT_COLLISION_ADDITION_AABB);
        }
    }
    ...
    public void randomTick(World worldIn, BlockPos pos, IBlockState state, Random random) {
        ...
        if (te != null && state.getValue(ILightableBlock.LIT) && worldIn.isRainingAt(pos.up())) {
            te.onRainDrop();
        }
    }
    ...
    public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random rng) {
        if (!state.getValue(ILightableBlock.LIT)) return;
        ...
        if ((state.getValue(ATTACHMENT) == FirePitAttachment.GRILL)) {
            ...
            if (tile != null) {
                ...
                if (cap != null) {
                    ...
                    if (state.getValue(ILightableBlock.LIT) && anythingInInv) {
    ...
    public void onEntityWalk(World worldIn, BlockPos pos, Entity entityIn) {
        ...
        if (state.getValue(ILightableBlock.LIT) && !entityIn.isImmuneToFire() && entityIn instanceof EntityLivingBase) {
    ...
    protected BlockStateContainer createBlockState() {
        return new BlockStateContainer(this, ILightableBlock.LIT, ATTACHMENT);
    }

    @Override
    public int getLightValue(IBlockState state, IBlockAccess world, BlockPos pos) {
        return state.getValue(ILightableBlock.LIT) ? super.getLightValue(state, world, pos) : 0;
    }
    ...
    public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
        drops.add(new ItemStack(ItemsCore.WOOD_ASH, 3 + Constants.RNG.nextInt(5)));
    }

...
public class BlockPitKiln extends BlockBase implements ILightableBlock, IHasModel {
    ...
    private static final AxisAlignedBB[] AABB_LEVELS = new AxisAlignedBB[]{
            BlockPlacedItem.PLACED_ITEM_AABB,
    ...
    public BlockPitKiln() {
        ...
        setHardness(0.5f);
        setDefaultState(blockState.getBaseState().withProperty(FULL, false).withProperty(ILightableBlock.LIT, false));
    ...
    public IBlockState getStateFromMeta(int meta) {
        return getDefaultState().withProperty(ILightableBlock.LIT, (meta & 1) > 0).withProperty(FULL, (meta & 2) > 0);
    }

    @Override
    public int getMetaFromState(IBlockState state) {
        return (state.getValue(ILightableBlock.LIT) ? 1 : 0) + (state.getValue(FULL) ? 2 : 0);
    }
    ...
    protected BlockStateContainer createBlockState() {
        return new BlockStateContainer(this, FULL, ILightableBlock.LIT);
    }
    ...
    public boolean isBurning(IBlockAccess world, BlockPos pos) {
        return world.getBlockState(pos).getActualState(world, pos).getValue(ILightableBlock.LIT);
    }
    ...
    public int getFireSpreadSpeed(IBlockAccess world, BlockPos pos, EnumFacing face) {
        return world.getBlockState(pos).getActualState(world, pos).getValue(ILightableBlock.LIT) ? 120 : 0; // Twice as much as the highest vanilla level (60)
    }

...
import su.terrafirmagreg.modules.core.object.potion.PotionCaffeine;
...
public final class PotionsCore {
  ...
  public static void onRegister(RegistryManager manager) {
    OVERBURDENED = manager.potion("overburdened", new PotionOverburdened());
    THIRST = manager.potion("thirst", new PotionThirst());
    PARASITES = manager.potion("parasites", new PotionParasites());
    SWARM = manager.potion("swarm", new PotionSwarm());
    HYPERTHERMIA = manager.potion("hyperthermia", new PotionHyperthermia());
    HYPOTHERMIA = manager.potion("hypothermia", new PotionHypothermia());
    COLD_RESIST = manager.potion("resist_cold", new PotionResistCold());
    HEAT_RESIST = manager.potion("resist_heat", new PotionResistHeat());
    CAFFEINE = manager.potion("caffeine", new PotionCaffeine());

    COLD_RESIST_TYPE = manager.potionType(COLD_RESIST, "cold_resist_type", 1200);           //TODO cfg duration
    LONG_COLD_RESIST_TYPE = manager.potionType(COLD_RESIST, "long_cold_resist_type", 2400); //TODO cfg duration
    HEAT_RESIST_TYPE = manager.potionType(HEAT_RESIST, "heat_resist_type", 1200);           //TODO cfg duration
    LONG_HEAT_RESIST_TYPE = manager.potionType(HEAT_RESIST, "long_heat_resist_type", 2400); //TODO cfg duration
  }
}

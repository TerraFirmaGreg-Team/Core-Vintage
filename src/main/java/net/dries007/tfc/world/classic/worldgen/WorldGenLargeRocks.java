...
public class WorldGenLargeRocks implements IWorldGenerator {
  ...
  public void generate(Random rng, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
    ...
    while (y-- > -2 && !isFlatEnough) {
      ...
      for (int x = -6; x <= 6; x++) {
        for (int z = -6; z <= 6; z++) {
          if (!world.getBlockState(start.add(x, y, z)).isBlockNormalCube()) {continue outer;}
        }
      }
      isFlatEnough = true;
    }
    ...
    if (rng.nextInt(1) == 0) {
      genFromPoint(world, rng, start.add(
        (rng.nextInt(2) + 1) * (rng.nextBoolean() ? 1 : -1),
        y + (rng.nextInt(2) + 1) * (rng.nextBoolean() ? 1 : -1), (rng.nextInt(2) + 1) * (rng.nextBoolean() ? 1 : -1)));
    }
  }

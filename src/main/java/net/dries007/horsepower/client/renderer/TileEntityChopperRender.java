...
public class TileEntityChopperRender extends TileEntityHPBaseRenderer<TileEntityChopper> {
  ...
  public void render(TileEntityChopper te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
    ...
    } else {dispatcher.getBlockModelRenderer().renderModel(te.getWorld(), bladeModel, blockState, te.getPos(), buffer, false);}
    ...
    if (te.hasWorker()) {renderLeash(te.getWorker(), x, y, z, 0D, 1.1D, 0D, partialTicks, te.getPos());}

    GlStateManager.pushMatrix();
    GlStateManager.translate(x, y, z);
    if (!te.getStackInSlot(0).isEmpty()) {renderStillItem(te, te.getStackInSlot(0), 0.5F, 0.54F, 0.5F, 1.3F);}
    GlStateManager.popMatrix();

    GlStateManager.pushMatrix();
    GlStateManager.translate(x, y, z);
    if (!te.getStackInSlot(1).isEmpty()) {renderStillItem(te, te.getStackInSlot(1), 0.5F, 0.54F, 0.5F, 1.3F);}
    GlStateManager.popMatrix();
    ...
    if (!te.isValid()) {RenderUtils.renderInvalidArea(te.getWorld(), te.getPos(), 0);}
    GlStateManager.popMatrix();
  }
}

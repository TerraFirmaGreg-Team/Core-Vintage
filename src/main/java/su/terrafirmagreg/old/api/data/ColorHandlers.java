package su.terrafirmagreg.old.api.data;

import su.terrafirmagreg.old.api.library.MCColor;
...
/**
 * A client side only class, which contains references to reusable color handlers for items and blocks.
 */
...
public class ColorHandlers {
  ...
  /**
   * A reusable color handler which uses a MCColor which is read from {@link net.minecraft.tileentity.TileEntity#getTileData()}.
   */
  public static IBlockColor BLOCK_MCCOLOR = (state, world, pos, index) ->
    MCColor.isAcceptable(world, pos) ? new MCColor(world, pos).getRGB() : MCColor.WHITE.getRGB();

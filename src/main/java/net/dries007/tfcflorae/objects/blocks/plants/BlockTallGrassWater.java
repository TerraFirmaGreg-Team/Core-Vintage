...
public class BlockTallGrassWater extends BlockShortGrassTFCF implements IGrowable, ITallPlant {
  ...
  protected boolean canSustainBush(IBlockState state) {
    return (BlocksTFC.isWater(state) || state.getMaterial() == Material.ICE && state == plant.getWaterType()) || (state.getMaterial() == Material.CORAL
                                                                                                                  && !(state.getBlock() instanceof BlockEmergentTallWaterPlantTFC));
  }
  ...
  public boolean canBlockStay(World worldIn, BlockPos pos, IBlockState state) {
    ...
    if (state.getBlock() == this) {
      ...
      return ((soil.getBlock().canSustainPlant(soil, worldIn, pos.down(), net.minecraft.util.EnumFacing.UP, this)) || (
        (material == Material.WATER && stateDown.getValue(BlockLiquid.LEVEL) == 0 && stateDown == plant.getWaterType()) || material == Material.ICE || (

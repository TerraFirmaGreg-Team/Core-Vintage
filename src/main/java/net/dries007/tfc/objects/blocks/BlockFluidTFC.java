...
public class BlockFluidTFC extends BlockFluidClassic {
  ...
  public void updateTick(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state, @Nonnull Random rand) {
    ...
    if (!isSourceBlock(world, pos)) {
      ...
      if (newState.getBlock() == this) {currentMeta = newState.getValue(LEVEL);}
      ...
      if (world.getBlockState(pos.down(densityDir)).getBlock() != this) {
        for (EnumFacing side : EnumFacing.HORIZONTALS) {
          ...
          if (block instanceof BlockFluidTFC) {
            ...
            if (neighborAboveBlock == neighborBlock) {neighborMeta = 0;} else {neighborMeta = neighborState.getValue(LEVEL);}
    ...
    if (displaceIfPossible(world, pos)) {
      ...
      if (targetBlock instanceof BlockFluidTFC) {
        ...
        if (!replace) {
          ...
          if (((BlockFluidTFC) targetBlock).getDensity() > this.getDensity()) {flowStrength += 1;}

          if (flowStrength > meta) {replace = true;}
        }
      }
  ...
  public float getFluidHeightForRender(IBlockAccess world, BlockPos adjPos, @Nonnull IBlockState upState) {
    ...
    if (isMergeableFluid(adjState)) {
      ...
      if (adjBlock == this || adjBlock instanceof BlockLiquid) {return super.getFluidHeightForRender(world, adjPos, upState);} else {
        return ((BlockFluidBase) adjBlock).getFluidHeightForRender(world, adjPos, upState);
      }
    }

    // adjacent solid
    if (adjState.getMaterial().isSolid()) {return -1;}

                ...
                for (int z = zMin; z < zMax; ++z) {
                    ...
                    for (int y = yMax - 1; y >= yMin; --y) {
                        ...
                        if (!((xNormalized * xNormalized + zNormalized * zNormalized) * multipliers[y] + yNormalized * yNormalized / 6.0D < 1.0D))
                            continue;
                        if (!BlocksTFC_old.isGround(primer.getBlockState(x, y, z))) continue;

                        for (int upCount = 1; BlocksTFC_old.isSoilOrGravel(primer.getBlockState(x, y + upCount, z)); upCount++)

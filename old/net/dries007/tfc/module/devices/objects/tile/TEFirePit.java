...
import net.dries007.tfc.module.food.api.types.category.FoodCategory;
import net.dries007.tfc.module.food.objects.items.ItemDynamicBowlFood;
...
public class TEFirePit extends TETickableInventory implements ICalendarTickable, ITileFields, IItemHandlerSidedCallback {
    ...
    public void onRainDrop() {
        ...
        world.playSound(null, pos, SoundEvents.BLOCK_LAVA_EXTINGUISH, SoundCategory.BLOCKS, 0.8f, 0.8f + net.dries007.tfc.util.Constants.RNG.nextFloat() * 0.4f);
    }
        ...
        if (!world.isRemote) {
            ...
            if (state.getValue(ILightableBlock.LIT)) {
                ...
                if (burnTicks <= 0) {
                    ...
                    if (stack.isEmpty()) {
                        world.setBlockState(pos, state.withProperty(ILightableBlock.LIT, false));
                        burnTicks = 0;
                        burnTemperature = 0;
                    } else {
            ...
            BlockFirePit.FirePitAttachment attachment = state.getValue(BlockFirePit.ATTACHMENT);
    ...
    public void setAndUpdateSlots(int slot) {
        ...
        if (world.getBlockState(pos).getValue(BlockFirePit.ATTACHMENT) == BlockFirePit.FirePitAttachment.GRILL) {
    ...
    public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
        ...
        BlockFirePit.FirePitAttachment attachment = world.getBlockState(pos).getValue(BlockFirePit.ATTACHMENT);
    ...
    private Item getSoupItem() {
        return switch (soupNutrient) {
            case Nutrient.GRAIN -> StorageFood.getFoodItem(FoodItemVariants.INGREDIENT, FoodTypes.SOUP_GRAIN); //TODO
            case Nutrient.VEGETABLES -> StorageFood.getFoodItem(FoodItemVariants.INGREDIENT, FoodTypes.SOUP_VEGETABLE);
            case Nutrient.FRUIT -> StorageFood.getFoodItem(FoodItemVariants.INGREDIENT, FoodTypes.SOUP_FRUIT);
            case Nutrient.PROTEIN -> StorageFood.getFoodItem(FoodItemVariants.INGREDIENT, FoodTypes.SOUP_MEAT);
            default -> StorageFood.getFoodItem(FoodItemVariants.INGREDIENT, FoodTypes.SOUP_DAIRY);
        };
    }

...
public class GuiKnapping extends GuiContainerTFC {
    ...
    if (clickedMouseButton == 0) {
      for (GuiButton button : this.buttonList) {
        if (button instanceof GuiButtonKnapping && button.mousePressed(mc, mouseX, mouseY)) {
          ...
          if (MinecraftForge.EVENT_BUS.post(event)) {break;} else if (selectedButton == event.getButton()) {continue;}
  ...
  protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
    ...
    if (type == KnappingTypes.CLAY || type == KnappingTypes.FIRE_CLAY) {
      ...
      mc.getTextureManager().bindTexture(type == KnappingTypes.CLAY ? TFCGuiHandler.CLAY_DISABLED_TEXTURE : TFCGuiHandler.FIRE_CLAY_DISABLED_TEXTURE);
      for (GuiButton button : buttonList) {
        if (!button.visible) {

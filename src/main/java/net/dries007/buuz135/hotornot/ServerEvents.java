package net.dries007.buuz135.hotornot;
...
import net.dries007.buuz135.hotornot.config.HotConfig;
import net.dries007.buuz135.hotornot.config.HotLists;
import net.dries007.buuz135.hotornot.object.item.ItemHotHolder;
import net.dries007.buuz135.hotornot.util.ItemEffect;
...
public class ServerEvents {
  ...
  public static void onTick(final WorldTickEvent event) {
    ...
    for (final EntityPlayer player : event.world.playerEntities) {
        ...
        for (int slotIndex = 0; slotIndex < playerItemHandler.getSlots(); slotIndex++) {
          ...
          for (final ItemEffect effect : ItemEffect.values()) {
            if (!effect.stackHasEffect(slotStack)) {
              ...
              if (!ItemEffect.contentsHaveEffect(internalHandler, effect)) {
                continue;
              }
            }
      ...
      if (preventedEffect) {
        ...
        if (HotConfig.EFFECT_HANDLING.damageRate == 0) {
          continue;
        }
        ...
        if (!shouldReplaceHotHolder(player)) {
          continue;
        }
  ...
  private static ItemStack findAndExtractHotHolder(final IItemHandler playerItemHandler) {
    for (int slotIndex = 0; slotIndex < playerItemHandler.getSlots(); slotIndex++) {
      ...
      if (!(slotStack.getItem() instanceof ItemHotHolder)) {
        continue;
      }

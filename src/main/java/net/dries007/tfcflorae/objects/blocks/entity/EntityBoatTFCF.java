package net.dries007.tfcflorae.objects.blocks.entity;
...
import net.dries007.tfcflorae.objects.items.ItemBoatTFCF;
...
public class EntityBoatTFCF extends EntityBoat {
  ...
  public IFruitTree getWood() {
    ...
    for (IFruitTree fruitTree : FruitTree.values()) {
      ...
      if (this.dataManager.get(WOOD_NAME) == name) {return fruitTree;}
    }
  ...
  public Tree getTree() {
    for (int i = 0; i < BlocksTFCF.bamboo.length; i++) {
      if (this.dataManager.get(WOOD_NAME) == BlocksTFCF.bamboo[i]) {return BlocksTFCF.bambooTrees[i];}
    }

    if (this.dataManager.get(WOOD_NAME) == "cassia_cinnamon") {return TreesTFCF.CASSIA_CINNAMON_TREE;}

    if (this.dataManager.get(WOOD_NAME) == "ceylon_cinnamon") {return TreesTFCF.CEYLON_CINNAMON_TREE;}

    if (TFCFlorae.FirmaLifeAdded) {
      if (this.dataManager.get(WOOD_NAME) == "cinnamon") {return PlantsFL.CINNAMON_TREE;}
    }

...
public abstract class EntityAnimalBase extends EntityAnimal implements IAnimal {
    ...
    public static void addCommonPreyAI(EntityAnimal entity, double speedMult) {
        ...
        entity.tasks.addTask(4,
                new EntityAIAvoidEntity<>(entity, EntityAnimalMammal.class, Predicates.instanceOf(IPredator.class), 12.0F, farSpeed, nearSpeed));
    ...
    public TextComponentTranslation getAnimalName() {
        ...
        return new TextComponentTranslation(ModUtils.localize("animal." + entityString + "." + this.getGender().name().toLowerCase()));
    }
    ...
    public void onLivingUpdate() {
        ...
        if (!this.world.isRemote) {
            ...
            if (this instanceof ILivestock) {
                ...
                if (this.getDaysToElderly() > 0 && this.getFamiliarity() < 0.10F && (this.getDaysToElderly() + this.getDaysToAdulthood()) * 1.25F <=
                        CalendarTFC.PLAYER_TIME.getTotalDays() - this.getBirthDay()) {
                    this.setDead();
                }
            }

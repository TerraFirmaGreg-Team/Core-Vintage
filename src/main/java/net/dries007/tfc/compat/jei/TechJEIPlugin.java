...
import su.terrafirmagreg.modules.device.content.gui.GuiSmelteryCauldron;
...
//    registry.addRecipeCatalyst(new ItemStack(TechBlocks.WIRE_DRAW_BENCH), WIRE_DRAWING_UID);
    ...
    List<TechUnmoldRecipeWrapper> unmoldList = new ArrayList<>();
    List<TechCastingRecipeWrapper> castingList = new ArrayList<>();
        ...
        if (ObfuscationReflectionHelper.getPrivateValue(Metal.class, metal, "usable").equals(true)) {
          for (ItemTechMetal.ItemType type : ItemTechMetal.ItemType.values()) {
            if (type.hasMold() && ItemTechMetal.get(metal, type) != null) {
              unmoldList.add(new TechUnmoldRecipeWrapper(metal, type));
              castingList.add(new TechCastingRecipeWrapper(metal, type));
            }
          }
        }
      });
    ...
    unmoldList.add(new TechUnmoldRecipeWrapper(input, new ItemStack(Blocks.GLASS)));
    ...
    unmoldList.add(new TechUnmoldRecipeWrapper(input, new ItemStack(Blocks.GLASS_PANE)));
    registry.addIngredientInfo(input, VanillaTypes.ITEM, "jei.information.tfctech.fill_mold");

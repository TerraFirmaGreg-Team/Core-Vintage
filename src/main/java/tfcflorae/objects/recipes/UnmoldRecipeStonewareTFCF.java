...
public class UnmoldRecipeStonewareTFCF extends IForgeRegistryEntry.Impl<IRecipe> implements IRecipe {
    ...
    private UnmoldRecipeStonewareTFCF(@Nullable ResourceLocation group, NonNullList<Ingredient> input, @NotNull Metal.ItemType type, float chance) {
        this.group = group;
        this.input = input;
        this.type = type;
        this.chance = chance;
    ...
    public boolean matches(@NotNull InventoryCrafting inv, @NotNull World world) {
        ...
        for (int slot = 0; slot < inv.getSizeInventory(); slot++) {
            ...
            if (!stack.isEmpty()) {
                if (stack.getItem() instanceof ItemStonewareMold) {
                    ItemStonewareMold moldItem = ((ItemStonewareMold) stack.getItem());
    ...
    public ItemStack getCraftingResult(InventoryCrafting inv) {
        ...
        for (int slot = 0; slot < inv.getSizeInventory(); slot++) {
            ...
            if (!stack.isEmpty()) {
                if (stack.getItem() instanceof ItemStonewareMold) {
                    ItemStonewareMold tmp = ((ItemStonewareMold) stack.getItem());
    ...
    public static class Factory implements IRecipeFactory {
        ...
        public IRecipe parse(final JsonContext context, final JsonObject json) {
            ...
            return new UnmoldRecipeStonewareTFCF(group.isEmpty() ? new ResourceLocation(result) : new ResourceLocation(group), ingredients, type,
                    chance);
        }
    }
}

...
public final class CalendarTFC implements INBTSerializable<NBTTagCompound> {
    ...
    public void setTimeFromCalendarTime(long calendarTimeToSetTo) {
        ...
        ModuleCoreOld.PACKET_SERVICE.sendToAll(new SCPacketCalendarUpdate(this));
    }
    ...
    public void onOverworldTick(World world) {
        ...
        if (deltaWorldTime > 1 || deltaWorldTime < -1) {
            ModuleCoreOld.LOGGER.debug("World time and Calendar Time are out of sync! Trying to fix...");
            ModuleCoreOld.LOGGER.debug("Calendar Time = {} ({}), Player Time = {}, World Time = {}, doDaylightCycle = {}, ArePlayersLoggedOn = {}", calendarTime, CALENDAR_TIME.getWorldTime(), playerTime, world.getWorldTime() % ICalendar.TICKS_IN_DAY, doDaylightCycle, arePlayersLoggedOn);
            ...
            if (arePlayersLoggedOn != checkArePlayersLoggedOn) {
                ...
                ModuleCoreOld.LOGGER.debug("Setting ArePlayersLoggedOn = {}", checkArePlayersLoggedOn);
            ...
            if (deltaWorldTime < 0) {
                ...
                ModuleCoreOld.LOGGER.debug("Calendar is ahead by {} ticks, jumping world time to catch up", -deltaWorldTime);
            } else {
                // World time is ahead, so jump calendar
                calendarTime += deltaWorldTime;
                ModuleCoreOld.LOGGER.debug("Calendar is behind by {} ticks, jumping calendar time to catch up", deltaWorldTime);
            }
            ModuleCoreOld.PACKET_SERVICE.sendToAll(new SCPacketCalendarUpdate(this));
        }
    }
    ...
    public void setPlayersLoggedOn(boolean arePlayersLoggedOn) {
        ...
        if (arePlayersLoggedOn) {
            ...
            ModuleCoreOld.LOGGER.info("Reverted doDaylightCycle to {} as players are logged in.", doDaylightCycle);
        } else {
            rules.setOrCreateGameRule("doDaylightCycle", Boolean.toString(false));
            ModuleCoreOld.LOGGER.info("Forced doDaylightCycle to false as no players are logged in. Will revert to {} as soon as a player logs in.", doDaylightCycle);
        }

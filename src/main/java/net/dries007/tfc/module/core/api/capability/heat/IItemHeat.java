...
/**
 * Рекомендуется наследоваться от {@link ItemHeatHandler}, а не реализовывать это напрямую.
...
public interface IItemHeat extends INBTSerializable<NBTTagCompound> {
    /**
     * Получить текущую температуру. Должен вызывать {@link CapabilityItemHeat#adjustTemp(float, float, long)} внутри себя.
     *
     ...
     * При переопределении этого метода для отображения дополнительной информации, используйте IItemHeat.super.addHeatInfo()
     *

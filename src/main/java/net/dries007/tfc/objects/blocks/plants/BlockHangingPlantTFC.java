...
public class BlockHangingPlantTFC extends BlockCreepingPlantTFC implements IGrowable {
    ...
    public boolean canGrow(World worldIn, BlockPos pos, IBlockState state, boolean isClient) {
        ...
        return i < this.plant.getMaxHeight() && worldIn.isAirBlock(pos.down()) && (!material.isSolid() || material == Material.LEAVES) && this.canBlockStay(worldIn, pos.down(), state);
    }
    ...
    public void grow(World worldIn, Random rand, BlockPos pos, IBlockState state) {
        ...
        IBlockState iblockstate = state.withProperty(AGE, 0).withProperty(this.growthStageProperty, this.plant.getStageForMonth()).withProperty(BOTTOM, false);
    ...
    public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
        ...
        if (worldIn.getBlockState(pos.down()).getBlock() == this && (Boolean) actualState.getValue(UP) && !(Boolean) actualState.getValue(DOWN) && !(Boolean) actualState.getValue(NORTH) && !(Boolean) actualState.getValue(SOUTH) && !(Boolean) actualState.getValue(EAST) && !(Boolean) actualState.getValue(WEST)) {
            actualState = actualState.withProperty(NORTH, true).withProperty(SOUTH, true).withProperty(EAST, true).withProperty(WEST, true);
        }

        if (worldIn.getBlockState(pos.up()).getBlock() == this && !(Boolean) actualState.getValue(UP) && !(Boolean) actualState.getValue(NORTH) && !(Boolean) actualState.getValue(SOUTH) && !(Boolean) actualState.getValue(EAST) && !(Boolean) actualState.getValue(WEST)) {
    ...
    protected boolean canPlantConnectTo(IBlockAccess world, BlockPos pos, EnumFacing facing) {
        return !super.canPlantConnectTo(world, pos, facing) && world.getBlockState(pos.up()).getBlock() == this && facing != EnumFacing.DOWN && facing != EnumFacing.UP ? this.canPlantConnectTo(world, pos.up(), facing) : super.canPlantConnectTo(world, pos, facing);
    }
            ...
            if (this.plant.isValidGrowthTemp(ClimateTFC.getActualTemp(worldIn, pos)) && this.plant.isValidSunlight(Math.subtractExact(worldIn.getLightFor(EnumSkyBlock.SKY, pos), worldIn.getSkylightSubtracted()))) {
                ...
                if (rand.nextDouble() < this.getGrowthRate(worldIn, pos) && ForgeHooks.onCropsGrowPre(worldIn, pos.down(), state, true)) {
                    if (j == 3) {
                        ...
                        worldIn.setBlockState(pos, state.withProperty(AGE, j + 1).withProperty(BOTTOM, this.getIsBottom(worldIn, pos)));
                    }
    ...
    private boolean canGrowDiagonally(World worldIn, BlockPos pos, IBlockState state) {
        ...
        if (!(Boolean) state.getValue(BOTTOM)) {
            ...
            for (int var7 = 0; var7 < var6; ++var7) {
                ...
                if (worldIn.isAirBlock(sidePos) && worldIn.isAirBlock(sidePos.down()) && (!sideMaterial.isSolid() || sideMaterial == Material.LEAVES) && this.canBlockStay(worldIn, sidePos.down(), state)) {
                    flag = true;
                }
            }
        }
    ...
    private boolean canGrowHorizontally(World worldIn, BlockPos pos, IBlockState state) {
        ...
        for (int var7 = 0; var7 < var6; ++var7) {
            ...
            Material sideMaterial = sideState.getMaterial();
            if (worldIn.isAirBlock(sidePos) && (!sideMaterial.isSolid() || sideMaterial == Material.LEAVES) && this.canBlockStay(worldIn, sidePos, state)) {
                flag = true;
            }
        }

...
public class ProviderQuernHorse implements IProbeInfoProvider {
  ...
  public void addProbeInfo(ProbeMode mode, IProbeInfo info, EntityPlayer player, World world, IBlockState state, IProbeHitData hitData) {
    ...
    if (block instanceof BlockQuernHorse) {
      ...
      double total = tile.getTotalItemMillTime();
      double current = tile.getCurrentItemMillTime();
      ...
      info.progress(tile.getCurrentItemMillTime() / tile.getTotalItemMillTime() * 100L, 100L,
                    new ProgressStyle().prefix(Localization.TOP.GRINDSTONE_PROGRESS.translate() + " ").suffix("%"));

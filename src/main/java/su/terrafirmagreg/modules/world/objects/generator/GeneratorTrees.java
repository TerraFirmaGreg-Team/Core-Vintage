...
import su.terrafirmagreg.modules.world.objects.generator.structures.StructureGeneratorCorals;
...
public class GeneratorTrees implements IWorldGenerator {
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
        ...
        if (!(biome instanceof BaseBiome)) return;
        ...
        if ((biome == BiomesWorld.ROLLING_HILLS || biome == BiomesWorld.HIGH_PLAINS) && (avgTemperature >= 1 + gauss)) {
            generateBush(random, chunkX, chunkZ, world, chunkData, 0.0f, 0.3f, 70f + gauss, 230f + gauss, 4 + random.nextInt(9), trees);
        }

        // Mid-dense foliage chaparral/shrubland forests in temperate regions
        // Similarly to steppes across Eurasian regions
        if ((biome == BiomesWorld.ROLLING_HILLS || biome == BiomesWorld.FIELDS || biome == BiomesWorld.FLATLANDS || biome == BiomesWorld.PLAINS ||
                biome == BiomesWorld.HIGH_PLAINS) && (avgTemperature <= 10 + gauss)) {
            generateBush(random, chunkX, chunkZ, world, chunkData, 0.0f, 0.3f, 150f + gauss, 380f + gauss, 1 + random.nextInt(7), trees);
        }

        // More foliage bushes to woodlands
        if (!(biome == BiomesWorld.OCEAN || biome == BiomesWorld.DEEP_OCEAN)) {
            generateBush(random, chunkX, chunkZ, world, chunkData, 0.3f, 1f, 150f + gauss, 500f - gauss, 1 + random.nextInt(5), trees);
        }

        // Jungle Foliage
        if (!(biome == BiomesWorld.OCEAN || biome == BiomesWorld.DEEP_OCEAN) && (avgTemperature >= 10 + gauss)) {
            generateBush(random, chunkX, chunkZ, world, chunkData, 0.3f, 1f, 150f + gauss, 500f - gauss, 5 + random.nextInt(10), trees);
        }
        ...
        for (int i = random.nextInt(Math.round(1 / diversity)); i < (1 + density) * treesPerChunk1; i++) {
        ...
        for (int i = random.nextInt(Math.round(1 / diversity)); i < (1 + density) * treesPerChunk2; i++) {
            final int x = (chunkX << 4) + random.nextInt(16) + 8;
            final int z = (chunkZ << 4) + random.nextInt(16) + 8;
            BlockPos blockPos = world.getTopSolidOrLiquidBlock(new BlockPos(x, 0, z));
            IBlockState down = world.getBlockState(blockPos.down());

...
/**
 * Powderkeg is an inventory that preserves the contents when sealed It can be picked up and keeps it's inventory Sealed state is stored in a block state
 * property, and cached in the TE (for gui purposes)
 */
@ParametersAreNonnullByDefault
public class BlockPowderKeg extends Block implements ICapabilitySize, ILightableBlock {
  ...
  public static void togglePowderKegSeal(World world, BlockPos pos) {
    TilePowderKeg tile = Helpers.getTE(world, pos, TilePowderKeg.class);
    if (tile != null) {
  ...
  public void trigger(World worldIn, BlockPos pos, IBlockState state, @Nullable EntityLivingBase igniter) {
    if (!worldIn.isRemote) {
      TilePowderKeg te = Helpers.getTE(worldIn, pos, TilePowderKeg.class);
  ...
  public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random rng) {
    ...
    TilePowderKeg te = Helpers.getTE(world, pos, TilePowderKeg.class);
    if (te != null) {
      int fuse = te.getFuse();
      if (rng.nextInt(6) == 0 && fuse > 20) {
        world.playSound(
          pos.getX() + 0.5F,
          pos.getY() + 0.5F, pos.getZ() + 0.5F, SoundEvents.ENTITY_TNT_PRIMED, SoundCategory.BLOCKS, 1.0F, rng.nextFloat() * 1.3F + 0.3F / fuse, false);
      }
  ...
  public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
    TilePowderKeg tile = Helpers.getTE(worldIn, pos, TilePowderKeg.class);

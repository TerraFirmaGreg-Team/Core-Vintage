...
import tfcflorae.compat.tfcelementia.recipes.UnmoldRecipeEarthenware;

import static net.minecraftforge.fluids.capability.CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY;

@ParametersAreNonnullByDefault
public class ItemEarthenwareMoldTFCE extends ItemPottery {
    private static final EnumMap<ItemMetalTFCE.ItemType, ItemEarthenwareMoldTFCE> MAP = new EnumMap<>(ItemMetalTFCE.ItemType.class);
    private final ItemMetalTFCE.ItemType type;
    protected int fluidCapacity = 100;
    public ItemEarthenwareMoldTFCE(ItemMetalTFCE.ItemType type) {
        this.type = type;
    ...
    public ItemEarthenwareMoldTFCE(ItemMetalTFCE.ItemType type, int fluidCapacity) {
        this.type = type;
    ...
    public static ItemEarthenwareMoldTFCE get(ItemMetalTFCE.ItemType category) {
        ...
        if (!world.isRemote) {
            ...
            if (player.isSneaking()) {
                ...
                for (IRecipe recipe : ForgeRegistries.RECIPES.getValuesCollection()) {
                    if (recipe instanceof UnmoldRecipeEarthenware && recipe.matches(craftMatrix, world)) {
                        ItemStack result = recipe.getCraftingResult(craftMatrix);
                        if (!result.isEmpty()) {
                            ItemStack moldResult = ((UnmoldRecipeEarthenware) recipe).getMoldResult(stack);
    ...
    public ICapabilityProvider initCapabilities(ItemStack stack, @Nullable NBTTagCompound nbt) {
        return new FilledMoldCapability(nbt, fluidCapacity);
    }
    ...
    private class FilledMoldCapability extends ItemHeatHandler implements ICapabilityProvider, IMoldHandler {
        ...
        FilledMoldCapability(@Nullable NBTTagCompound nbt, int capasity) {

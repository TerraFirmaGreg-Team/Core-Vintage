...
public class TreeGenKapok implements ITreeGenerator {
  ...
  public void generateTree(TemplateManager manager, World world, BlockPos pos, Tree tree, Random rand, boolean isWorldGen) {
    ...
    for (int i = 0; i < branches; i++) {
      ...
      x1 = rand.nextInt(3);
      z1 = rand.nextInt(3);
      if (rand.nextBoolean()) {x1 = -x1 - 1;}
      if (rand.nextBoolean()) {x1 = -z1 - 1;}
      if (x1 == 0 || x1 == -1) {x1 = x1 * 3 + 1;}
      if (z1 == 0 || z1 == -1) {z1 = z1 * 3 + 1;}
    ...
    for (int i = -1; i < height; i++) {placeTrunk(world, pos.add(0, i, 0));}
  ...
  public boolean canGenerateTree(World world, BlockPos pos, Tree treeType) {
    for (BlockPos p1 : trunkPos) {
      if (BlocksTFC.isSoil(world.getBlockState(pos.add(p1)))) {continue;}
      if (world.getBlockState(pos.add(p1)).getMaterial().isReplaceable()) {
        if (BlocksTFC.isSoil(world.getBlockState(pos.add(p1).down(1)))) {continue;}
        if (BlocksTFC.isSoil(world.getBlockState(pos.add(p1).down(2))) && world.getBlockState(pos.add(p1.down(1))).getMaterial().isReplaceable()) {continue;}
      }
      return false;
    }
  ...
  private void placeTrunk(World world, BlockPos pos) {
    for (BlockPos p1 : trunkPos) {checkAndPlace(world, pos.add(p1), false);}

...
public class Utils {
  ...
  public static void errorMessage(String message, boolean showDirectly) {
    if (FMLCommonHandler.instance().getSide().isClient()) {
      if (FMLClientHandler.instance().getClientPlayerEntity() != null && showDirectly) {
        FMLClientHandler.instance().getClientPlayerEntity()
          .sendMessage(new TextComponentString(
            TextFormatting.RED + message).setStyle(new Style().setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_FILE,
      ...
      } else {HPRecipes.ERRORS.add(message);}
    }
  ...
  public static void sendSavedErrors() {
    if (FMLCommonHandler.instance().getSide().isClient() && FMLClientHandler.instance().getClientPlayerEntity() != null && HPRecipes.ERRORS.size() > 0) {
      ...
      HPRecipes.ERRORS.forEach(s -> FMLClientHandler.instance().getClientPlayerEntity()
        .sendMessage(new TextComponentString(TextFormatting.RED + s).setStyle(new Style().setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_FILE,
            Loader.instance().getConfigDir() + "/horsepower.cfg"))
  ...
  public static Object parseItemStack(String item, boolean acceptOre, boolean acceptAmount) throws Exception {
    ...
    if (data.length == 2) {item = item.substring(0, item.indexOf("$"));}
    ...
    if (item.startsWith("ore:")) {
      if (!acceptOre) {throw new InvalidParameterException();}
      if (amount > 1) {
      ...
      } else {return OreDictionary.getOres(item.substring(4));}
      ...
      if (nbt != null) {compound.setTag("tag", nbt);}

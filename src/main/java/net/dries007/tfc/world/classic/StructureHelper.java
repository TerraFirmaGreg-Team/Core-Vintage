  ...
  public static void addStructureToWorld(World worldIn, BlockPos pos, Template template, PlacementSettings placementIn) {
    ...
    for (Template.BlockInfo template$blockinfo : template.blocks) {
      ...
      if (template$blockinfo1 != null) {
        ...
        if ((!placementIn.getIgnoreStructureBlock() || block1 != Blocks.STRUCTURE_BLOCK) && (structureboundingbox == null
                                                                                             || structureboundingbox.isVecInside(blockpos))) {
          ...
          if (stateToReplace.getMaterial().isReplaceable() || stateToReplace.getBlock() instanceof BlockLeavesTFC

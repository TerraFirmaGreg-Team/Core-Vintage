...
/**
 * Generic recipe for items that heat up and transform Used by pit kilns, fire pit, and charcoal forge for item transformations Note! This recipe only
 * supports inputs of stack size = 1, as they get "transformed" independent of their size. As of currently, all inventories that use this recipe also
 * have a stack size limit of 1, which is as intended
 */
...
public abstract class HeatRecipe extends IForgeRegistryEntry.Impl<HeatRecipe> implements IJEISimpleRecipe {
    ...
    /**
     * Use this to check if the recipe matches the input. Since querying the recipe is somewhat intensive (i.e. not a do every tick thing), cache the
     * recipe and only re-check on input change Check if the recipe is hot enough to complete with {@link HeatRecipe#isValidTemperature(float)}
     *
    ...
    /**
     * Gets the output item. This output will be placed in the same slot if possible (charcoal forge), or an output slot if not (fire pit) If EMPTY is
     * returned, then this recipe produces no special output
     *

...
public abstract class TileEntityHPHorseBase extends TileEntityHPBase implements ITickable {
  ...
  public boolean hasWorker() {
    if (worker != null && !worker.isDead && !worker.getLeashed() && worker.getDistanceSq(pos) < 45) {
      ...
      if (worker != null) {
        worker = null;
        nbtWorker = null;
        if (!getWorld().isRemote) {InventoryHelper.spawnItemStack(world, pos.getX(), pos.getY() + 1, pos.getZ(), new ItemStack(Items.LEAD));}
      }
      hasWorker = false;
      return false;
    }
  ...
  public void update() {
    ...
    if (validationTimer <= 0) {
      valid = validateArea();
      if (valid) {validationTimer = 220;} else {validationTimer = 60;}
    }
    boolean flag = false;

    if (!hasWorker()) {locateHorseTimer--;}
    ...
    if (!world.isRemote && valid) {
      ...
      if (hasWorker()) {
        if (running) {
          ...
          if (searchAreas[target] == null) {searchAreas[target] = new AxisAlignedBB(x - 0.5D, y - 0.5D, z - 0.5D, x + 0.5D, y + 0.5D, z + 0.5D);}

...
public class CTBlastFurnace {
	...
	public static void addRecipe(String outputMetal, String inputMetal, crafttweaker.api.item.IIngredient additive) {
		Metal result = TFCRegistries.METALS.getValuesCollection()
		                                   .stream()
		                                   .filter(x -> x.getRegistryName().getPath().equalsIgnoreCase(outputMetal))
		                                   .findFirst()
		                                   .orElse(null);
		if (result == null) {
			throw new IllegalArgumentException("Output metal specified not found!");
		}
		Metal input = TFCRegistries.METALS.getValuesCollection()
		                                  .stream()
		                                  .filter(x -> x.getRegistryName().getPath().equalsIgnoreCase(inputMetal))
		                                  .findFirst()
		                                  .orElse(null);
		if (input == null) {
		...
		CraftTweakerAPI.apply(new IAction() {
			...
			public String describe() {
				...
				return "Adding blast furnace recipe for " + input.getRegistryName()
				                                                 .getPath() + " -> " + result.getRegistryName()
				                                                                             .getPath();
			}
		});
	}

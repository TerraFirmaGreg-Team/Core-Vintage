...
public class BlockSoilGrass extends BlockGrass implements ISoilBlock, IProviderBlockColor, IGrass {
        ...
        if (worldIn.getLightFromNeighbors(pos.up()) < 4
          ...
          if (worldIn.getLightFromNeighbors(pos.up()) >= 9) {
            for (int i = 0; i < 4; ++i) {
              ...
              if (blockpos.getY() >= 0 && blockpos.getY() < 256 && !worldIn.isBlockLoaded(
                  blockpos)) {
                return;
              }
              ...
              if (iblockstate1.getBlock() == BlocksSoil.DIRT.get(soil)
  ...
  public IBlockState getActualState(IBlockState state, IBlockAccess world, BlockPos pos) {
    ...
    return state
        .withProperty(NORTH,
            world.getBlockState(pos.offset(EnumFacing.NORTH)).getBlock() instanceof BlockSoilGrass)
        .withProperty(EAST,
            world.getBlockState(pos.offset(EnumFacing.EAST)).getBlock() instanceof BlockSoilGrass)
        .withProperty(SOUTH,
            world.getBlockState(pos.offset(EnumFacing.SOUTH)).getBlock() instanceof BlockSoilGrass)
        .withProperty(WEST,
            world.getBlockState(pos.offset(EnumFacing.WEST)).getBlock() instanceof BlockSoilGrass)
  ...
  public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random rand) {
    if (this.variant.canFall() && rand.nextInt(16) == 0 && FallingBlockManager.shouldFall(world,
        pos, pos, state, false)) {

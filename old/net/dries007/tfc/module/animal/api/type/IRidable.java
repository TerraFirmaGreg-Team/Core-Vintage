        ...
        if (animal.getAge() != Age.CHILD && animal.getFamiliarity() > 0.15f) {
            ...
            if (!world.isRemote) {
                if (animal.getAge() == Age.CHILD) {
                    ModuleCoreOld.PACKET_SERVICE.sendTo(SCPacketSimpleMessage.translateMessage(MessageCategory.ANIMAL, Tags.MOD_ID + ".tooltip.animal.product.young", animal.getAnimalName()), (EntityPlayerMP) player);
                } else {
                    ModuleCoreOld.PACKET_SERVICE.sendTo(SCPacketSimpleMessage.translateMessage(MessageCategory.ANIMAL, Tags.MOD_ID + ".tooltip.animal.product.low_familiarity", animal.getAnimalName()), (EntityPlayerMP) player);
                }
            }
            return false;
        }

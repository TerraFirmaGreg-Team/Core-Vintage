...
public class WorldGenLargeRocks implements IWorldGenerator {
    ...
    public void generate(Random rng, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
        ...
        BlockPos start = world.getTopSolidOrLiquidBlock(chunkBlockPos.add(8 + rng.nextInt(16), 0, 8 + rng.nextInt(16)))
                .add(0, -1, 0);
        ...
        if (rng.nextInt(1) == 0)
            genFromPoint(world, rng,
                    start.add((rng.nextInt(2) + 1) * (rng.nextBoolean() ? 1 : -1), y + (rng.nextInt(2) + 1) * (rng.nextBoolean() ? 1 : -1),
                            (rng.nextInt(2) + 1) * (rng.nextBoolean() ? 1 : -1)));
    }

    private void genFromPoint(World world, Random rng, BlockPos start) {
        Rock rock = ChunkData.getRockHeight(world, start);
        final int size = rng.nextInt(10) == 0 ? 4 : 3;
        for (int x = -size; x <= size; x++) {
            for (int z = -size; z <= size; z++) {
                for (int y = -2; y <= 2; y++) {
                    if (x * x + z * z + y * y > size * size) continue;
                    world.setBlockState(start.add(x, y, z), BlockRockVariant.get(rock, Rock.Type.RAW)
                            .getDefaultState());
                }
            }
        }

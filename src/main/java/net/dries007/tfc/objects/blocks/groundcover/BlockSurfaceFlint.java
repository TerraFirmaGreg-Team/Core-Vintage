...
public class BlockSurfaceFlint extends BlockBush {
  ...
  public BlockSurfaceFlint() {
    ...
    setHardness(0.1f);
    net.dries007.tfc.util.OreDictionaryHelper.register(this, "flint");
  }

  private Item getWeightedDrop(int chance, int index, int currentNumber) {
    this.index = index;
    if (chance <= currentNumber) {return drops[index];} else {return getWeightedDrop(chance, index + 1, currentNumber + this.chance[index + 1]);}
  }

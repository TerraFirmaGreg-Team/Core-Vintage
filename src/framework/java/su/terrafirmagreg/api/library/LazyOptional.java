...
import su.terrafirmagreg.api.library.function.NonNullConsumer;
import su.terrafirmagreg.api.library.function.NonNullFunction;
import su.terrafirmagreg.api.library.function.NonNullPredicate;
import su.terrafirmagreg.api.library.function.NonNullSupplier;
...
public class LazyOptional<T> {
  ...
  /**
   * Construct a new {@link LazyOptional} that wraps the given {@link NonNullSupplier}.
   *
   * @param instanceSupplier The {@link NonNullSupplier} to wrap. Cannot return null, but can be null itself. If null, this method returns {@link #empty()}.
   */
  ...
  /**
   * This method hides an unchecked cast to the inferred type. Only use this if you are sure the type should match. For capabilities, generally  should be used.
   *
  ...
  private @Nullable T getValue() {
    ...
    if (resolved == null) {
      synchronized (lock) {
        ...
        if (resolved == null) {
          T temp = supplier.get();
          if (temp == null) {LOGGER.catching(Level.WARN, new NullPointerException("Supplier should not return null value"));}
   ...
   *
   * @return {@code true} if this {@link LazyOptional} is non-empty, i.e. holds a non-null supplier
   */
  ...
  /**
   * If non-empty, invoke the specified {@link NonNullConsumer} with the object, otherwise do nothing.
   *
  ...
  public void ifPresent(NonNullConsumer<? super T> consumer) {
    ...
    T val = getValue();
    if (isValid && val != null) {consumer.accept(val);}
  }

  /**
   * If a this {@link LazyOptional} is non-empty, return a new {@link LazyOptional} encapsulating the mapping function. Otherwise, returns {@link #empty()}.
   * <p>
   ...
   * @return A {@link LazyOptional} describing the result of applying a mapping function to the value of this {@link LazyOptional}, if a value is present, otherwise an empty {@link LazyOptional}
   * @throws NullPointerException if {@code mapper} is null.
   * @apiNote This method supports post-processing on optional values, without the need to explicitly check for a return status.
   * @apiNote The returned value does not receive invalidation messages from the original {@link LazyOptional}. If you need the invalidation, you will need to manage them yourself.
   */
   ...
   *
   * @param mapper A mapping function to apply to the mod object, if present
   * @return An {@link Optional} describing the result of applying a mapping function to the value of this {@link Optional}, if a value is present, otherwise an empty {@link Optional}
   * @throws NullPointerException if {@code mapper} is null.
   * @apiNote This method explicitly resolves the value of the {@link LazyOptional}. For a non-resolving mapper that will lazily run the mapping, use {@link #lazyMap(NonNullFunction)}.
   */
  ...
  /**
   * Resolve the contained supplier if non-empty, and filter it by the given {@link NonNullPredicate}, returning empty if false.
   * <p>
   * <em>It is important to note that this method is <strong>not</strong> lazy, as
   * it must resolve the value of the supplier to validate it with the predicate.</em>
   *
   * @param predicate A {@link NonNullPredicate} to apply to the result of the contained supplier, if non-empty
   * @return An {@link Optional} containing the result of the contained supplier, if and only if the passed {@link NonNullPredicate} returns true, otherwise an empty {@link Optional}
   * @throws NullPointerException If {@code predicate} is null and this {@link Optional} is non-empty
   */
  ...
  /**
   * Resolve the contained supplier if non-empty and return the result, otherwise return {@code other}.
   *
  ...
  /**
   * Resolve the contained supplier if non-empty and return the result, otherwise return the result of {@code other}.
   *
   * @param other A {@link NonNullSupplier} whose result is returned if this {@link LazyOptional} is empty
   * @return The result of the supplier, if non-empty, otherwise the result of {@code other.get()}
   * @throws NullPointerException If {@code other} is null and this {@link LazyOptional} is non-empty
   */
  ...
  /**
   * Resolve the contained supplier if non-empty and return the result, otherwise throw the exception created by the provided {@link NonNullSupplier}.
   *
   * @param <X>               Type of the exception to be thrown
   * @param exceptionSupplier The {@link NonNullSupplier} which will return the exception to be thrown
   * @return The result of the supplier
   * @throws X                    If this {@link LazyOptional} is empty
   * @throws NullPointerException If {@code exceptionSupplier} is null and this {@link LazyOptional} is empty
  ...
  /**
   * Invalidate this {@link LazyOptional}, making it unavailable for further use, and notifying any {@link #addListener(NonNullConsumer) listeners} that this has become invalid and they should update.
   * <p>
   ...
   * <p>
   * Also should be called for all when a TE is invalidated (for example, when the TE is removed or unloaded), or a world/chunk unloads, or a entity dies, etc... This allows modders to keep a cache of capability objects instead of

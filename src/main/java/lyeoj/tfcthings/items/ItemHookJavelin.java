  ...
  private void dismountHookedPlayer(EntityPlayer playerIn) {
    ...
    if (enumfacing1 != null) {
      ...
      AxisAlignedBB axisalignedbb = new AxisAlignedBB(
        d5 - d7 / 2.0D, playerIn.getEntityBoundingBox().minY,
        d6 - d8 / 2.0D, d5 + d7 / 2.0D, Math.floor(playerIn.getEntityBoundingBox().minY) + (double) playerIn.height, d6 + d8 / 2.0D);
      ...
      for (int[] aint : aint1) {
        ...
        if (!playerIn.world.collidesWithAnyBlock(axisalignedbb1)) {
          ...
          if (playerIn.world.getBlockState(blockpos).isSideSolid(playerIn.world, blockpos, EnumFacing.UP)
              || playerIn.world.getBlockState(blockpos).getMaterial() == Material.WATER) {
            d1 = d11;

...
public class BlockPlantTallWater extends BlockPlantWater implements IGrowable, ITallPlant {
    ...
    public BlockPlantTallWater(PlantEnumVariant plantBlockVariant, PlantType plant) {
    ...
    public void grow(World worldIn, Random rand, BlockPos pos, IBlockState state) {
        ...
        IBlockState iblockstate = state.withProperty(BlockPlant.AGE, 0).withProperty(growthStageProperty, plant.getStageForMonth()).withProperty(PART, getPlantPart(worldIn, pos));
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        ...
        if (plant.isValidGrowthTemp(ClimateTFC.getActualTemp(worldIn, pos)) && plant.isValidSunlight(Math.subtractExact(worldIn.getLightFor(EnumSkyBlock.SKY, pos), worldIn.getSkylightSubtracted()))) {
            int j = state.getValue(BlockPlant.AGE);
            ...
            if (rand.nextDouble() < getGrowthRate(worldIn, pos) && net.minecraftforge.common.ForgeHooks.onCropsGrowPre(worldIn, pos.up(), state, true)) {
                if (j == 3 && canGrow(worldIn, pos, state, worldIn.isRemote)) {
                    ...
                    worldIn.setBlockState(pos, state.withProperty(BlockPlant.AGE, j + 1).withProperty(PART, getPlantPart(worldIn, pos)));
                }
    ...
    public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
        return getTallBoundingBax(state.getValue(BlockPlant.AGE), state, source, pos);
    }

    @Override
    @Nonnull
    protected BlockStateContainer createPlantBlockState() {
        return new BlockStateContainer(this, BlockPlant.AGE, growthStageProperty, PART, BlockPlant.DAYPERIOD);
    }

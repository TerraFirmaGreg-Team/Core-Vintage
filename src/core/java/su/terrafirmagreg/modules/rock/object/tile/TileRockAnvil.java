...
import su.terrafirmagreg.modules.rock.object.container.ContainerRockAnvil;
import su.terrafirmagreg.modules.rock.object.gui.GuiRockAnvil;
import su.terrafirmagreg.modules.rock.object.inventory.InventoryRockAnvil;
import su.terrafirmagreg.modules.rock.object.recipe.anvil.AnvilRecipeManager;
import su.terrafirmagreg.modules.rock.object.recipe.anvil.IAnvilRecipe;
...
public class TileRockAnvil extends BaseTileInventory
  implements IProviderContainer<ContainerRockAnvil, GuiRockAnvil> {
  ...
  public TileRockAnvil() {
    super(InventoryRockAnvil::new, 4);
  ...
  public void readFromNBT(NBTTagCompound nbt) {
    ...
    if (StringUtils.isNullOrEmpty(recipeName)) {
      this.recipe = null;
    } else {
      this.recipe = AnvilRecipeManager.findMatchingRecipe(new ResourceLocation(recipeName));
    }
  ...
  public boolean setRecipe(@Nullable IAnvilRecipe recipe) {
    ...
    if (cap != null && recipe != null) {
      ...
      cap.setRecipe((AnvilRecipe) recipe);
      this.recipe = recipe;
  ...
  public boolean isItemValid(int slot, ItemStack stack) {
    return switch (slot) {
      case SLOT_INPUT_1, SLOT_INPUT_2 -> stack.hasCapability(CapabilityForgeable.CAPABILITY, null);
   ...
   */
  public void addStep(EntityPlayer player, @Nullable ForgeStep step) {
    ...
    if (cap != null) {
      ...
      if (recipe != null) {
        ...
        int workMin = workingTarget - ConfigRock.BLOCK.ANVIL.acceptableAnvilRange;
        int workMax = workingTarget + ConfigRock.BLOCK.ANVIL.acceptableAnvilRange;
  ...
  public boolean attemptWelding(EntityPlayer player) {
    ...
    if (recipe != null) {
      ...
      if (fluxStack.isEmpty()) {
        // No flux
        SCPacketSimple.translateMessage(SCPacketSimple.MessageCategory.ANVIL, ModUtils.localize("tooltip", "metal.anvil_no_flux")).sendTo(player);
        return false;
      }
      ...
      if (cap1 == null || cap2 == null || !cap1.isWeldable() || !cap2.isWeldable()) {
        if (cap1 instanceof ICapabilityHeat && cap2 instanceof ICapabilityHeat) {
          SCPacketSimple.translateMessage(SCPacketSimple.MessageCategory.ANVIL, ModUtils.localize("tooltip", "metal.anvil_too_cold")).sendTo(player);
        } else {
          SCPacketSimple.translateMessage(SCPacketSimple.MessageCategory.ANVIL, ModUtils.localize("tooltip", "metal.anvil_not_weldable")).sendTo(player);
        }
        return false;
      }
    ...
    SCPacketSimple.translateMessage(SCPacketSimple.MessageCategory.ANVIL, ModUtils.localize("tooltip", "anvil_not_weldable")).sendTo(player);
    return false;
  }

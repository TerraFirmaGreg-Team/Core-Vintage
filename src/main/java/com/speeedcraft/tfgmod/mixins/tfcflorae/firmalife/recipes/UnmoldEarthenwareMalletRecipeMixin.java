...
import com.speeedcraft.tfgmod.mixins.tfcflorae.firmalife.IUnmoldEarthenwareMalletRecipeMixin;
...
public abstract class UnmoldEarthenwareMalletRecipeMixin extends IForgeRegistryEntry.Impl<IRecipe> implements IRecipe {
	...
	private UnmoldEarthenwareMalletRecipeMixin(@Nullable ResourceLocation group, NonNullList<Ingredient> input, @Nonnull String type, float chance) {
		this.group = group;
		this.input = input;
		this.type = type;
		this.chance = chance;
		...
		if (moldStack != null) {
			...
			if (moldCap instanceof IMoldHandler moldHandler) {
				if (!moldHandler.isMolten() && moldHandler.getAmount() == 144) {
					return ((IUnmoldEarthenwareMalletRecipeMixin) this).invokeGetOutputItem(moldHandler);
				}
			}
		}
		return ItemStack.EMPTY;

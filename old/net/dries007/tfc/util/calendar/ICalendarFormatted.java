...
public interface ICalendarFormatted extends ICalendar {
    ...
    static long getTotalMonths(long time, long daysInMonth) {
        return time / (daysInMonth * ICalendar.TICKS_IN_DAY);
    }

    static long getTotalYears(long time, long daysInMonth) {
        return 1000 + (time / (12 * daysInMonth * ICalendar.TICKS_IN_DAY));
    }

    /* Fraction Calculation Methods */

    static int getMinuteOfHour(long time) {
        return (int) ((time % ICalendar.TICKS_IN_HOUR) / ICalendar.TICKS_IN_MINUTE);
    }

    static int getHourOfDay(long time) {
        return (int) ((time / ICalendar.TICKS_IN_HOUR) % ICalendar.HOURS_IN_DAY);
    }

    static int getDayOfMonth(long time, long daysInMonth) {
        return 1 + (int) ((time / ICalendar.TICKS_IN_DAY) % daysInMonth);
    }
    ...
    static Month getMonthOfYear(long time, long daysInMonth) {
        return Month.valueOf((int) ((time / (ICalendar.TICKS_IN_DAY * daysInMonth)) % 12));
    }
    ...
    default long getTotalMonths() {
        return getTicks() / (getDaysInMonth() * ICalendar.TICKS_IN_DAY);
    }

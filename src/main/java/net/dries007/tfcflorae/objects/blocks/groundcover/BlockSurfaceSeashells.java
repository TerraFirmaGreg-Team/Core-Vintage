...
public class BlockSurfaceSeashells extends BlockBush {
  ...
  private Item getWeightedDrop(int chance, int index, int currentNumber) {
    ...
    if (chance <= currentNumber) {
      Item[] drops = {ItemsTFCF.CLAM, ItemsTFCF.LIVE_CLAM, ItemsTFCF.SCALLOP, ItemsTFCF.LIVE_SCALLOP, ItemsTFCF.LIVE_STARFISH, ItemsTFCF.CONCH, ItemsTFCF.PEARL,
                      ItemsTFCF.BLACK_PEARL};
      return drops[index];
    } else {return getWeightedDrop(chance, index + 1, currentNumber + this.chance[index + 1]);}
  }
  ...
  public boolean canBlockStay(World worldIn, BlockPos pos, IBlockState state) {
    ...
    if (state.getBlock() == this) {
      return (BlocksTFC.isGround(soil) || BlocksTFCF.isGround(soil) || worldIn.getBlockState(pos.down()).isFullBlock()) && !(BlocksTFC.isSaltWater(soil)
                                                                                                                             || BlocksTFC.isFreshWater(soil));
    }

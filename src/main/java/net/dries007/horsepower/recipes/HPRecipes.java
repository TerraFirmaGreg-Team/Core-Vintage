...
public class HPRecipes {
  ...
  private static final HPRecipes INSTANCE = new HPRecipes();
  ...
  public void addGrindstoneRecipe(GrindstoneRecipe recipe, boolean hand) {
    ...
    if (hand && Configs.recipes.useSeperateGrindstoneRecipes) {addHandGrindstoneRecipe(recipe);} else {addGrindstoneRecipe(recipe);}
  }
  ...
  public void addChoppingRecipe(ChoppingBlockRecipe recipe, boolean hand) {
    ...
    if (hand && Configs.recipes.useSeperateChoppingRecipes) {addManualChoppingRecipe(recipe);} else {addChoppingRecipe(recipe);}
  }
  ...
  public void removeGrindstoneRecipe(ItemStack input, boolean hand) {
    if (hand && Configs.recipes.useSeperateGrindstoneRecipes) {handGrindstoneRecipes.remove(new ComparableItemStack(input));} else {
  ...
  public void removeChoppingRecipe(ItemStack input, boolean hand) {
    if (hand && Configs.recipes.useSeperateChoppingRecipes) {manualChoppingBlockRecipes.remove(new ComparableItemStack(input));} else {
  ...
  private <T extends HPRecipeBase> List<T> createRecipes(Class<T> clazz, List<String> data, boolean requireTime) {
    ...
    for (String aData : data) {
      ...
      for (String item : comp) {
        if (item.contains(":")) {
          ...
          try {
            ...
            Utils.errorMessage(
              "Parse error with " + clazz.getSimpleName().replaceAll("Recipe", "") + " recipe item '" + item + "' from config" + (stacks.size() > 0 ?
                                                                                                                                  " with item" + stacks.get(0)
                                                                                                                                                    : "")
              + ", index: " + index, false);
            break;
          }
          if ((stack instanceof ItemStack && !((ItemStack) stack).isEmpty()) || (!(stack instanceof ItemStack) && stack != null)) {stacks.add(stack);}
        } else if (stacks.size() == 2) {
          try {
            time = Integer.parseInt(item);
          } catch (NumberFormatException e) {
            Utils.errorMessage(
              "Parse error with " + clazz.getSimpleName().replaceAll("Recipe", "") + " recipe time '" + item + "' from config for input " + stacks.get(0)
              + " and output " + stacks.get(1) + ", index: " + index, false);
            time = -1;
          }
          ...
          try {
            ...
            Utils.errorMessage(
              "Parse error with " + clazz.getSimpleName().replaceAll("Recipe", "") + " recipe secondary chance '" + secondaryChance + "' from config for input "
      ...
      if (!flag) {
        Utils.errorMessage(
          "Couldn't load " + clazz.getSimpleName().replaceAll("Recipe", "") + " recipe (" + Joiner.on("-").join(comp) + "), index: " + index, false);
      }
      index++;
    }
    return recipes;

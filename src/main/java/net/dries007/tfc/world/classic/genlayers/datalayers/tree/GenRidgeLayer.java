...
public abstract class GenRidgeLayer extends GenLayerTFC {
    private static boolean shouldDraw;

    public GenRidgeLayer(long par1) {
        super(par1);
    }
    ...
    public static GenLayerTFC initialize(long par0, DataLayer[] regions) {
        ...
        for (int zoomLevel = 0; zoomLevel < 4; ++zoomLevel) {
            layer = new GenLayerZoomTFC((long) (1000 + zoomLevel), (GenLayerTFC) layer);
            drawImage(1024, (GenLayerTFC) layer, "Ridge " + (7 + zoomLevel));
        }

        GenLayerSmoothTFC smoothedLayer = new GenLayerSmoothTFC(1000L, (GenLayer) layer);
        GenLayerVoronoiZoomTFC voronoiLayer = new GenLayerVoronoiZoomTFC(10L, smoothedLayer);
        shouldDraw = true;
        drawImage(1024, (GenLayerTFC) layer, "Ridge Final");
        shouldDraw = false;

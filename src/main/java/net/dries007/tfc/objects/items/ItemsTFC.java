...
//    register(r, "metal/iron_bowl_mount", new ItemMetal(Metal.WROUGHT_IRON, Metal.ItemType.BOWL_MOUNT), CT_METAL);
    ...
    for (Powder powder : Powder.values()) {simpleItems.add(register(r, "powder/" + powder.name().toLowerCase(), new ItemPowder(powder), CT_MISC));}
    ...
    for (Food food : new Food[]{Food.BARLEY_BREAD_SANDWICH, Food.CORNBREAD_SANDWICH, Food.OAT_BREAD_SANDWICH, Food.RICE_BREAD_SANDWICH, Food.RYE_BREAD_SANDWICH,
                                Food.WHEAT_BREAD_SANDWICH}) {
  ...
  public static void init() {
    for (Metal metal : TFCRegistries.METALS.getValuesCollection()) {
      if (metal.getToolMetal() != null) {metal.getToolMetal().setRepairItem(new ItemStack(ItemMetal.get(metal, Metal.ItemType.SCRAP)));}
    }
  }

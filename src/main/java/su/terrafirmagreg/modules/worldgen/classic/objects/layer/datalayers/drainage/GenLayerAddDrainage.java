package su.terrafirmagreg.modules.worldgen.classic.objects.layer.datalayers.drainage;
...
public class GenLayerAddDrainage extends GenLayerBase {
    ...
    for (int var11 = 0; var11 < zSize; ++var11) {
      for (int var12 = 0; var12 < xSize; ++var12) {
        ...
        if (id0 > thisID || id1 > thisID || id2 > thisID || id3 > thisID) {
          ...
          if (id0 < GenLayerDrainage.MAX && this.nextInt(count++) == 0) {
            outID = id0 + 1;
          }

          if (id1 < GenLayerDrainage.MAX && this.nextInt(count++) == 0) {
            outID = id1 + 1;
          }

          if (id2 < GenLayerDrainage.MAX && this.nextInt(count++) == 0) {
            outID = id2 + 1;
          }

          if (id3 < GenLayerDrainage.MAX && this.nextInt(count++) == 0) {
            outID = id3 + 1;
          }

          if (this.nextInt(3) == 0 && outID <= GenLayerDrainage.MAX) {
          ...
          if (id0 > GenLayerDrainage.MIN && this.nextInt(count++) == 0) {
            outID = id0 - 1;
          }

          if (id1 > GenLayerDrainage.MIN && this.nextInt(count++) == 0) {
            outID = id1 - 1;
          }

          if (id2 > GenLayerDrainage.MIN && this.nextInt(count++) == 0) {
            outID = id2 - 1;
          }

          if (id3 > GenLayerDrainage.MIN && this.nextInt(count++) == 0) {
            outID = id3 - 1;
          }

          if (this.nextInt(3) == 0 && outID >= GenLayerDrainage.MIN) {
            outCache[var12 + var11 * xSize] = outID;
          } else {
            outCache[var12 + var11 * xSize] = thisID;
          }

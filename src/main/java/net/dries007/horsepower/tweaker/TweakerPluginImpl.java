...
import net.dries007.horsepower.tweaker.recipes.ChoppingRecipeTweaker;
import net.dries007.horsepower.tweaker.recipes.GrindstoneRecipeTweaker;
import net.dries007.horsepower.tweaker.recipes.PressRecipeTweaker;
...
public class TweakerPluginImpl implements ITweakerPlugin {
  ...
  public static ShapedChoppingRecipe convert(CTShapedChoppingRecipe recipe, ResourceLocation name) {
    ...
    return new ShapedChoppingRecipe(name, Lists.newArrayList(getItemStacks(recipe.getOre().getItems())), (ItemStack) recipe.getOutput()
      .getInternal(), rarguments.toArray());
  }
  ...
  public static ShapelessChoppingRecipe convert(CTShapelessChoppingRecipe recipe, ResourceLocation name) {
    ...
    return new ShapelessChoppingRecipe(name, Lists.newArrayList(getItemStacks(recipe.getOre().getItems())), (ItemStack) recipe.getOutput()
      .getInternal(), items);
  }

  @Override
  public void applyTweaker() {
    for (IAction action : actions) {action.apply();}
  }

...
public class BlockHangingCreepingPlantTFCF extends BlockCreepingPlantTFCF implements IGrowable {
	...
	public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
		...
		if (worldIn.getBlockState(pos.down())
		           .getBlock() == this && actualState.getValue(UP) && !actualState.getValue(DOWN) && !actualState.getValue(NORTH) && !actualState.getValue(SOUTH) && !actualState.getValue(EAST) && !actualState.getValue(WEST)) {
		...
		if (worldIn.getBlockState(pos.up())
		           .getBlock() == this && !actualState.getValue(UP) && !actualState.getValue(NORTH) && !actualState.getValue(SOUTH) && !actualState.getValue(EAST) && !actualState.getValue(WEST)) {
	...
	public boolean canBlockStay(World worldIn, BlockPos pos, IBlockState state) {
		for (EnumFacing face : EnumFacing.values()) {
			...
			if (material == Material.LEAVES || material == Material.GROUND || material == Material.ROCK || material == Material.WOOD || BlocksTFC.isGround(blockState) || worldIn.getBlockState(pos.up())
			                                                                                                                                                                     .getBlock() == this) {
				return plant.isValidTemp(ClimateTFC.getActualTemp(worldIn, pos)) && plant.isValidRain(ChunkDataTFC.getRainfall(worldIn, pos));
			}
		}
	...
	protected boolean canPlantConnectTo(IBlockAccess world, BlockPos pos, EnumFacing facing) {
		if (!super.canPlantConnectTo(world, pos, facing) && world.getBlockState(pos.up())
		                                                         .getBlock() == this && facing != EnumFacing.DOWN && facing != EnumFacing.UP) {

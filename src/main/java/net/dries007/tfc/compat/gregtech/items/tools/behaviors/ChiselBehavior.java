...
import net.dries007.tfc.module.core.common.objects.container.ContainerEmpty;
...
public class ChiselBehavior implements IToolBehavior {
    ...
    public static boolean hasHammerForChisel(EntityPlayer player) {
        ...
        if (player.inventory.offHandInventory.get(0).getItem() == ToolItems.HARD_HAMMER.get())
            return true;

        // config alters whether toolbar counts as a hammer slot or not.
        if (!ConfigTFC.Devices.CHISEL.requireHammerInOffHand) {
            for (int i = 0; i < 9; i++) {
                if (player.inventory.offHandInventory.get(i).getItem() == ToolItems.HARD_HAMMER.get()) {
                    return true;
                }
            }
        }

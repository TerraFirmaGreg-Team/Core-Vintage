...
public interface ITreeGenerator {
  ...
  /**
   * Called to generate a type. Each Tree must have one of these. Used for world gen and sapling growth
   *
   ...
   * @param type    The type type to spawn
   * @param rand    A random to use in generation
   */
  void generateTree(TemplateManager manager, World world, BlockPos pos, WoodType type, Random rand, boolean isWorldGen);
   ...
   * @param type  The tree type (for checking if the tree can generate)
   * @return true if the tree can generate.
   */
  default boolean canGenerateTree(World world, BlockPos pos, WoodType type) {
    // Check if ground is flat enough
    final int radius = type.getMaxGrowthRadius();
    ...
    final int height = type.getMaxHeight();

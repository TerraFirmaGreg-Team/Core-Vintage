...
import static net.dries007.tfc.module.rock.api.types.variant.block.RockBlockVariants.SPELEOTHEM;
...
public class WorldGenSpeleothem implements IWorldGenerator {
    ...
    private void placeSpeleothem(Random random, World world, BlockPos pos, Block block, boolean up) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (block instanceof IRockBlock rockTypeBlock) {
                ...
                IBlockState targetBlock = StorageRock.getRockBlock(RockBlockVariants.SPELEOTHEM, rockTypeBlock.getType()).getDefaultState().withProperty(BlockRockSpeleothem.SIZE, sizeType);
    ...
    private Block getSpeleothemType(IBlockState state) {
        ...
        for (var rock : RockType.getRockTypes()) {
            if (StorageRock.getRockBlock(RockBlockVariants.RAW, rock) == block) {
                return StorageRock.getRockBlock(RockBlockVariants.RAW, rock);
            }
        }

...
public class ItemProspectorsHammer extends ItemTFC implements ICapabilityMetal, ItemOreDict, TFCThingsConfigurableItem {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        ...
        if (raytraceresult == null) {
            ...
            if (!worldIn.isRemote) {
                ...
                switch (messageType) {
                    case 0:
                        playerIn.sendStatusMessage(new TextComponentTranslation("tfcthings.tooltip.prohammer_na", new Object[0]),
                                ConfigTFC.Client.TOOLTIP.propickOutputToActionBar);
                        break;
                    case 1:
                        playerIn.sendStatusMessage(new TextComponentTranslation("tfcthings.tooltip.prohammer_safe", new Object[0]),
                                ConfigTFC.Client.TOOLTIP.propickOutputToActionBar);
                        break;
                    case 2:
                        playerIn.sendStatusMessage(new TextComponentTranslation("tfcthings.tooltip.prohammer_unsafe", new Object[0]),
                                ConfigTFC.Client.TOOLTIP.propickOutputToActionBar);
                        break;
                    case 3:
                        playerIn.sendStatusMessage(new TextComponentTranslation("tfcthings.tooltip.prohammer_na_fall", new Object[0]),
                                ConfigTFC.Client.TOOLTIP.propickOutputToActionBar);
                        break;
                    case 4:
                        playerIn.sendStatusMessage(new TextComponentTranslation("tfcthings.tooltip.prohammer_safe_fall", new Object[0]),
                                ConfigTFC.Client.TOOLTIP.propickOutputToActionBar);
                        break;
                    case 5:
                        playerIn.sendStatusMessage(new TextComponentTranslation("tfcthings.tooltip.prohammer_unsafe_fall", new Object[0]),
                                ConfigTFC.Client.TOOLTIP.propickOutputToActionBar);
                        break;
                }
                float skillModifier = SmithingSkill.getSkillBonus(itemstack, SmithingSkill.Type.TOOLS) / 2.0F;
    ...
    private void checkRockLayers(EntityPlayer playerIn, World worldIn, BlockPos pos, ProspectingSkill skill) {
        ...
        if (rocks.isEmpty()) {
            playerIn.sendStatusMessage(new TextComponentTranslation("tfcthings.tooltip.prohammer_no_rocks", new Object[0]),
                    ConfigTFC.Client.TOOLTIP.propickOutputToActionBar);
        } else if (rocks.size() == 1) {
            playerIn.sendStatusMessage(
                    new TextComponentTranslation("tfcthings.tooltip.prohammer_1_rock_found", new Object[] { rocks.get(0).toString() }),
                    ConfigTFC.Client.TOOLTIP.propickOutputToActionBar);

        } else if (rocks.size() == 2) {
            playerIn.sendStatusMessage(new TextComponentTranslation("tfcthings.tooltip.prohammer_2_rocks_found",
                    new Object[] { rocks.get(0).toString(), rocks.get(1).toString() }), ConfigTFC.Client.TOOLTIP.propickOutputToActionBar);
        } else if (rocks.size() >= 3) {
            playerIn.sendStatusMessage(new TextComponentTranslation("tfcthings.tooltip.prohammer_3_rocks_found",
                            new Object[] { rocks.get(0).toString(), rocks.get(1).toString(), rocks.get(2).toString() }),
                    ConfigTFC.Client.TOOLTIP.propickOutputToActionBar);
        }
    }

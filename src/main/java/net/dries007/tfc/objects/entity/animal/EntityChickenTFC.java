...
public class EntityChickenTFC extends EntityAnimalTFC implements ILivestock {
  ...
  public List<ItemStack> getProducts() {
    ...
    if (this.isFertilized()) {
      ICapabilityEgg cap = egg.getCapability(CapabilityEgg.CAPABILITY, null);
      if (cap != null) {
  ...
  public TextComponentTranslation getTooltip() {
    if (this.getGender() == Gender.MALE) {
      return new TextComponentTranslation(TFC + ".tooltip.animal.product.male_egg");
    } else if (this.getAge() == Age.OLD) {
      return new TextComponentTranslation(TFC + ".tooltip.animal.product.old", getAnimalName());
    } else if (this.getAge() == Age.CHILD) {
      return new TextComponentTranslation(TFC + ".tooltip.animal.product.young", getAnimalName());
    } else if (getFamiliarity() <= 0.15f) {
      return new TextComponentTranslation(TFC + ".tooltip.animal.product.low_familiarity", getAnimalName());
    } else if (!hasEggs()) {
      return new TextComponentTranslation(TFC + ".tooltip.animal.product.no_egg", getAnimalName());
    }
    return null;
  }

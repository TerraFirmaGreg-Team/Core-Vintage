    ...
    for (int yy = 0; yy < h; ++yy) {
      for (int xx = 0; xx < w; ++xx) {
        ...
        if (us == oceanID && (dl != oceanID || dr != oceanID || ul != oceanID || ur != oceanID)) {
          ...
          if (dl != oceanID && this.nextInt(countNonOcean++) == 0) {lastNonOcean = dl;}

          if (dr != oceanID && this.nextInt(countNonOcean++) == 0) {lastNonOcean = dr;}

          if (ul != oceanID && this.nextInt(countNonOcean++) == 0) {lastNonOcean = ul;}

          if (ur != oceanID && this.nextInt(countNonOcean/*++*/) == 0) {lastNonOcean = ur;}

          if (this.nextInt(3) == 0) {out[xx + yy * w] = lastNonOcean;} else {out[xx + yy * w] = oceanID;}
        } else if (us != oceanID && (dl == oceanID || dr == oceanID || ul == oceanID || ur == oceanID)) {
          if (this.nextInt(5) == 0) {out[xx + yy * w] = oceanID;} else {out[xx + yy * w] = us;}
        } else {
          out[xx + yy * w] = us;
        }
      }

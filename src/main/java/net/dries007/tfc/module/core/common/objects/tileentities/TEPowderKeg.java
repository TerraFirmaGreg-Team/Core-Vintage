...
import net.dries007.tfc.module.core.common.objects.blocks.BlockPowderKeg;
...
public class TEPowderKeg extends TETickableInventory implements IItemHandlerSidedCallback {
    ...
    public void onLoad() {
        if (!world.isRemote) {
            sealed = world.getBlockState(pos).getValue(BlockPowderKeg.SEALED);
        }
    }

    @Override
    public boolean canInsert(int slot, ItemStack stack, EnumFacing side) {
        return !world.getBlockState(pos).getValue(BlockPowderKeg.SEALED) && isItemValid(slot, stack);
    }
    ...
    public void onBreakBlock(World world, BlockPos pos, IBlockState state) {
        if (!state.getValue(BlockPowderKeg.SEALED)) {

...
public class CapabilityFood {
  ...
  /**
   * Most TFC foods have decay modifiers in the range [1, 4] (high = faster decay) That puts decay times at 25% - 100% of this value So meat / fruit will decay in ~5 days, grains take ~20 days Other modifiers are applied on top of that
   */
  ...
  /**
   * Helper method to handle applying a trait to a food item. Do NOT just directly apply the trait, as that can lead to strange interactions with decay dates / creation dates This calculates a creation date that interpolates between no
   * preservation (if the food is rotten), to full preservation (if the food is new)
   */
  ...
  /**
   * This is used to update a stack from an old stack, in the case where a food is created from another Any method that creates derivative food should call this, as it avoids extending the decay of the item If called with non food items,
   * nothing happens
   *
  ...
  /**
   * Call this from any function that is meant to create a new item stack. In MOST cases, you should use {@link CapabilityFood#updateFoodFromPrevious(ItemStack, ItemStack)}, as the decay should transfer from input -> output This is only for
   * where there is no input. (i.e. on a direct {@code stack.copy()} from non-food inputs
   *
  ...
  /**
   * This is a nice way of checking if two stacks are stackable, ignoring the creation date: copy both stacks, give them the same creation date, then check compatibility This will also not stack stacks which have different traits, which is
   * intended
   *
  ...
  public static long getRoundedCreationDate() {
    return (Calendar.PLAYER_TIME.getTotalHours() / ConfigFood.FEATURE.DECAY.stackTime) * ICalendar.TICKS_IN_HOUR * ConfigFood.FEATURE.DECAY.stackTime;
  }
   ...
   * <p>
   * To apply preservation p at time T: want remaining decay fraction to be invariant under preservation Let Ri = (T - Ci) / (Ei - Ci) = (T - Ci) / d, Rf = (T - Cf) / (d * p) Then if Ri = Rf => d * p * (T - Ci) = d * (T - Cf) => Cf = (1 -
   * p) * T + p * Ci (affine combination)
   * <p>
   * In order to show that E > T is invariant under preservation: (i.e. see TerraFirmaCraft#352) Let T, Ci, Ei, d, p > 0 such that Ei > T (1.), and Ei = Ci + d Cf = (1 - p) * T + p * Ci => Ef = Cf + p * d = (1 - p) * T + p * Ci + p * d = (1
   * - p) * T + p * (Ci + d) via 1. > (1 - p) * T + p * T = T QED
   *

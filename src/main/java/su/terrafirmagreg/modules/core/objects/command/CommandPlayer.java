...
public class CommandPlayer extends BaseCommand {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        if (sender.getCommandSenderEntity() instanceof EntityPlayer player) {
            if (args.length < 1) {
                throw new WrongUsageException(ModUtils.localize("command", "player.usage_expected_first_argument"));
            }
            ExecuteType executeType = ExecuteType.parse(args[0]);
            if (args.length < 2) {
                throw new WrongUsageException(ModUtils.localize("command", "player.usage_expected_second_argument"), args[0]);
            }
            switch (args[1]) {
                case "nutrition":
    ...
    private void executeNutrient(ICommandSender sender, EntityPlayer player, ExecuteType executeType) throws CommandException {
        ...
        if (executeType == ExecuteType.SET || executeType == ExecuteType.ADD) {
            throw new WrongUsageException(ModUtils.localize("command", "player.usage_cant_set_add_nutrients"));
        } else if (executeType == ExecuteType.RESET) {
            nutritionStats.reset();
            sender.sendMessage(new TextComponentTranslation(ModUtils.localize("command", "player.reset_nutrients")));
        } else {
            sender.sendMessage(new TextComponentTranslation(ModUtils.localize("command", "player.get_nutrients"), String.format("%.2f", nutritionStats.getAverageNutrition())));
            for (Nutrient nutrient : Nutrient.values()) {
                sender.sendMessage(new TextComponentTranslation(ModUtils.localize("command", "player.get_nutrients_nutrient"),
            ...
            if (lastRecord != null) {
                ...
                sender.sendMessage(new TextComponentTranslation(ModUtils.localize("command", "player.get_nutrients_last_eaten"),
                        lastRecord.getHunger(),
                        String.format("%.2f", lastRecord.getSaturation()),
                        String.format("%.2f", lastRecord.getDecayModifier())
                ));
                for (Nutrient nutrient : Nutrient.values()) {
                    sender.sendMessage(new TextComponentTranslation(ModUtils.localize("command", "player.get_nutrients_last_eaten_nutrient"),
                            new TextComponentTranslation(Helpers.getEnumName(nutrient)),
    ...
    private void executeSkill(ICommandSender sender, EntityPlayer player, String[] args, ExecuteType executeType) throws CommandException {
        if (args.length < 3) {
            throw new WrongUsageException(ModUtils.localize("command", "player.usage_expected_id"), args[0] + " " + args[1]);
        }
        SkillType<Skill> inputSkill = SkillType.get(args[2].toLowerCase(), Skill.class);
        if (inputSkill == null) {
            throw new WrongUsageException(ModUtils.localize("command", "player.usage_unknown_skill"), args[2]);
        }
        Skill skill = CapabilityPlayer.getSkill(player, inputSkill);
        if (skill != null) {
            if (executeType == ExecuteType.GET) {
                sender.sendMessage(new TextComponentTranslation(ModUtils.localize("command", "player.get_skill"), inputSkill.getName(), skill.getTotalLevel(),
                        new TextComponentTranslation(Helpers.getEnumName(skill.getTier())), skill.getLevel()));
            } else if (executeType == ExecuteType.RESET) {
                skill.setTotalLevel(0);
                sender.sendMessage(new TextComponentTranslation(ModUtils.localize("command", "player.set_skill"), inputSkill.getName(), 0));
            } else {
                if (args.length < 4) {
                    throw new WrongUsageException(ModUtils.localize("command", "player.usage_expected_value"), args[0] + " " + args[1] + " " + args[2]);
                }
                double level = parseDouble(args[3], 0, 4);

                if (executeType == ExecuteType.ADD) {
                    skill.addTotalLevel(level / 4.0D);
                    sender.sendMessage(new TextComponentTranslation(ModUtils.localize("command", "player.add_skill"), level, inputSkill.getName()));
                } else {
                    skill.setTotalLevel(level / 4.0D);
                    sender.sendMessage(new TextComponentTranslation(ModUtils.localize("command", "player.set_skill"), inputSkill.getName(), level));
                }
            }
        }
    }
    ...
    private void executeFood(ICommandSender sender, EntityPlayer player, String[] args, ExecuteType executeType) throws CommandException {
        ...
        if (executeType == ExecuteType.GET) {
            ...
            sender.sendMessage(new TextComponentTranslation(ModUtils.localize("command", "player.get_food"), food));
        } else if (executeType == ExecuteType.RESET) {
            foodStats.setFoodLevel(20);
            sender.sendMessage(new TextComponentTranslation(ModUtils.localize("command", "player.set_food"), 20));
        } else {
            if (args.length < 3) {
                throw new WrongUsageException(ModUtils.localize("command", "player.usage_expected_value"), args[0] + " " + args[1]);
            }
            int value = parseInt(args[2], 0, 20);
            if (executeType == ExecuteType.SET) {
                foodStats.setFoodLevel(value);
                sender.sendMessage(new TextComponentTranslation(ModUtils.localize("command", "player.set_food"), value));
    ...
    private void executeSaturation(ICommandSender sender, EntityPlayer player, String[] args, ExecuteType executeType) throws CommandException {
        ...
        if (executeType == ExecuteType.GET) {
            ...
            sender.sendMessage(new TextComponentTranslation(ModUtils.localize("command", "player.get_saturation"), saturation));
        } else if (executeType == ExecuteType.RESET) {
            foodStats.setSaturation(0);
            sender.sendMessage(new TextComponentTranslation(ModUtils.localize("command", "player.set_saturation"), 0));
        } else {
            if (args.length < 3) {
                throw new WrongUsageException(ModUtils.localize("command", "player.usage_expected_value"), args[0] + " " + args[1]);
            }
            ...
            if (executeType == ExecuteType.SET) {
                ...
                sender.sendMessage(new TextComponentTranslation(ModUtils.localize("command", "player.set_saturation"), value));
            } else if (executeType == ExecuteType.ADD) {
    ...
    private void executeWater(ICommandSender sender, EntityPlayer player, String[] args, ExecuteType executeType) throws CommandException {
        ...
        if (executeType == ExecuteType.GET) {
            ...
            sender.sendMessage(new TextComponentTranslation(ModUtils.localize("command", "player.get_water"), thirst));
        } else if (executeType == ExecuteType.RESET) {
            foodStats.setThirst(100);
            sender.sendMessage(new TextComponentTranslation(ModUtils.localize("command", "player.set_water"), 100));
        } else {
            if (args.length < 3) {
                throw new WrongUsageException(ModUtils.localize("command", "player.usage_expected_value"), args[0] + " " + args[1]);
            }
            ...
            if (executeType == ExecuteType.SET) {
                ...
                sender.sendMessage(new TextComponentTranslation(ModUtils.localize("command", "player.set_water"), value));
            } else if (executeType == ExecuteType.ADD) {

...
public class TreeGenWillow implements ITreeGenerator {
  ...
  public void generateTree(TemplateManager manager, World world, BlockPos pos, Tree tree, Random rand, boolean isWorldGen) {
    ...
    ResourceLocation base = new ResourceLocation(TerraFirmaCraft.MODID_TFC, tree.getRegistryName().getPath() + "/base");
    ResourceLocation overlay = new ResourceLocation(TerraFirmaCraft.MODID_TFC, tree.getRegistryName().getPath() + "/overlay");
    ...
    for (int n = 0; n <= height; n++) {
      if (n > 3) {createLeafGroup(world, pos.up(n));}
  ...
  private void createBranch(World world, BlockPos pos1, int x, int y, int z, Random rand, Tree tree) {
    int x1 = x < 0 ? 1 : -1,
      z1 = z < 0 ? 1 : -1;
    do {
      if (x != 0 && rand.nextBoolean()) {x += x1;}
      if (z != 0 && rand.nextBoolean()) {z += z1;}
      tryPlaceLog(world, pos1.add(x, y, z), tree, BlockLog.EnumAxis.NONE);
      if (rand.nextBoolean()) {createLeafGroup(world, pos1.add(x, y, z));}
    }

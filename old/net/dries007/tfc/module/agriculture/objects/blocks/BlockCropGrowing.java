...
import net.dries007.tfc.module.agriculture.api.types.crop.category.CropCategories;
...
import static net.dries007.tfc.module.agriculture.api.types.crop.variant.item.CropItemVariants.SEED;
...
public class BlockCropGrowing extends BlockCrops implements IGrowingPlant, IPlantable, ICropBlock {
    ...
    @Override
    public Item getSeed() {
        return StorageAgriculture.getCropItem(SEED, type);
    }
    ...
    public void getDrops(@Nonnull NonNullList<ItemStack> drops, @Nonnull IBlockAccess world, @Nonnull BlockPos pos, IBlockState state, int fortune) {
        ...
        if (player != null) {
            ...
            if (skill != null) {
                if (isMature(state)) {
                    foodStack.setCount(1 + CropType.getSkillFoodBonus(skill, Block.RANDOM));
                    seedStack.setCount(1 + CropType.getSkillSeedBonus(skill, Block.RANDOM));
                    skill.add(0.04f);
                }
            }
        }
    ...
    public void die(World worldIn, BlockPos pos, IBlockState state) {
        ...
        if (ConfigTFC.General.FOOD.enableCropDeath) {
            ...
            worldIn.setBlockState(pos, StorageAgriculture.getCropBlock(CropBlockVariants.DEAD, type).getDefaultState()
    ...
    public boolean onBlockActivated(@Nonnull World worldIn, @Nonnull BlockPos pos, @Nonnull IBlockState state, @Nonnull EntityPlayer playerIn, @Nonnull EnumHand hand, @Nonnull EnumFacing facing, float hitX, float hitY, float hitZ) {
        if (getType().getCropCategory() == CropCategories.PICKABLE) {
            ...
            if (isMature(state)) {
                ...
                if (skill != null) {
                    foodDrop.setCount(1 + CropType.getSkillFoodBonus(skill, Block.RANDOM));
                    // omit the +1 because the plant stays alive.
                    seedDrop.setCount(CropType.getSkillSeedBonus(skill, Block.RANDOM));
                }

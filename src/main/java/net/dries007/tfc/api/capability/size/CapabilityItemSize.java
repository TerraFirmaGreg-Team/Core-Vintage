...
public final class CapabilityItemSize {
    ...
    public static final ResourceLocation KEY = new ResourceLocation(Constants.MODID_TFC, "item_size");
    ...
    public static void init() {
        ...
        CUSTOM_ITEMS.put(IIngredient.of(Items.CLAY_BALL),
                () -> ItemSizeHandler.get(Size.SMALL, Weight.VERY_LIGHT, true)); // Store anywhere stacksize = 64
        CUSTOM_ITEMS.put(IIngredient.of(Items.BED),
                () -> ItemSizeHandler.get(Size.LARGE, Weight.VERY_HEAVY, false)); // Store only in chests stacksize = 1
        CUSTOM_ITEMS.put(IIngredient.of(Items.MINECART),
                () -> ItemSizeHandler.get(Size.LARGE, Weight.VERY_HEAVY, false)); // Store only in chests stacksize = 1
        CUSTOM_ITEMS.put(IIngredient.of(Items.ARMOR_STAND),
                () -> ItemSizeHandler.get(Size.LARGE, Weight.HEAVY, true)); // Store only in chests stacksize = 4
        CUSTOM_ITEMS.put(IIngredient.of(Items.CAULDRON),
                () -> ItemSizeHandler.get(Size.LARGE, Weight.LIGHT, true)); // Store only in chests stacksize = 32

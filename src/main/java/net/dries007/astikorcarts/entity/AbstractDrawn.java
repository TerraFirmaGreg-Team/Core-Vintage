...
public abstract class AbstractDrawn extends Entity implements IEntityAdditionalSpawnData {
  ...
  protected boolean shouldRemovePulling() {
    if (this.pulling != null) {
      if (this.pulling.isRiding()) {return true;}
      if (this.collidedHorizontally) {
        RayTraceResult result = this.world.rayTraceBlocks(new Vec3d(this.posX, this.posY + this.height, this.posZ), new Vec3d(this.pulling.posX,
          this.pulling.posY + (this.height / 2.0F), this.pulling.posZ), false, true, false);
        if (result != null) {if (result.typeOfHit == RayTraceResult.Type.BLOCK) {return true;}}
      }
    }
...
//            }
      ...
      this.motionZ = moveZ;
      if (this.world.isRemote) {
        this.factor = Math.sqrt((moveX + lookX) * (moveX + lookX) + (moveZ + lookZ) * (moveZ + lookZ)) > 1 ? Math.sqrt(moveX * moveX + moveZ * moveZ)

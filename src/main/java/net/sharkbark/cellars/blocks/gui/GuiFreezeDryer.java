...
public class GuiFreezeDryer extends GuiContainerTE<TEFreezeDryer> {
	...
	public static final ResourceLocation FREEZE_DRYER_BACKGROUND = new ResourceLocation(MODID_CELLARS, "textures/gui/freeze_dryer.png");
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
		...
		if (mouseX >= guiLeft + 61 && mouseX <= guiLeft + 79 && mouseY >= guiTop + 16 && mouseY <= guiTop + 34) {
			if (!TE.getSeal()) {
				PacketHandler.packetReq.sendToServer(new FDPacket(TE.getPos().getX(), TE.getPos().getY(), TE.getPos()
				                                                                                            .getZ(), 0, true));
				//TE.seal();
			} else {
				PacketHandler.packetReq.sendToServer(new FDPacket(TE.getPos().getX(), TE.getPos().getY(), TE.getPos()
			...
			if ((TE.getSeal() && TE.getPower() > 0) || TE.getPump()) {
				if (!TE.getPump()) {
					PacketHandler.packetReq.sendToServer(new FDPacket(TE.getPos().getX(), TE.getPos()
					                                                                        .getY(), TE.getPos()
					                                                                                   .getZ(), 1, true));
					//TE.startPump();
				} else {

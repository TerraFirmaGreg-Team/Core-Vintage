...
public class BlockPlantHangingCreeping extends BlockPlantCreeping implements IGrowable {
  ...
  public BlockPlantHangingCreeping(FloraBlockVariant variant, FloraType type) {
    super(variant, type);
  }
  ...
  protected BlockStateContainer createPlantBlockState() {
    return new BlockStateContainer(this, DOWN, UP, NORTH, EAST, WEST, SOUTH, STAGE, DAYPERIOD, AGE_4, BOTTOM);
  }
  ...
  public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
    ...
    if (worldIn.getBlockState(pos.down())
               .getBlock() == this && actualState.getValue(UP) && !actualState.getValue(DOWN) && !actualState.getValue(NORTH) &&
    ...
    if (worldIn.getBlockState(pos.up())
               .getBlock() == this && !actualState.getValue(UP) && !actualState.getValue(NORTH) && !actualState.getValue(SOUTH) &&
    ...
    if (type.isValidGrowthTemp(Climate.getActualTemp(worldIn, pos)) &&
      ...
      if (rand.nextDouble() < getGrowthRate(worldIn, pos) &&
        ...
        if (j == 3) {
          ...
          } else if (canGrowHorizontally(worldIn, pos, state)) {
      ...
      if (rand.nextDouble() < getGrowthRate(worldIn, pos) && net.minecraftforge.common.ForgeHooks.onCropsGrowPre(worldIn, pos, state, true)) {
        if (j == 0) {
          if (canShrink(worldIn, pos)) {
            shrink(worldIn, pos);
          } else if (canShrinkHorizontally(worldIn, pos)) {

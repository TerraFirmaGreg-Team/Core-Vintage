...
import net.dries007.tfc.module.rock.objects.blocks.itemblock.ItemRockSlab;
...
public abstract class BlockRockSlab extends BlockSlab implements IRockBlock {
    ...
    protected static Block getFullBlockFromSlab(RockBlockVariant variant, RockType type) {
        return switch (variant.toString()) {
            case "slab/raw", "slab_double/raw" -> StorageRock.getRockBlock(RockBlockVariants.RAW, type);
            case "slab/cobble", "slab_double/cobble" -> StorageRock.getRockBlock(RockBlockVariants.COBBLE, type);
            case "slab/smooth", "slab_double/smooth" -> StorageRock.getRockBlock(RockBlockVariants.SMOOTH, type);
            case "slab/bricks", "slab_double/bricks" -> StorageRock.getRockBlock(RockBlockVariants.BRICKS, type);
            default ->
    ...
    protected static Block getDoubleSlabFromSlab(RockBlockVariant variant, RockType type) {
        return switch (variant.toString()) {
            case "slab/raw" -> StorageRock.getRockBlock(RockBlockVariants.SLAB_DOUBLE_RAW, type);
            case "slab/cobble" -> StorageRock.getRockBlock(RockBlockVariants.SLAB_DOUBLE_COBBLE, type);
            case "slab/smooth" -> StorageRock.getRockBlock(RockBlockVariants.SLAB_DOUBLE_SMOOTH, type);
            case "slab/bricks" -> StorageRock.getRockBlock(RockBlockVariants.SLAB_DOUBLE_BRICK, type);
            default ->
                    throw new RuntimeException(String.format("Double slab from slab not founded: %s, %s", variant, type));
        };
    }
    ...
    public static class Half extends BlockRockSlab {
        ...
        public void onModelRegister() {
            ModelLoader.setCustomStateMapper(this, new DefaultStateMapper() {
                ...
                protected ModelResourceLocation getModelResourceLocation(@Nonnull IBlockState state) {
                    return new ModelResourceLocation(getResourceLocation(),
                            "half=" + state.getValue(BlockSlab.HALF) + "," +
                                    "rocktype=" + getType());
                }
            });

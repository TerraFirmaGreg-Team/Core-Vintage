...
import su.terrafirmagreg.modules.device.objects.containers.ContainerLogPile;
...
public class TileLogPile extends BaseTileInventory implements ITickable, IProviderContainer<ContainerLogPile, GuiLogPile> {
    ...
    private void createCharcoal() {
        ...
        if (j == 1) {
            ...
            world.setBlockState(pos, BlocksDevice.CHARCOAL_PILE.getDefaultState().withProperty(LAYERS, charcoal));
            return;
        }
        for (int k = j - 1; k >= 0; k--) {
    ...
    @Override
    public ContainerLogPile getContainer(InventoryPlayer inventoryPlayer, World world, IBlockState state, BlockPos pos) {
        return new ContainerLogPile(inventoryPlayer, this);
    }

    @Override
    public GuiLogPile getGuiContainer(InventoryPlayer inventoryPlayer, World world, IBlockState state, BlockPos pos) {

...
public final class Calendar implements INBTSerializable<NBTTagCompound> {
  ...
  /**
   * Calendar time. Advances when player sleeps, stops when doDaylightCycle is false Synced with the daylight cycle Players can see this via the calendar GUI tab Calendar Time 0 = Midnight, January 1, 1000
   */
  ...
  static {
    ...
    for (int i = 0; i < ConfigCore.FEATURE.CALENDAR.BIRTHDAYS.dayList.length; i++) {
      addBirthday(ConfigCore.FEATURE.CALENDAR.BIRTHDAYS.dayList[i]);
    }
  }
  ...
  /**
   * This runs a sequence of code, but first will set the calendar and player time by an offset Useful if we need to run code that technically needs to happen at a different calendar time The offsets are removed once the transaction is
   * complete
   *
  ...
  /**
   * Called from {@link net.minecraftforge.fml.common.event.FMLServerStartingEvent} Initializes the calendar with the current minecraft server instance, reloading all values from world saved data
   */

...
public class BlockFluidBase extends BlockFluidClassic implements IHasModel {
    ...
    public BlockFluidBase(Fluid fluid, Material material, boolean canCreateSources) {
        this(fluid, material);
    ...
    public void updateTick(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state, @Nonnull Random rand) {
        ...
        if (!isSourceBlock(world, pos)) {
            ...
            BlockFluidBase blockType = this;

            if (newState.getBlock() == this)
                currentMeta = newState.getValue(BlockFluidBase.LEVEL);
            ...
            if (minMeta + 1 < currentMeta && blockType != this) {
                world.setBlockState(pos, blockType.getDefaultState().withProperty(BlockFluidBase.LEVEL, currentMeta), 3);
            }
        }
    }
    ...
    protected boolean[] getOptimalFlowDirections(World world, BlockPos pos) {
        for (int side = 0; side < 4; side++) {
            ...
            BlockPos pos2 = pos.offset(BlockFluidClassic.SIDES.get(side));
    ...
    protected int calculateFlowCost(World world, BlockPos pos, int recurseDepth, int side) {
        ...
        for (int adjSide = 0; adjSide < 4; adjSide++) {
            if (BlockFluidClassic.SIDES.get(adjSide) == BlockFluidClassic.SIDES.get(side).getOpposite()) {
                continue;
            }

            BlockPos pos2 = pos.offset(BlockFluidClassic.SIDES.get(adjSide));
    ...
    protected void flowIntoBlock(World world, BlockPos pos, int meta) {
        ...
        if (displaceIfPossible(world, pos)) {
            ...
            boolean replace = !(targetBlock instanceof net.minecraftforge.fluids.BlockFluidBase);

            // to make sure we only replace BlockFluidTFC blocks, and not any other modded fluid blocks.
            if (targetBlock instanceof BlockFluidBase) {
                // always replace flows if we're flowing in from above
                if (world.getBlockState(pos.down(densityDir)).getBlock() == this) {
                    replace = true;
                }

                final int blockFlowStrength = targetBlockState.getValue(BlockFluidBase.LEVEL);

                // replace flows immediately when their supporting flows are gone
                if (!replace) {
                    boolean supported = world.getBlockState(pos.down(densityDir)).getBlock() == targetBlock;

                    if (!supported) {
                        for (EnumFacing side : EnumFacing.HORIZONTALS) {
                            IBlockState neighbor = world.getBlockState(pos.offset(side));
                            if (neighbor.getBlock() == targetBlock &&
                                    (blockFlowStrength > neighbor.getValue(BlockFluidBase.LEVEL) ||
                ...
                if (!replace) {
                    ...
                    if (((BlockFluidBase) targetBlock).getDensity() > this.getDensity())
                        flowStrength += 1;
            ...
            if (replace) {
                world.setBlockState(pos, this.getDefaultState().withProperty(BlockFluidBase.LEVEL, meta), 3);
            }
        }
    }
    ...
    public float getFluidHeightForRender(IBlockAccess world, BlockPos adjPos, @Nonnull IBlockState upState) {
        ...
        if (isMergeableFluid(adjState)) {
            ...
            if (adjBlock == this || adjBlock instanceof BlockLiquid)
                ...
                return ((net.minecraftforge.fluids.BlockFluidBase) adjBlock).getFluidHeightForRender(world, adjPos, upState);
        }
    ...
    protected boolean isBlockingSourceBlock(IBlockAccess world, BlockPos pos) {
        IBlockState state = world.getBlockState(pos);
        return isMergeableFluid(state) && state.getValue(BlockFluidBase.LEVEL) == 0;
    }

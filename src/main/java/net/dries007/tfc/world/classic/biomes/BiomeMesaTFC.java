...
public class BiomeMesaTFC extends BiomeTFC {
  ...
  public BiomeMesaTFC(boolean hasBrycePillars, boolean hasForest, int debugColor, Biome.BiomeProperties properties, int lilyPadPerChunk, int waterPlantsPerChunk) {
          ...
          if (l == -1) {
            ...
            if (j1 >= i2 - 1) {
              if (this.hasForest && j1 > 86 + k * 2) {
                if (flag) {
                  chunkPrimerIn.setBlockState(l1, j1, k1, BlockRockVariant.get(ChunkDataTFC.getRockHeight(worldIn, chunkBlockPos), Rock.Type.DIRT)
                                                                          .getDefaultState());
                } else {
                  chunkPrimerIn.setBlockState(l1, j1, k1, BlockRockVariant.get(ChunkDataTFC.getRockHeight(worldIn, chunkBlockPos), Rock.Type.GRASS)
                                                                          .getDefaultState());
                }
              } else if (j1 > i2 + 3 + k) {

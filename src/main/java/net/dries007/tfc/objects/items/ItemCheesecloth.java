...
public class ItemCheesecloth extends ItemTFC {
  ...
  public boolean canStack(@Nonnull ItemStack stack) {
    ...
    if (bucketCap != null) {return bucketCap.drain(CAPACITY, false) == null;}
    return true;
  }
    ...
    if (!worldIn.isRemote && !stack.isEmpty()) {
      ...
      if (rayTrace != null && bucketCap != null && rayTrace.typeOfHit == RayTraceResult.Type.BLOCK) {
        ...
        if (bucketCap.drain(CAPACITY, false) == null) {
          ...
          if (result.isSuccess()) {
            stack.shrink(1);
            if (stack.isEmpty()) {return new ActionResult<>(EnumActionResult.SUCCESS, result.getResult());}

            if (!playerIn.isCreative()) {ItemHandlerHelper.giveItemToPlayer(playerIn, result.getResult());}

            return new ActionResult<>(EnumActionResult.SUCCESS, stack);
          }
        } else {
          IBlockState stateAt = worldIn.getBlockState(pos);
          if (!(stateAt.getBlock().isReplaceable(worldIn, pos) || !stateAt.getMaterial().isSolid())) {pos = pos.offset(rayTrace.sideHit);}

          stateAt = worldIn.getBlockState(pos);
          if (stateAt.getBlock().isReplaceable(worldIn, pos) || !stateAt.getMaterial().isSolid()) {
            if (!stateAt.getMaterial().isLiquid()) {worldIn.destroyBlock(pos, true);}
            ...
            if (fluidStack != null) {
              ...
              if (recipeExists(fluidStack)) {
                ...
                if (item != null) {Helpers.spawnItemStack(worldIn, pos, item);}
              }
            }

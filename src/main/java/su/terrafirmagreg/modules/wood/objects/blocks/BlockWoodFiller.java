...
public class BlockWoodFiller extends BlockDirectional {

  private final boolean useTileEntity;
  private final boolean providePower;

  public BlockWoodFiller(Material materialIn, String name, boolean useTileEntity,
      boolean providePower) {
    super(materialIn);
  ...
  public BlockWoodFiller setHarvestLevel1(String tool, int level) {
  ...
  @Override
  public void addCollisionBoxToList(IBlockState state, World world, BlockPos pos0,
    ...
    if (validateFilled(world, state1, pos0)) {
      state1.getBlock()
          .addCollisionBoxToList(state1, world, pos, entityBox, collidingBoxes, entityIn,
              p_185477_7_);
    }
  }
  ...
  @Override
  public final RayTraceResult collisionRayTrace(IBlockState s, World w, BlockPos p0, Vec3d start,
      Vec3d end) {
    BlockPos p = p0.offset(s.getValue(FACING));
    IBlockState state1 = w.getBlockState(p);
    if (validateFilled(w, state1, p0)) {
      RayTraceResult trace = state1.getBlock().collisionRayTrace(state1, w, p, start, end);
      return trace != null ? new RayTraceResult(trace.typeOfHit, trace.hitVec, trace.sideHit, p0)
          : trace;
    } else {
  ...
  @Override
  public boolean onBlockActivated(World w, BlockPos p0, IBlockState s, EntityPlayer e, EnumHand h,
      EnumFacing f, float x, float y, float z) {
    BlockPos p = p0.offset(s.getValue(FACING));
    IBlockState state1 = w.getBlockState(p);
    if (validateFilled(w, state1, p0)) {
  ...
  @Override
  public boolean removedByPlayer(IBlockState state, World world, BlockPos pos0, EntityPlayer player,
      boolean willHarvest) {
    BlockPos pos = pos0.offset(state.getValue(FACING));
    IBlockState state1 = world.getBlockState(pos);
    if (validateFilled(world, state1, pos0)) {
  ...
  @Override
  public boolean canConnectRedstone(IBlockState state, IBlockAccess world, BlockPos pos0,
  ...
  public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World world,
      BlockPos pos0, EntityPlayer player) {
    BlockPos pos = pos0.offset(state.getValue(FACING));
    IBlockState state1 = world.getBlockState(pos);
    if (validateFilled(world, state1, pos0)) {
  ...
  @Override
  public boolean addLandingEffects(IBlockState state, WorldServer world, BlockPos pos0,
      IBlockState iblockstate, EntityLivingBase entity,
    ...
    boolean flag = state1.getBlock()
        .addLandingEffects(state1, world, pos, iblockstate, entity, numberOfParticles);
    if (!flag) {
      world.spawnParticle(EnumParticleTypes.BLOCK_DUST, pos0.getX() + 0.5, pos0.getY() + 1,
          pos0.getZ() + 0.5, numberOfParticles, 0.0D, 0.0D,
          0.0D, 0.15000000596046448D, Block.getStateId(state1));
    }
    return true;
  }
  ...
  public boolean addHitEffects(IBlockState state, World world, RayTraceResult target,
    ...
    RayTraceResult target1 = new RayTraceResult(target.typeOfHit, target.hitVec.subtract(0, 1, 0),
        target.sideHit, pos);
  ...
  @Override
  @SideOnly(Side.CLIENT)
  public boolean addDestroyEffects(World world, BlockPos pos0, @NotNull ParticleManager manager) {
    BlockPos pos = pos0.offset(world.getBlockState(pos0).getValue(FACING));
    IBlockState state1 = world.getBlockState(pos);
    if (!validateFilled(world, state1, pos0)) {
      return true;
  ...
  @Override
  public boolean shouldCheckWeakPower(IBlockState state, IBlockAccess world, BlockPos pos0,
      EnumFacing side) {
    BlockPos pos = pos0.offset(state.getValue(FACING));
    IBlockState state1 = world.getBlockState(pos);
    if (world instanceof World && validateFilled((World) world, state1, pos0)) {
  ...
  @Override
  public SoundType getSoundType(IBlockState state, World world, BlockPos pos0,
  ...
  public boolean validateFilled(World world, IBlockState state, BlockPos pos) {

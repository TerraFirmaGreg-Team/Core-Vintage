...
import net.dries007.tfc.objects.items.ItemMetalMallet;
import net.dries007.tfc.objects.items.ItemMetalMalletHead;
...
public class ItemsFL {
  ...
  public static ItemMetalMalletHead getMetalMalletHead(Metal metal) {
    return malletHeads.get(metal);
  }
  ...
  private static <T extends Item> T register(IForgeRegistry<Item> r, String name, T item, CreativeTabs ct) {
    item.setRegistryName(MODID_FL, name);
    item.setCreativeTab(ct);
    item.setTranslationKey(MODID_FL + "." + name.replace('/', '.'));
    r.register(item);
    return item;
  }

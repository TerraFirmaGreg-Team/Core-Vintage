...
public final class ClientRegisterEvents {
  ...
  public static void registerModels(ModelRegistryEvent event) {
    ...
    // Gems
    for (ItemGem item : ItemsTFC.getAllGemItems()) {for (Gem.Grade grade : Gem.Grade.values()) {registerEnumBasedMetaItems("gem", grade, item);}}

    // Ore Items
    for (ItemOreTFC item : ItemsTFC.getAllOreItems()) {
      if (item.ore.isGraded()) {for (Ore.Grade grade : Ore.Grade.values()) {registerEnumBasedMetaItems("ore", grade, item);}} else {
    ...
    ModelBakery.registerItemVariants(ItemMold.get(Metal.ItemType.INGOT), new ModelResourceLocation(
      ItemMold.get(Metal.ItemType.INGOT).getRegistryName() + "/unknown"));
    ...
    for (Block block : BlocksTFC.getAllFluidBlocks()) {ModelLoader.setCustomStateMapper(block, new StateMap.Builder().ignore(BlockFluidBase.LEVEL).build());}
    ...
    for (Block block : BlocksTFC.getAllLeafBlocks()) {ModelLoader.setCustomStateMapper(block, new StateMap.Builder().ignore(BlockLeaves.DECAYABLE).build());}

    for (Block block : BlocksTFC.getAllOreBlocks()) {ModelLoader.setCustomStateMapper(block, new StateMap.Builder().ignore(BlockOreTFC.GRADE).build());}

    for (Block block : BlocksTFC.getAllWallBlocks()) {ModelLoader.setCustomStateMapper(block, new StateMap.Builder().ignore(BlockWall.VARIANT).build());}

    for (Block block : BlocksTFC.getAllLogBlocks()) {ModelLoader.setCustomStateMapper(block, new StateMap.Builder().ignore(BlockLogTFC.PLACED).build());}

    for (Block block : BlocksTFC.getAllSaplingBlocks()) {ModelLoader.setCustomStateMapper(block, new StateMap.Builder().ignore(BlockSaplingTFC.STAGE).build());}

    for (Block block : BlocksTFC.getAllDoorBlocks()) {ModelLoader.setCustomStateMapper(block, new StateMap.Builder().ignore(BlockDoor.POWERED).build());}

    for (Block block : BlocksTFC.getAllChestBlocks()) {ModelLoader.setCustomStateMapper(block, new StateMap.Builder().ignore(BlockChest.FACING).build());}
    ...
    for (Block block : BlocksTFC.getAllCropBlocks()) {ModelLoader.setCustomStateMapper(block, new StateMap.Builder().ignore(WILD).build());}
    ...
    // Empty Models
    final ModelResourceLocation empty = new ModelResourceLocation(TFC + ":empty");

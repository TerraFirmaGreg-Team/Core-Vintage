package net.dries007.tfcflorae.objects.blocks.blocktype;
    ...
    if (rockTFCF.canFall()) {
      ...
      switch (rockTFCF) {
        case MUD:
          ...
          if (tempRock != null) {spec.setResultingState(BlockRockVariantTFCF.get(rock, tempRock).getDefaultState());} else {
            Rock.Type tempRockTFC = RockTFCF.getNonGrassVersionTFCStatic(rockTFCF);
            if (tempRockTFC != null) {spec.setResultingState(BlockRockVariant.get(rock, tempRockTFC).getDefaultState());}
          }

...
public class PropickBehavior implements IToolBehavior {
    ...
    public EnumActionResult onItemUse(@Nonnull EntityPlayer player, World worldIn, @Nonnull BlockPos pos, @Nonnull EnumHand hand, @Nullable EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (facing != null) {
            ...
            if (!worldIn.isRemote) {
                ...
                ModuleCoreOld.PACKET_SERVICE.sendTo(packet, (EntityPlayerMP) player);
            } else {
    ...
    private void addHitBlockParticle(World world, BlockPos pos, EnumFacing side, IBlockState state) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            switch (side) {
                case EnumFacing.WEST -> xOffset = x + axisalignedbb.minX - 0.1D;
                case EnumFacing.EAST -> xOffset = x + axisalignedbb.maxX + 0.1D;
                case EnumFacing.DOWN -> yOffset = y + axisalignedbb.minY - 0.1D;
                case EnumFacing.UP -> yOffset = y + axisalignedbb.maxY + 0.1D;
                case EnumFacing.NORTH -> zOffset = z + axisalignedbb.minZ - 0.1D;
                case EnumFacing.SOUTH -> zOffset = z + axisalignedbb.maxZ + 0.1D;
            }

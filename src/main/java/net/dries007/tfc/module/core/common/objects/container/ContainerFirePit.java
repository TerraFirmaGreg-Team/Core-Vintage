...
public class ContainerFirePit extends ContainerTE<TEFirePit> {
    ...
    protected void addContainerSlots() {
        ...
        if (inventory != null) {
            ...
            if (attachment == BlockFirePit.FirePitAttachment.NONE) {
                // input slot
                addSlotToContainer(new SlotCallback(inventory, TEFirePit.SLOT_ITEM_INPUT, 80, 20, tile));
                // output slots
                addSlotToContainer(new SlotCallback(inventory, TEFirePit.SLOT_OUTPUT_1, 71, 48, tile));
                addSlotToContainer(new SlotCallback(inventory, TEFirePit.SLOT_OUTPUT_2, 89, 48, tile));
            } else {
                ...
                for (int i = TEFirePit.SLOT_EXTRA_INPUT_START; i <= TEFirePit.SLOT_EXTRA_INPUT_END; i++) {
                    addSlotToContainer(new SlotCallback(inventory, i, 62 + (i - TEFirePit.SLOT_EXTRA_INPUT_START) * 18, 20, tile));
                }
            }
        }
    }
    ...
    protected boolean transferStackIntoContainer(ItemStack stack, int containerSlots) {
        switch (attachment) {
            case NONE:
                return !mergeItemStack(stack, TEFirePit.SLOT_FUEL_INPUT, TEFirePit.SLOT_ITEM_INPUT + 1, false);
            case COOKING_POT:
            case GRILL:
                return !mergeItemStack(stack, TEFirePit.SLOT_FUEL_INPUT, TEFirePit.SLOT_FUEL_INPUT + 1, false) && !mergeItemStack(stack, TEFirePit.SLOT_FUEL_INPUT + 1, TEFirePit.SLOT_FUEL_INPUT + 6, false); // this uses index of the slots sequentially, not the slot IDs themselves
        }
        return false;
    }
}

...
public class BlockWoodFiller extends BlockDirectional {
    ...
    public BlockWoodFiller(Material materialIn, String name, boolean useTileEntity, boolean providePower) {
        super(materialIn);
    ...
    @Override
    public boolean addLandingEffects(IBlockState state, WorldServer world, BlockPos pos0, IBlockState iblockstate, EntityLivingBase entity,
        ...
        if (!flag)
            world.spawnParticle(EnumParticleTypes.BLOCK_DUST, pos0.getX() + 0.5, pos0.getY() + 1, pos0.getZ() + 0.5, numberOfParticles, 0.0D, 0.0D,
    ...
    @Override
    @SideOnly(Side.CLIENT)
    public boolean addHitEffects(IBlockState state, World world, RayTraceResult target, @NotNull ParticleManager manager) {

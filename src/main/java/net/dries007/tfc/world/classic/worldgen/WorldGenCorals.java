...
public class WorldGenCorals implements IWorldGenerator {
  ...
  private void generateCoral(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
    ...
    for (int i = 0; i < (coralsInChunk) * 5; i++) {
      ...
      if ((b == BiomesWorld.OCEAN || b == BiomesWorld.DEEP_OCEAN || b == BiomesWorld.BEACH || b == BiomesWorld.GRAVEL_BEACH) && world.provider.getDimension() == 0) {
        if (isValidPosition(world, pos) && pos.getY() < WorldTypeTFC.SEALEVEL - 1 && pos.getY() > 119 && floraDensity >= 0.4f && floraDiversity >= 0.3f
            && floraDensity <= 0.6f && floraDiversity <= 0.5f && avgTemperature >= 10f && avgTemperature <= 28f && rainfall >= 150f) {
    ...
    for (int i = 0; i < (coralsInChunk) * 100; i++) {
      ...
      if ((b == BiomesWorld.OCEAN || b == BiomesWorld.DEEP_OCEAN || b == BiomesWorld.BEACH || b == BiomesWorld.GRAVEL_BEACH) && world.provider.getDimension() == 0) {
        if (isValidPosition2(world, pos) && pos.getY() < WorldTypeTFC.SEALEVEL - 1 && pos.getY() > 119 && floraDensity >= 0.4f && floraDiversity >= 0.3f
            && avgTemperature >= 10f && rainfall >= 150f) {
          int dyeColorRandom = random.nextInt(16);
          if (dyeColorRandom == 0) {
            int coralType = random.nextInt(10);
            if (coralType == 0) {
  ...
  protected boolean isValidPosition(World world, BlockPos pos) {
    ...
    return ((BlocksTFC.isGround(down) || up.getBlock() instanceof BlockCoralBlock || down.getBlock() instanceof BlockCoralBlock
             || north.getBlock() instanceof BlockCoralBlock || south.getBlock() instanceof BlockCoralBlock || east.getBlock() instanceof BlockCoralBlock
  ...
  protected boolean isValidPosition2(World world, BlockPos pos) {
    ...
    IBlockState east = world.getBlockState(pos.east());
    IBlockState west = world.getBlockState(pos.west());
    return ((up.getBlock() instanceof BlockCoralBlock || down.getBlock() instanceof BlockCoralBlock || north.getBlock() instanceof BlockCoralBlock
             || south.getBlock() instanceof BlockCoralBlock || east.getBlock() instanceof BlockCoralBlock || west.getBlock() instanceof BlockCoralBlock)

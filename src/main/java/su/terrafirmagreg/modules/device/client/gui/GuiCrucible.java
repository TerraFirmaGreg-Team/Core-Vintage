    ...
    protected void renderHoveredToolTip(int mouseX, int mouseY) {
        if (tile.getAlloy().getAmount() > 0) {
            ...
            if (mouseX >= guiLeft + startX && mouseX < guiLeft + endX && mouseY >= guiTop + startY && mouseY < guiTop + endY) {
                ...
                tooltip.add(I18n.format(MODID_TFC + ".tooltip.crucible_units", amount, maxAmount));
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        if (alloy.getAmount() > 0) {
            ...
            for (Map.Entry<Metal, Double> entry : alloy.getMetals().entrySet()) {
                ...
                String units;
                if (entry.getValue() >= 1) {
                    units = I18n.format(MODID_TFC + ".tooltip.units", entry.getValue().intValue());
                } else {
                    units = I18n.format(MODID_TFC + ".tooltip.crucible_less_than_one");
                }
                String content = String.format("  %s(%s%2.1f%%%s)", units, TextFormatting.DARK_GREEN, 100 * entry.getValue() / alloy.getAmount(),

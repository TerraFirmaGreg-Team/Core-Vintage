...
public abstract class BlockHPBase extends Block implements IItemSize {
  ...
  public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
    ...
    if (te instanceof TileEntityHPHorseBase) {teH = (TileEntityHPHorseBase) te;}
    int x = pos.getX();
    int y = pos.getY();
    int z = pos.getZ();
    ...
    if (slot > -1) {
      ...
      if (!result.isEmpty()) {emptiedOutput(worldIn, pos);}
    }

    if (result.isEmpty()) {
      if (!stack.isEmpty()) {return false;}
      if (teH != null) {teH.setWorkerToPlayer(playerIn);}
    }

    if (!result.isEmpty()) {ItemHandlerHelper.giveItemToPlayer(playerIn, result, EntityEquipmentSlot.MAINHAND.getSlotIndex());}

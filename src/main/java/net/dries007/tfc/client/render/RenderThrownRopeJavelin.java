...
public class RenderThrownRopeJavelin extends Render<EntityThrownRopeJavelin> {
  ...
  protected void renderLeash(EntityThrownRopeJavelin javelin, double x, double y, double z, float entityYaw, float partialTicks) {
    ...
    if (javelin.getThrower() instanceof EntityPlayer entityplayer) {
      ...
      if ((this.renderManager.options == null || this.renderManager.options.thirdPersonView <= 0) && entityplayer == Minecraft.getMinecraft().player) {
        ...
        vec3d = vec3d.rotatePitch(
          -(entityplayer.prevRotationPitch + (entityplayer.rotationPitch - entityplayer.prevRotationPitch) * partialTicks) * 0.017453292F);
      ...
      for (int j = 0; j <= 24; ++j) {
        ...
        bufferbuilder.pos(
            x + d10 * (double) f3 + 0.0D, y + d11 * (double) (f3 * f3 + f3) * 0.5D + (double) ((24.0F - (float) j) / 18.0F + 0.125F), z + d12 * (double) f3)
          .color(f, f1, f2, 1.0F).endVertex();
        bufferbuilder.pos(
            x + d10 * (double) f3 + 0.025D,
            y + d11 * (double) (f3 * f3 + f3) * 0.5D + (double) ((24.0F - (float) j) / 18.0F + 0.125F) + 0.025D, z + d12 * (double) f3).color(f, f1, f2, 1.0F)
          .endVertex();
      }
      ...
      for (int l = 0; l <= 24; ++l) {
        ...
        bufferbuilder.pos(
            x + d10 * (double) f7_1 + 0.0D,
            y + d11 * (double) (f7_1 * f7_1 + f7_1) * 0.5D + (double) ((24.0F - (float) l) / 18.0F + 0.125F) + 0.025D, z + d12 * (double) f7_1)
          .color(f4, f5, f6, 1.0F).endVertex();
        bufferbuilder.pos(
            x + d10 * (double) f7_1 + 0.025D,
            y + d11 * (double) (f7_1 * f7_1 + f7_1) * 0.5D + (double) ((24.0F - (float) l) / 18.0F + 0.125F), z + d12 * (double) f7_1 + 0.025D)
          .color(f4, f5, f6, 1.0F).endVertex();
      }

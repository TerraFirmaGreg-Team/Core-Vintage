...
public class ChoppingRecipeTweaker {
  ...
  private static class AddChoppingRecipe extends BaseHPAction {
    ...
    public void apply() {
      ...
      if (items == null) {
        HorsePowerMod.logger.error("Cannot turn " + input + " into a chopping recipe");
        return;
      }
  ...
  private static class RemoveChoppingRecipe extends BaseHPAction {
    ...
    public void apply() {
      ...
      Collection<ChoppingBlockRecipe> recipeList =
        hand && Configs.recipes.useSeperateChoppingRecipes ? HPRecipes.instance().getManualChoppingRecipes() : HPRecipes.instance().getChoppingRecipes();

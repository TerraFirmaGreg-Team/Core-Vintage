...
public class TEBloomery extends TETickableInventory implements ITickable {
        ...
        if (!world.isRemote && world.getTotalWorldTime() % 20 == 0) {
            ...
            if (state.getValue(ILightableBlock.LIT)) {
                if (this.getRemainingTicks() <= 0) {
                    ...
                    state = state.withProperty(ILightableBlock.LIT, false);
            ...
            if (turnOff && state.getValue(ILightableBlock.LIT)) {
                state = state.withProperty(ILightableBlock.LIT, false);
                world.setBlockState(pos, state);
            }
            ...
            updateSlagBlock(state.getValue(ILightableBlock.LIT));
        }
    }
    ...
    protected boolean isInternalBlockComplete() {
        ...
        return inside.getBlock() == BlocksDevice.CHARCOAL_PILE && inside.getValue(BlockCharcoalPile.LAYERS) >= 8;
    }
    ...
    protected void updateSlagBlock(boolean cooking) {
        ...
        for (int i = 1; i < 4; i++) {
            if (slagLayers > 0) {
                if (slagLayers >= 4) {
                    slagLayers -= 4;
                    world.setBlockState(getInternalBlock().up(i), BlocksDevice.MOLTEN.getDefaultState().withProperty(ILightableBlock.LIT, cooking).withProperty(BlockMolten.LAYERS, 4));
                } else {
                    world.setBlockState(getInternalBlock().up(i), BlocksDevice.MOLTEN.getDefaultState().withProperty(ILightableBlock.LIT, cooking).withProperty(BlockMolten.LAYERS, slagLayers));
                    slagLayers = 0;
                }
            } else {

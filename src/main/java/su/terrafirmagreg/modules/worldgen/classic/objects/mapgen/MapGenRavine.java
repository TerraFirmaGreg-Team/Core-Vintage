package su.terrafirmagreg.modules.worldgen.classic.objects.mapgen;
...
import static su.terrafirmagreg.modules.worldgen.classic.ChunkGenClassic.AIR;
import static su.terrafirmagreg.modules.worldgen.classic.ChunkGenClassic.LAVA;
...
public class MapGenRavine extends MapGenBase {
  ...
  public MapGenRavine(int rarity, int h, int v) {
    height = h;
    variability = v;
    ravineRarity = rarity;
  }
  ...
  private void generateRavine(long seed, int chunkX, int chunkZ, ChunkPrimer primer, double xCoord,
    ...
    for (int round = 0; round < rounds; ++round) {
      ...
      if (RNG.nextInt(4) == 0) {
        continue; // <--Determines the length of the ravine // todo: make setting?
      }
      ...
      for (int x = xMin; x < xMax; ++x) {
        ...
        for (int z = zMin; z < zMax; ++z) {
          ...
          if (xNormalized * xNormalized + zNormalized * zNormalized >= 1.0D) {
            continue;
          }

        ...
        if (up.getMaterial().isLiquid() || ((neighborLight = world.getLightFromNeighbors(upPos)) < 4 && up.getLightOpacity(world, upPos) > 2)) {
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                if (currentBlock instanceof BlockPeat) {
                    ...
                    ISoilTypeBlock block = ((ISoilTypeBlock) current.getBlock());
                    world.setBlockState(target, getBlockSoilMap(block.getSoilVariant().getGrassVersion(spreader), block.getSoilType()).getDefaultState()); //TODO распространение травы сломаль
                }
            }

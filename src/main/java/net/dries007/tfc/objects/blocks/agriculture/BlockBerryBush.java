...
public class BlockBerryBush extends Block implements IGrowingPlant {
  ...
  public BlockFaceShape getBlockFaceShape(IBlockAccess worldIn, IBlockState state, BlockPos pos, EnumFacing face) {
    if (bush.getSize() == IBerryBush.Size.LARGE && face == EnumFacing.UP) {return BlockFaceShape.SOLID;} else {return BlockFaceShape.UNDEFINED;}
  }
  ...
  public void onEntityCollision(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
    if (!(entityIn instanceof EntityPlayer && ((EntityPlayer) entityIn).isCreative())) {
      ...
      if (bush.isSpiky() && entityIn instanceof EntityLivingBase) {
        entityIn.attackEntityFrom(DamageSources.BERRY_BUSH, 1.0F);
      }
    }
  }

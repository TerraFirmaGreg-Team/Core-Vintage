      ...
      if (ConfigTFCF.General.STRUCTURES.activateStructureGeneration) {
        if (!(world.getBlockState(pos).getBlock() == ChunkGenTFC.FRESH_WATER.getBlock()
          ...
          if (data.isInitialized() && data.getRainfall() >= 100f && random.nextInt(ConfigTFCF.General.STRUCTURES.spawnChanceRuins) == 0) {
            ...
            if (chance == 0) {generateStructure(StructureList.STONE_CIRCLE_RUIN, world, random, pos);} else if (chance == 1) {
          ...
          if (data.isInitialized() && data.getRainfall() >= 320f && data.getFloraDensity() >= 0.3f && data.getAverageTemp() >= 9f
              && random.nextInt(ConfigTFCF.General.STRUCTURES.spawnChanceMoai) == 0) {
            int chance = random.nextInt(6);

            if (chance == 0) {generateStructure(StructureList.MOAI_1, world, random, pos);} else if (chance == 1) {
          ...
          if (data.isInitialized() && data.getRainfall() >= 320f && data.getFloraDensity() >= 0.2f && data.getAverageTemp() >= 13f

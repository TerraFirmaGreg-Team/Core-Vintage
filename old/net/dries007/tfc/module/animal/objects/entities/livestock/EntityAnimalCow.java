...
public class EntityAnimalCow extends EntityAnimalMammal implements ILivestock {
    ...
    public void birthChildren() {
        ...
        for (int i = 0; i < numberOfChildren; i++) {
            EntityAnimalCow baby = new EntityAnimalCow(world, Gender.valueOf(Constants.RNG.nextBoolean()), (int) CalendarTFC.PLAYER_TIME.getTotalDays());
    ...
    public boolean processInteract(@Nonnull EntityPlayer player, @Nonnull EnumHand hand) {
        ...
        if (fillResult.isSuccess()) {
            if (getFamiliarity() > 0.15f && isReadyForAnimalProduct()) {
                ...
                if (tooltip != null) {
                    ModuleCoreOld.PACKET_SERVICE.sendTo(new SCPacketSimpleMessage(MessageCategory.ANIMAL, tooltip), (EntityPlayerMP) player);
                }
            }
            return true;
        } else {

...
public class PlantType implements Comparable<PlantType> {
    ...
    private PlantType(Builder builder) {
        ...
        if (name.isEmpty()) throw new RuntimeException(String.format("Plant name must contain any character: [%s]", name));

        if (!types.add(this)) throw new RuntimeException(String.format("Plant: [%s] already exists!", name));
    }

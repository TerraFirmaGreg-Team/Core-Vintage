...
public class BlockJoshuaTreeLog extends Block {
  ...
  public BlockJoshuaTreeLog(Tree wood) {
    ...
    this.setDefaultState(this.blockState.getBaseState().withProperty(NORTH, Boolean.valueOf(false)).withProperty(EAST, Boolean.valueOf(false))
      ...
      .withProperty(UP, Boolean.valueOf(false)).withProperty(DOWN, Boolean.valueOf(false)));
    this.setHarvestLevel("axe", 0);
    this.setHardness(20.0F);
    this.setResistance(5.0F);
    this.setSoundType(SoundType.WOOD);
    net.dries007.tfc.util.OreDictionaryHelper.register(this, "log", "wood");
    //noinspection ConstantConditions
    net.dries007.tfc.util.OreDictionaryHelper.register(this, "log", "wood", wood.getRegistryName().getPath());
  ...
  public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
    ...
    return state.withProperty(DOWN, Boolean.valueOf(
        block == this || block == BlockJoshuaTreeFlower.get(wood) || BlocksTFC.isSand(worldIn.getBlockState(pos.down()))
        || BlocksTFC.isSoilOrGravel(worldIn.getBlockState(pos.down())) || BlocksTFCF.isSand(worldIn.getBlockState(pos.down()))
        || BlocksTFCF.isSoilOrGravel(worldIn.getBlockState(pos.down())) || block == Blocks.HARDENED_CLAY || block == Blocks.STAINED_HARDENED_CLAY))
  ...
  public Item getItemDropped(IBlockState state, Random rand, int fortune) {
    ...
    if (BlockLogTFC.get(wood) != null) {return BlockLogTFC.get(wood).getItemDropped(state, rand, fortune);} else {
  ...
  public boolean canSurviveAt(World worldIn, BlockPos pos) {
    ...
    for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
      ...
      if (block == this) {
        ...
        if (block1 == this || BlocksTFC.isSand(worldIn.getBlockState(blockpos.down())) || BlocksTFC.isSoilOrGravel(worldIn.getBlockState(blockpos.down()))
            || BlocksTFCF.isSand(worldIn.getBlockState(blockpos.down())) || BlocksTFCF.isSoilOrGravel(worldIn.getBlockState(blockpos.down()))
            || block1 == Blocks.HARDENED_CLAY || block1 == Blocks.STAINED_HARDENED_CLAY) {
          return true;
        }
      }
    }
    ...
    return block2 == this || BlocksTFC.isSand(worldIn.getBlockState(pos.down())) || BlocksTFC.isSoilOrGravel(worldIn.getBlockState(pos.down()))
  ...
  public boolean shouldSideBeRendered(IBlockState blockState, IBlockAccess blockAccess, BlockPos pos, EnumFacing side) {
    ...
    return block != this && block != BlockJoshuaTreeFlower.get(wood) && (side != EnumFacing.DOWN
                                                                         || !BlocksTFC.isSand(blockAccess.getBlockState(pos.offset(side)))
                                                                         || !BlocksTFC.isSoilOrGravel(blockAccess.getBlockState(pos.offset(side)))
                                                                         || !BlocksTFCF.isSand(blockAccess.getBlockState(pos.offset(side)))
                                                                         || !BlocksTFCF.isSoilOrGravel(blockAccess.getBlockState(pos.offset(side)))

			...
			// Welding requires a hammer in main hand
				...
				if (!worldIn.isRemote && te.attemptWelding(playerIn)) {
					...
					for (int i = 0; i < Constants.RANDOM.nextInt(5) + 3; i++)
						TFCParticles.SPARK.sendToAllNear(worldIn, x + (Constants.RANDOM.nextFloat() - 0.5) / 7, y, z + (Constants.RANDOM.nextFloat() - 0.5) / 7, 6 * (Constants.RANDOM.nextFloat() - 0.5), 2D, 6 * (Constants.RANDOM.nextFloat() - 0.5), 22);
					return true;
				}
			}

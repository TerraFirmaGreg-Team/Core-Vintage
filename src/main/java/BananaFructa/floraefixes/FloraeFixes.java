...
public class FloraeFixes {
    ...
    public static final String modId = "floraefixes";
    ...
    public void onBlockClick(PlayerInteractEvent.RightClickBlock event) {
        ...
        if (!event.getWorld().isRemote) {
            if (b instanceof FarmlandTFCF) {
                ...
                if (ItemSeedsTFC.get(Crop.RICE) == event.getEntityPlayer().getHeldItem(EnumHand.MAIN_HAND).getItem() && event.getWorld().getBlockState(event.getPos().up()).getMaterial() != Material.WATER) {

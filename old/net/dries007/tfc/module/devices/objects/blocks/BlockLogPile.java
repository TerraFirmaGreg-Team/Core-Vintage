...
public class BlockLogPile extends BlockBase implements ILightableBlock, IHasModel {
    ...
    public BlockLogPile() {
        ...
        setDefaultState(this.getDefaultState().withProperty(AXIS, EnumFacing.Axis.X).withProperty(ILightableBlock.LIT, false));
    }
...
//    }
    ...
    public IBlockState getStateFromMeta(int meta) {
        return this.getDefaultState().withProperty(AXIS, meta == 0 ? EnumFacing.Axis.Z : EnumFacing.Axis.X).withProperty(ILightableBlock.LIT, meta >= 2);
    }

    @Override
    public int getMetaFromState(IBlockState state) {
        return (state.getValue(AXIS) == EnumFacing.Axis.Z ? 0 : 1) + (state.getValue(ILightableBlock.LIT) ? 2 : 0);
    }

    @Override
    public void randomTick(World worldIn, BlockPos pos, IBlockState state, Random random) {
        if (!worldIn.isRemote && state.getValue(ILightableBlock.LIT)) {
            for (EnumFacing side : EnumFacing.values()) {
                ...
                if (isValidCoverBlock(offsetState, worldIn, offsetPos, side.getOpposite())) {
                    if (offsetState.getBlock() instanceof BlockLogPile && !offsetState.getValue(ILightableBlock.LIT)) {
                        worldIn.setBlockState(offsetPos, offsetState.withProperty(ILightableBlock.LIT, true));
                    }
                } else {
    ...
    public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
        if (stateIn.getValue(ILightableBlock.LIT)) {
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (te != null) {
            ...
            if (!state.getValue(ILightableBlock.LIT) && side == EnumFacing.UP && world.getBlockState(pos.up()).getBlock().isReplaceable(world, pos) && ItemFireStarter.onIgnition(stack)) {
                // Light the Pile
                if (!world.isRemote) {
                    world.setBlockState(pos, state.withProperty(ILightableBlock.LIT, true));
                    te.light();
            ...
            if (!player.isSneaking() && !state.getValue(ILightableBlock.LIT)) {
                if (!world.isRemote) {
    ...
    protected BlockStateContainer createBlockState() {
        return new BlockStateContainer(this, AXIS, ILightableBlock.LIT);
    }

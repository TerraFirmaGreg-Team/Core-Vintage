      ...
      if (result.getBlockPos() != null && world.getBlockState(result.getBlockPos())
        ...
        if (getThrower() != null) {
          ...
          if (length - 1 > bridges.getCount()) {
            getThrower().sendMessage(new TextComponentTranslation("tfcthings.tooltip.rope_bridge_too_long"));
          } else if (((length - 2) / 8) < Math.abs(yDif)) {
            getThrower().sendMessage(new TextComponentTranslation("tfcthings.tooltip.rope_bridge_too_steep"));
          } else {
            if (axis) {
              if (Math.abs(xDif) < 3) {
                ...
                getThrower().sendMessage(new TextComponentTranslation("tfcthings.tooltip.rope_bridge_diagonal"));
                diagonal = true;
              }
    ...
    while (remainingPieces > 1) {
      if (startDif == endDif) {
        if (shouldReplaceBlock(start, world)) {
          ...
          getThrower().sendMessage(new TextComponentTranslation("tfcthings.tooltip.rope_bridge_interrupted"));
          return;
        }

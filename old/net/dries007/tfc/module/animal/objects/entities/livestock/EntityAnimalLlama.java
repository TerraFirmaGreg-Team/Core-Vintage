...
import net.dries007.tfc.module.animal.objects.entities.ai.EntityAnimalAIPanic;
...
public class EntityAnimalLlama extends EntityLlama implements IAnimal, ILivestock {
    ...
    protected static final DataParameter<Integer> BIRTHDAY = EntityDataManager.createKey(EntityAnimalLlama.class, DataSerializers.VARINT);
    protected static final DataParameter<Float> FAMILIARITY = EntityDataManager.createKey(EntityAnimalLlama.class, DataSerializers.FLOAT);
    //Is this female fertilized?
    private static final DataParameter<Boolean> FERTILIZED = EntityDataManager.createKey(EntityAnimalLlama.class, DataSerializers.BOOLEAN);
    // The time(in days) this entity became pregnant
    private static final DataParameter<Long> PREGNANT_TIME = EntityDataManager.createKey(EntityAnimalLlama.class, TFCEntities.getLongDataSerializer());
    ...
    public EntityAnimalLlama(World world) {
        this(world, IAnimal.Gender.valueOf(Constants.RNG.nextBoolean()), TFCEntityAnimal.getRandomGrowth(ConfigTFC.Animals.LLAMA.adulthood, ConfigTFC.Animals.LLAMA.elder));
    }
                ...
                if (this.isHungry()) {
                    ...
                    if (!this.world.isRemote) {
                        ...
                        if (this.isFertilized() && this.getType() == Type.MAMMAL) {
                            ModuleCoreOld.PACKET_SERVICE.sendTo(SCPacketSimpleMessage.translateMessage(MessageCategory.ANIMAL, Tags.MOD_ID + ".tooltip.animal.mating.pregnant", getAnimalName()), (EntityPlayerMP) player);
                        }
                    }
                }
            }
    ...
    public void onFertilized(@Nonnull IAnimal male) {
        ...
        this.geneJump = (float) ((father.getEntityAttribute(AbstractHorse.JUMP_STRENGTH).getBaseValue() + this.getEntityAttribute(AbstractHorse.JUMP_STRENGTH).getBaseValue() + this.getModifiedJumpStrength()) / 3.0D);

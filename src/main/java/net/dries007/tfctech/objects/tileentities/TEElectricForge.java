...
import net.dries007.tfctech.client.audio.IMachineSoundEffect;
import net.dries007.tfctech.objects.blocks.devices.BlockElectricForge;
import net.dries007.tfctech.objects.storage.MachineEnergyContainer;
...
import static net.dries007.tfctech.objects.blocks.devices.BlockElectricForge.LIT;
...
public class TEElectricForge extends TEInventory implements ITickable, ITileFields, IMachineSoundEffect, IEnergySink {
    ...
    for (int i = SLOT_INPUT_MIN; i <= SLOT_INPUT_MAX; i++) {
      ...
      if (cap != null) {
        ...
        if (targetTemperature > itemTemp && energyContainer.consumeEnergy(energy, false)) {
          ...
          float temp = cap.getTemperature() + heatSpeed * cap.getHeatCapacity() * (float) ConfigTFC.Devices.HEAT.globalModifier;
  ...
  public boolean hasCapability(Capability<?> capability, @Nullable EnumFacing facing) {
    if (facing == null || facing == EnumFacing.UP || facing == EnumFacing.DOWN || facing == world.getBlockState(pos).getValue(BlockElectricForge.FACING)
      .getOpposite()) {
  ...
  public boolean acceptsEnergyFrom(IEnergyEmitter iEnergyEmitter, EnumFacing facing) {
    return TechConfig.DEVICES.acceptIc2EU && (facing == EnumFacing.UP || facing == EnumFacing.DOWN || facing == world.getBlockState(pos)
      .getValue(BlockElectricForge.FACING).getOpposite());
  }

package net.dries007.tfcflorae.objects.blocks.groundcover;
...
public class BlockPebbleWater extends BlockFluidTFC implements ICapabilitySize {
  ...
  public boolean canPlaceBlockAt(World worldIn, BlockPos pos) {
    if (this.getFluid() == SALT_WATER) {
      return worldIn.getBlockState(pos).getBlock().isReplaceable(worldIn, pos) && worldIn.getBlockState(pos.down()).isFullBlock()
             && BlocksTFC.isSaltWater(worldIn.getBlockState(pos)) && BlocksTFC.isSaltWater(worldIn.getBlockState(pos.up())) && !worldIn.isAirBlock(pos.up());
    } else if (this.getFluid() == FRESH_WATER) {
      return worldIn.getBlockState(pos).getBlock().isReplaceable(worldIn, pos) && worldIn.getBlockState(pos.down()).isFullBlock()
             && BlocksTFC.isFreshWater(worldIn.getBlockState(pos)) && BlocksTFC.isFreshWater(worldIn.getBlockState(pos.up())) && !worldIn.isAirBlock(pos.up());
    } else {return false;}
  }
    ...
    for (Rock rock : TFCRegistries.ROCKS.getValuesCollection()) {
      if (!world.isRemote) {
        if (!player.getHeldItem(hand).isEmpty() && player.getHeldItem(hand).getItem() == ItemRock.get(rock)) {
          if (state.getValue(PILE_TYPE) == EnumPileType.FOUR || state.getValue(PILE_TYPE) == EnumPileType.FOUR_PLANT) {return false;}
          ...
          if (state.getValue(PILE_TYPE) == EnumPileType.FOUR) {world.setBlockState(pos, state.withProperty(PILE_TYPE, EnumPileType.THREE));}
          if (state.getValue(PILE_TYPE) == EnumPileType.THREE) {world.setBlockState(pos, state.withProperty(PILE_TYPE, EnumPileType.TWO));}
          if (state.getValue(PILE_TYPE) == EnumPileType.TWO) {world.setBlockState(pos, state.withProperty(PILE_TYPE, EnumPileType.ONE));}
          if (state.getValue(PILE_TYPE) == EnumPileType.ONE) {world.setBlockToAir(pos);}

          if (state.getValue(PILE_TYPE) == EnumPileType.FOUR_PLANT) {world.setBlockState(pos, state.withProperty(PILE_TYPE, EnumPileType.THREE_PLANT));}
          if (state.getValue(PILE_TYPE) == EnumPileType.THREE_PLANT) {world.setBlockState(pos, state.withProperty(PILE_TYPE, EnumPileType.TWO_PLANT));}
          if (state.getValue(PILE_TYPE) == EnumPileType.TWO_PLANT) {world.setBlockState(pos, state.withProperty(PILE_TYPE, EnumPileType.ONE_PLANT));}
          if (state.getValue(PILE_TYPE) == EnumPileType.ONE_PLANT) {world.setBlockToAir(pos);}
          return true;
        }
      }
    }
  ...
  public IBlockState getStateForPlacement(World world, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer, EnumHand hand) {
    if (!(world.getBlockState(pos).getBlock() instanceof BlockPebbleWater)) {
      ...
      } else {return getStateForPlacement(world, pos, facing, hitX, hitY, hitZ, meta, placer).withProperty(PILE_TYPE, EnumPileType.ONE);}
    }
  ...
  public int quantityDropped(IBlockState state, int fortune, Random random) {
    ...
    if (meta <= 3) {dropCount = meta;}
    if (meta >= 4 && meta <= 7) {dropCount = meta - 4;}
    return 1 + dropCount;
  }
  ...
  protected BlockStateContainer createBlockState() {
    return new BlockStateContainer.Builder(this).add(LEVEL).add(new IProperty[]{PILE_TYPE}).add(FLUID_RENDER_PROPS.toArray(new IUnlistedProperty<?>[0]))
      .build();
  }

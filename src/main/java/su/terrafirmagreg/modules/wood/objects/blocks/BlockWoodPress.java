...
public class BlockWoodPress extends BlockHPBase {
  ...
  public static final PropertyEnum<PressModels> PART = PropertyEnum.create("part",
      PressModels.class);

  private static final AxisAlignedBB BOUND_AABB = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D,
      1.0D + 12D / 16D, 1.0D);
  private static final AxisAlignedBB COLLISION_AABB = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D,
      1.0D + 3D / 16D, 1.0D);
  ...
  public void onBlockAdded(World worldIn, BlockPos pos, IBlockState state) {
    if (!worldIn.isRemote) {
      ...
      worldIn.setBlockState(pos,
          state.withProperty(FACING, filled).withProperty(PART, PressModels.BASE), 2);
    }
  }
  ...
  public void addInformation(ItemStack stack, @Nullable World player, List<String> tooltip,
    ...
    tooltip.add(Localization.ITEM.HORSE_PRESS.SIZE.translate(Colors.WHITE.toString(),
  ...
  public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state,
    ...
    if (!worldIn.isRemote && tileentity != null) {
      final IFluidHandler fluidHandler = tileentity.getCapability(
          CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, null);
      if (fluidHandler != null && FluidUtil.interactWithFluidHandler(playerIn, hand,
          fluidHandler)) {

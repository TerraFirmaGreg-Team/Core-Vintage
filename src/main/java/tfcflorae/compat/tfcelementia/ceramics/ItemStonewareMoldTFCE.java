...
import tfcflorae.compat.tfcelementia.recipes.UnmoldRecipeStoneware;
...
public class ItemStonewareMoldTFCE extends ItemPottery {
	private static final EnumMap<ItemMetalTFCE.ItemType, ItemStonewareMoldTFCE> MAP = new EnumMap<>(ItemMetalTFCE.ItemType.class);
	private final ItemMetalTFCE.ItemType type;
	protected int fluidCapacity = 100;

	public ItemStonewareMoldTFCE(ItemMetalTFCE.ItemType type) {
		this.type = type;
	...
	public static ItemStonewareMoldTFCE get(ItemMetalTFCE.ItemType category) {
		...
		if (!world.isRemote) {
			...
			if (player.isSneaking()) {
				...
				for (IRecipe recipe : ForgeRegistries.RECIPES.getValuesCollection()) {
					if (recipe instanceof UnmoldRecipeStoneware && recipe.matches(craftMatrix, world)) {
						ItemStack result = recipe.getCraftingResult(craftMatrix);
						if (!result.isEmpty()) {
							ItemStack moldResult = ((UnmoldRecipeStoneware) recipe).getMoldResult(stack);

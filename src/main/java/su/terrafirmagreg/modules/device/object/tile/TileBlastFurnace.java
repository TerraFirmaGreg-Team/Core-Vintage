...
public class TileBlastFurnace extends TETickableInventory implements ITickable, ITileFields {
  ...
  public void update() {
    ...
    if (!world.isRemote) {
      ...
      if (alloy.removeAlloy(1, true) > 0) {
        ...
        TileInductionCrucible teI = Helpers.getTE(world, pos.down(), TileInductionCrucible.class);
        if (teI != null && teI.addMetal(alloy.getResult(), 1) <= 0) {
          alloy.removeAlloy(1, false);
        }

        TileCrucible te = Helpers.getTE(world, pos.down(), TileCrucible.class);

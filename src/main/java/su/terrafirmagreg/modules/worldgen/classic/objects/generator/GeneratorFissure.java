...
import static su.terrafirmagreg.modules.worldgen.classic.init.BiomesWorld.BEACH;
import static su.terrafirmagreg.modules.worldgen.classic.init.BiomesWorld.DEEP_OCEAN;
import static su.terrafirmagreg.modules.worldgen.classic.init.BiomesWorld.GRAVEL_BEACH;
import static su.terrafirmagreg.modules.worldgen.classic.init.BiomesWorld.LAKE;
import static su.terrafirmagreg.modules.worldgen.classic.init.BiomesWorld.OCEAN;
import static su.terrafirmagreg.modules.worldgen.classic.init.BiomesWorld.RIVER;

/**
 * Rewrite on fissure generation logic *EXPERIMENTAL* Needs more tweaking in rock placement
 */
public class GeneratorFissure implements IWorldGenerator {

  private final IBlockState fillBlock;
  private final boolean checkStability;

  public GeneratorFissure(boolean lava) {
    fillBlock = lava ? ChunkGenClassic.LAVA : ChunkGenClassic.HOT_WATER;
    checkStability = lava;
  }

  @Override
  public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
    BlockPos start = new ChunkPos(chunkX, chunkZ).getBlock(
      random.nextInt(14) + 9, 0,
      random.nextInt(14) + 9
    );

    Biome biome = world.getBiome(start);

    if (BiomeUtils.isBiomeDesired(biome, BEACH, OCEAN, GRAVEL_BEACH, LAKE, RIVER, DEEP_OCEAN)) {
      return;
    }
  ...
  /**
   * Gives a list of block positions for a circle. Used to clear the blocks above the fissure
   *
  ...
  private void smartFill(World world, BlockPos pos, Set<BlockPos> fillBlockPos, IBlockState rock,
    ...
    for (EnumFacing facing : EnumFacing.VALUES) {
      ...
      if (world.getBlockState(pos.offset(facing)) == fillBlock) {
        continue;
      }

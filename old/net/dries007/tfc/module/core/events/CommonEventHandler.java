...
@Mod.EventBusSubscriber(modid = net.dries007.tfc.Tags.MOD_ID)
public final class CommonEventHandler {
    ...
    public static void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event) {
        if (event.player instanceof EntityPlayerMP player) {
            ...
            if (playerData != null) {
                // Sync
                ModuleCoreOld.PACKET_SERVICE.sendTo(new SCPacketPlayerDataUpdate(playerData.serializeNBT()), player);
            }
        }
    }
    ...
    public static void onPlayerRespawn(PlayerEvent.PlayerRespawnEvent event) {
        if (event.player instanceof EntityPlayerMP player) {
            ...
            IPlayerData cap = player.getCapability(CapabilityPlayerData.CAPABILITY, null);
            if (cap != null) {
                ModuleCoreOld.PACKET_SERVICE.sendTo(new SCPacketPlayerDataUpdate(cap.serializeNBT()), player);
            }
        }
    }
    ...
    public static void onChangeDimension(PlayerEvent.PlayerChangedDimensionEvent event) {
        if (event.player instanceof EntityPlayerMP player) {
            ...
            if (skills != null) {
                ModuleCoreOld.PACKET_SERVICE.sendTo(new SCPacketPlayerDataUpdate(skills.serializeNBT()), player);
            }
        }
    }
        ...
        if (event.getWorld().getWorldType() == TerraFirmaCraft.WORLD_TYPE_TFC && event.getWorld().provider.getDimensionType() == DimensionType.OVERWORLD) {
            ...
            if (ConfigTFC.General.DIFFICULTY.giveVanillaMobsEquipment) {
                ...
                if (equipment != null)
                {
                    ...
                    {
                        equipment.getEquipment(slot, Tags.RNG).ifPresent(stack -> entity.setItemStackToSlot(slot, stack));
                    }
                }*/
            }
        }
    ...
    public static void onGameRuleChange(GameRuleChangeEvent event) {
        ...
        if ("naturalRegeneration".equals(event.getRuleName()) && ConfigTFC.General.OVERRIDES.forceNoVanillaNaturalRegeneration) {
            ...
            ModuleCoreOld.LOGGER.warn("Something tried to set natural regeneration to true, reverting!");
        }
    }
    ...
    public static void onWorldLoad(WorldEvent.Load event) {
        ...
        if (world.provider.getDimension() == 0 && !world.isRemote) {
            ...
            ModuleCoreOld.PACKET_SERVICE.sendToAll(new SCPacketCalendarUpdate(CalendarTFC.INSTANCE));
        }
        ...
        if (ConfigTFC.General.OVERRIDES.forceNoVanillaNaturalRegeneration) {
            ...
            ModuleCoreOld.LOGGER.warn("Updating gamerule naturalRegeneration to false!");
        }
    }
     ...
     *
     * @param event {@link WorldEvent.CreateSpawnPosition}
     */
    @SubscribeEvent
    public static void onCreateSpawn(WorldEvent.CreateSpawnPosition event) {
        event.getSettings().bonusChestEnabled = false;
        ModuleCoreOld.LOGGER.info("Disabling bonus chest, you cheaty cheater!");
    }
        ...
        if (TerraFirmaCraft.getInstance().isModuleEnabled(ModuleRock.class)) {
            ...
            if (ConfigTFC.General.OVERRIDES.enableLavaWaterPlacesTFCBlocks) {
                if (event.getNewState().getBlock() == Blocks.STONE) {
                    event.setNewState(StorageRock.getRockBlock(RockBlockVariants.RAW, RockTypes.BASALT).getDefaultState().withProperty(BlockRockRaw.CAN_FALL, false));
                }
                if (event.getNewState().getBlock() == Blocks.COBBLESTONE) {
                    event.setNewState(StorageRock.getRockBlock(RockBlockVariants.RAW, RockTypes.RHYOLITE).getDefaultState().withProperty(BlockRockRaw.CAN_FALL, false));
                }
            }
        }
    }

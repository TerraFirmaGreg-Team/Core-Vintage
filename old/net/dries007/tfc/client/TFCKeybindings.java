...
public class TFCKeybindings {
    public static final KeyBinding OPEN_CRAFTING_TABLE = new KeyBinding("tfc.key.craft", KeyConflictContext.IN_GAME, Keyboard.KEY_C, Tags.MOD_NAME);
    public static final KeyBinding PLACE_BLOCK = new KeyBinding("tfc.key.placeblock", KeyConflictContext.IN_GAME, Keyboard.KEY_V, Tags.MOD_NAME);
    public static final KeyBinding CHANGE_ITEM_MODE = new KeyBinding("tfc.key.itemmode", KeyConflictContext.IN_GAME, Keyboard.KEY_M, Tags.MOD_NAME);
    public static final KeyBinding STACK_FOOD = new KeyBinding("tfc.key.stack", KeyConflictContext.GUI, Keyboard.KEY_X, Tags.MOD_NAME);
    ...
    public static void onKeyEvent(InputEvent event) {
        ...
        if (TFCKeybindings.OPEN_CRAFTING_TABLE.isPressed()) {
            ModuleCoreOld.PACKET_SERVICE.sendToServer(new SCPacketOpenCraftingGui());
        }
        if (TFCKeybindings.PLACE_BLOCK.isPressed()) {
            ModuleCoreOld.PACKET_SERVICE.sendToServer(new SCPacketPlaceBlockSpecial());
        }
        if (TFCKeybindings.CHANGE_ITEM_MODE.isPressed()) {
            ModuleCoreOld.PACKET_SERVICE.sendToServer(new SCPacketCycleItemMode());
        }
    }
        ...
        if (!Keyboard.isRepeatEvent() && Keyboard.getEventKeyState() && Keyboard.getEventKey() == TFCKeybindings.STACK_FOOD.getKeyCode()) {
            if (event.getGui() instanceof GuiContainer) {
                ...
                if (slotUnderMouse != null) {
                    ModuleCoreOld.PACKET_SERVICE.sendToServer(new SCPacketStackFood(slotUnderMouse.slotNumber));
                }
            }
        }
    }

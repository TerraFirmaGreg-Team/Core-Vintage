package su.terrafirmagreg.modules.worldgen.classic.objects.layer.datalayers.ph;
...
public class GenLayerAddPH extends GenLayerBase {
    ...
    for (int var11 = 0; var11 < zSize; ++var11) {
      for (int var12 = 0; var12 < xSize; ++var12) {
        ...
        if (id0 > thisID || id1 > thisID || id2 > thisID || id3 > thisID) {
          ...
          if (id0 < GenLayerPH.MAX && this.nextInt(count++) == 0) {
            outID = id0 + 1;
          }

          if (id1 < GenLayerPH.MAX && this.nextInt(count++) == 0) {
            outID = id1 + 1;
          }

          if (id2 < GenLayerPH.MAX && this.nextInt(count++) == 0) {
            outID = id2 + 1;
          }

          if (id3 < GenLayerPH.MAX && this.nextInt(count++) == 0) {
            outID = id3 + 1;
          }

          if (this.nextInt(3) == 0 && outID <= GenLayerPH.MAX) {
          ...
          if (id0 > GenLayerPH.MIN && this.nextInt(count++) == 0) {
            outID = id0 - 1;
          }

          if (id1 > GenLayerPH.MIN && this.nextInt(count++) == 0) {
            outID = id1 - 1;
          }

          if (id2 > GenLayerPH.MIN && this.nextInt(count++) == 0) {
            outID = id2 - 1;
          }

          if (id3 > GenLayerPH.MIN && this.nextInt(count++) == 0) {
            outID = id3 - 1;
          }

          if (this.nextInt(3) == 0 && outID >= GenLayerPH.MIN) {
            outCache[var12 + var11 * xSize] = outID;
          } else {
            outCache[var12 + var11 * xSize] = thisID;
          }

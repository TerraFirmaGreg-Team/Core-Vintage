...
/**
 * ModelYakTFC Created using Tabula 7.1.0
 */

@SideOnly(Side.CLIENT)

public class ModelAnimalYak extends ModelBase {
    ...
    public void render(@NotNull Entity entity, float par2, float par3, float par4, float par5, float par6, float par7) {
        ...
        if (yak.getGender() == EntityAnimalBase.Gender.MALE) {
            if (percent < 0.5) {
                udder.isHidden = true;
    ...
    public void setRotationAngles(float par1, float par2, float par3, float par4, float par5, float par6, Entity ent) {
        ...
        this.legFrontLeft.rotateAngleX = MathHelper.cos(par1 * 0.6662F + MathConstants.PI) * 1.4F * par2;
        this.legBackRight.rotateAngleX = MathHelper.cos(par1 * 0.6662F + MathConstants.PI) * 1.4F * par2;

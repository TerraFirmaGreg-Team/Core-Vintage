...
public class UnmoldRecipeStoneware extends IForgeRegistryEntry.Impl<IRecipe> implements IRecipe {
    ...
    private UnmoldRecipeStoneware(@Nullable ResourceLocation group, NonNullList<Ingredient> input, @NotNull ItemMetalTFCE.ItemType type,
                                  float chance) {
        this.group = group;
        this.input = input;
        this.type = type;
    ...
    public boolean matches(@NotNull InventoryCrafting inv, @NotNull World world) {
        ...
        for (int slot = 0; slot < inv.getSizeInventory(); slot++) {
            ...
            if (!stack.isEmpty()) {
                if (stack.getItem() instanceof ItemStonewareMoldTFCE) {
                    ItemStonewareMoldTFCE moldItem = ((ItemStonewareMoldTFCE) stack.getItem());
    ...
    public ItemStack getCraftingResult(InventoryCrafting inv) {
        ...
        for (int slot = 0; slot < inv.getSizeInventory(); slot++) {
            ...
            if (!stack.isEmpty()) {
                if (stack.getItem() instanceof ItemStonewareMoldTFCE) {
                    ItemStonewareMoldTFCE tmp = ((ItemStonewareMoldTFCE) stack.getItem());

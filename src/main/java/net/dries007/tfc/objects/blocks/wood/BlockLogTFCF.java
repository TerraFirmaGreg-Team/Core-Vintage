...
public class BlockLogTFCF extends BlockLog implements ICapabilitySize {
  ...
  public static BlockLogTFCF get(Tree wood) {
    return MAP_TREE.get(wood);
  }
  ...
  public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
    ...
    for (int x = -1; x <= 1; x++) {
      for (int y = -1; y <= 1; y++) {
        for (int z = -1; z <= 1; z++) {if (world.getBlockState(pos.add(x, y, z)).getBlock() == this && (z != 0 || y != 0 || x != 0)) {return;}}
      }
    }
  ...
  // Временно отключено, из-за несовместимости с gt рубкой
  @Override

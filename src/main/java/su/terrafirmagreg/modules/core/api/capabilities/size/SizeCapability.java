...
public final class SizeCapability {
    ...
    public static void init() {
        ...
        CUSTOM_ITEMS.put(IIngredient.of(Items.CLAY_BALL),
                () -> SizeProvider.get(Size.SMALL, Weight.VERY_LIGHT, true)); // Store anywhere stacksize = 64
        CUSTOM_ITEMS.put(IIngredient.of(Items.BED),
                () -> SizeProvider.get(Size.LARGE, Weight.VERY_HEAVY, false)); // Store only in chests stacksize = 1
        CUSTOM_ITEMS.put(IIngredient.of(Items.MINECART),
                () -> SizeProvider.get(Size.LARGE, Weight.VERY_HEAVY, false)); // Store only in chests stacksize = 1
        CUSTOM_ITEMS.put(IIngredient.of(Items.ARMOR_STAND),
                () -> SizeProvider.get(Size.LARGE, Weight.HEAVY, true)); // Store only in chests stacksize = 4
        CUSTOM_ITEMS.put(IIngredient.of(Items.CAULDRON),
                () -> SizeProvider.get(Size.LARGE, Weight.LIGHT, true)); // Store only in chests stacksize = 32

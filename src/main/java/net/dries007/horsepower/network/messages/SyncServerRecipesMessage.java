package net.dries007.horsepower.network.messages;
...
import net.dries007.horsepower.network.ClientMessageHandler;
...
public class SyncServerRecipesMessage extends ClientMessageHandler<SyncServerRecipesMessage> implements IMessage {
  ...
  public void fromBytes(ByteBuf buf) {
    ...
    for (int i = 0; i < size; i++) {grindstoneRecipes.add(ByteBufUtils.readUTF8String(buf));}
    size = buf.readInt();
    for (int i = 0; i < size; i++) {handGrindstoneRecipes.add(ByteBufUtils.readUTF8String(buf));}
    size = buf.readInt();
    for (int i = 0; i < size; i++) {choppingRecipes.add(ByteBufUtils.readUTF8String(buf));}
    size = buf.readInt();
    for (int i = 0; i < size; i++) {manualChoppingRecipes.add(ByteBufUtils.readUTF8String(buf));}
    size = buf.readInt();
    for (int i = 0; i < size; i++) {pressRecipes.add(ByteBufUtils.readUTF8String(buf));}
  }
  ...
  protected void handle(SyncServerRecipesMessage message, MessageContext ctx) {
    ...
    HPRecipes.instance()
      .reloadRecipes(message.grindstoneRecipes, message.handGrindstoneRecipes, message.choppingRecipes, message.manualChoppingRecipes, message.pressRecipes);

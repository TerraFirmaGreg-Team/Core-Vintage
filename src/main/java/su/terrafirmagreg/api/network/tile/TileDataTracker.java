...
public class TileDataTracker {
    ...
    private final ArrayList<ITileData> data;
     ...
     * <p>
     * Returns a packet buffer containing the serialized bytes of only the data that has updated. If no data has updated, an empty buffer is
     * returned.
     */
     ...
     * <p>
     * 2020-07-04 This method has been modified to clear the dirty flag on all the data elements that a TE has instead of only clearing data elements
     * that have arrived on the client from the server. This prevents a case in which a client reads a TE from NBT and triggers the dirty flag on
     * something like an item stack handler, then the flag never gets cleared and any code that is meant to run only in the event of a change gets run
     * every tick instead.
     *

...
public class BaseBlockFluid extends BlockFluidClassic implements IBlockSettings {
  ...
  public BaseBlockFluid(Fluid fluid, Material material, boolean canCreateSources) {
    this(fluid, material);
  ...
  public BaseBlockFluid(Fluid fluid, Material material) {
    ...
    this.settings = Settings.of(material);
    ...
    if (!isSourceBlock(world, pos)) {
      ...
      if (world.getBlockState(pos.down(densityDir)).getBlock() != this) {
        for (EnumFacing side : EnumFacing.HORIZONTALS) {
          ...
          if (block instanceof BaseBlockFluid neighborBlock) {
            int neighborMeta;
  ...
  protected void flowIntoBlock(World world, BlockPos pos, int meta) {
    ...
    if (displaceIfPossible(world, pos)) {
      ...
      if (targetBlock instanceof BaseBlockFluid) {
        ...
        if (!replace) {
          ...
          if (((BaseBlockFluid) targetBlock).getDensity() > this.getDensity()) {
            flowStrength += 1;
          }
  ...
  public boolean canDisplace(IBlockAccess world, BlockPos pos) {
    ...
    if (block instanceof BaseBlockFluid) {

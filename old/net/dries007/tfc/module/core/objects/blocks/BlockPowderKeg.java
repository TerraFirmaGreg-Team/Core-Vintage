...
public class BlockPowderKeg extends BlockBase implements IItemSizeAndWeight, ILightableBlock {
    ...
    public void trigger(World worldIn, BlockPos pos, IBlockState state, @Nullable EntityLivingBase igniter) {
        if (!worldIn.isRemote) {
            ...
            if (te != null && state.getValue(SEALED) && state.getValue(ILightableBlock.LIT) && te.getStrength() > 0) //lit state set before called
            {
    ...
    public void neighborChanged(IBlockState state, World world, BlockPos pos, Block blockIn, BlockPos fromPos) {
        if (world.isBlockPowered(pos) || world.getBlockState(fromPos).getMaterial() == Material.FIRE) {
            onPlayerDestroy(world, pos, state.withProperty(ILightableBlock.LIT, true));
        } else if (state.getValue(ILightableBlock.LIT) && pos.up().equals(fromPos) && world.getBlockState(fromPos).getMaterial() == Material.WATER) {
            TEPowderKeg tile = Helpers.getTE(world, pos, TEPowderKeg.class);
            if (tile != null) {
    ...
    public void onBlockAdded(World worldIn, BlockPos pos, IBlockState state) {
        ...
        if (worldIn.isBlockPowered(pos)) {
            onPlayerDestroy(worldIn, pos, state.withProperty(ILightableBlock.LIT, true));
        }
    }
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if (!worldIn.isRemote) {
            ...
            TEPowderKeg te = Helpers.getTE(worldIn, pos, TEPowderKeg.class);
            if (te != null) {
                if (heldItem.isEmpty() && state.getValue(ILightableBlock.LIT)) {
                    worldIn.setBlockState(pos, state.withProperty(ILightableBlock.LIT, false));
                    te.setLit(false);
                    ...
                    if (!state.getValue(ILightableBlock.LIT)) {
                        ...
                        trigger(worldIn, pos, state.withProperty(ILightableBlock.LIT, true), playerIn);
    ...
    public void onEntityCollision(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
        if (!worldIn.isRemote && entityIn instanceof EntityArrow entityarrow) {
            ...
            if (entityarrow.isBurning()) {
                trigger(worldIn, pos, worldIn.getBlockState(pos).withProperty(ILightableBlock.LIT, true), entityarrow.shootingEntity instanceof EntityLivingBase ? (EntityLivingBase) entityarrow.shootingEntity : null);
            }
        }
    }
    ...
    public BlockStateContainer createBlockState() {
        return new BlockStateContainer(this, SEALED, ILightableBlock.LIT);
    }

    @Override
    public int getLightValue(IBlockState state, IBlockAccess world, BlockPos pos) {
        return state.getValue(ILightableBlock.LIT) ? 14 : 0;
    }
    ...
    public void onBlockExploded(World worldIn, BlockPos pos, Explosion explosionIn) {
        ...
        {
            trigger(worldIn, pos, worldIn.getBlockState(pos).withProperty(SEALED, true).withProperty(ILightableBlock.LIT, true), null);
        } else {

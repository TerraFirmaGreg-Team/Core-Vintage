package net.dries007.tfc.module.metal.objects.tiles;
    ...
    public boolean attemptWelding(EntityPlayer player) {
        ...
        if (recipe != null) {
            ...
            if (fluxStack.isEmpty()) {
                // No flux
                ModuleCoreOld.PACKET_SERVICE.sendTo(SCPacketSimpleMessage.translateMessage(MessageCategory.ANVIL, Tags.MOD_ID + ".tooltip.anvil_no_flux"), (EntityPlayerMP) player);
                return false;
            }
            ...
            if (cap1 == null || cap2 == null || !cap1.isWeldable() || !cap2.isWeldable()) {
                if (cap1 instanceof IItemHeat && cap2 instanceof IItemHeat) {
                    ModuleCoreOld.PACKET_SERVICE.sendTo(SCPacketSimpleMessage.translateMessage(MessageCategory.ANVIL, Tags.MOD_ID + ".tooltip.anvil_too_cold"), (EntityPlayerMP) player);
                } else {
                    ModuleCoreOld.PACKET_SERVICE.sendTo(SCPacketSimpleMessage.translateMessage(MessageCategory.ANVIL, Tags.MOD_ID + ".tooltip.anvil_not_weldable"), (EntityPlayerMP) player);
                }
                return false;
            }

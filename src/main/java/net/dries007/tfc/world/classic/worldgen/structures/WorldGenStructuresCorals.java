            ...
            for (int i = 0; i < coralsInChunk; i++) {
                ...
                if (ConfigTFCF.General.STRUCTURES.activateStructureGeneration) {
                    if (data.isInitialized() &&
                            (b == BiomesTFC.OCEAN || b == BiomesTFC.DEEP_OCEAN || b == BiomesTFC.BEACH || b == BiomesTFC.GRAVEL_BEACH)) {
                        if ((up.getBlock() instanceof BlockCoral || world.getBlockState(pos)
                                ...
                                .getBlock() == ChunkGenTFC.SALT_WATER.getBlock()) &&
                                (pos.getY() < WorldTypeTFC.SEALEVEL - 7 && pos.getY() > 119 && data.getFloraDiversity() >= 0.3f &&
                                        data.getFloraDensity() >= 0.4f && data.getFloraDensity() <= 0.6f && data.getFloraDiversity() <= 0.5f &&
                                        data.getAverageTemp() >= 10f && data.getAverageTemp() <= 28f && data.getRainfall() >= 150f)) {

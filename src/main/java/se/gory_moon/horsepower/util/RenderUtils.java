...
public class RenderUtils {
  ...
  public static TextureAtlasSprite getTopTextureFromBlockstate(IBlockState state) {
    ...
    if (model != Minecraft.getMinecraft()
            .getBlockRendererDispatcher()
            .getBlockModelShapes()
            .getModelManager()
            .getMissingModel()) {
      List<BakedQuad> quads = model.getQuads(state, EnumFacing.UP, 0);
      return quads.size() >= 1 ? quads.get(0).getSprite() : Minecraft.getMinecraft()
              .getBlockRendererDispatcher()
              .getBlockModelShapes()
              .getTexture(state);
    }
        ...
        for (int zo = -3; zo <= 3; zo++) {
          ...
          if (pos.getY() >= 0) {
            ...
            if (!state.getBlock().isReplaceable(world, pos)) {
              ...
              drawBoundingBoxOutline(new AxisAlignedBB(pos).offset(-Minecraft.getMinecraft()
                      .getRenderManager().viewerPosX, -Minecraft.getMinecraft()
                      .getRenderManager().viewerPosY, -Minecraft.getMinecraft()
                      .getRenderManager().viewerPosZ));
              drawBoundingBox(state.getBoundingBox(world, pos)
                      .offset(pos)
                      .offset(-Minecraft.getMinecraft()
                              .getRenderManager().viewerPosX, -Minecraft.getMinecraft()
                              .getRenderManager().viewerPosY, -Minecraft.getMinecraft()
                              .getRenderManager().viewerPosZ));
            } else {

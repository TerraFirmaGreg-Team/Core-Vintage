...
import net.dries007.tfc.world.classic.spawner.WorldEntitySpawner;
...
public class ChunkGenTFC implements IChunkGenerator {
  ...
  public ChunkGenTFC(World w, String settingsString) {
    ...
    WorldEntitySpawner.init(); // Called here so only TFC Worlds are affected
  }
          ...
          for (int yy = 0; yy < 8; ++yy) {
            ...
            for (int xx = 0; xx < 4; ++xx) {
              ...
              for (int zz = 0; zz < 4; ++zz) {
                if ((var47 += var49) > 0.0D) {primer.setBlockState(x * 4 + xx, y * 8 + yy, z * 4 + zz, STONE);} else if (y * 8 + yy < seaLevel) {
      ...
      for (int z = 0; z < 5; ++z) {
        ...
        for (int xR = -2; xR <= 2; ++xR) {
          for (int zR = -2; zR <= 2; ++zR) {
            ...
            if (blendBiome.getBaseHeight() > baseBiome.getBaseHeight()) {blendedHeight *= 0.5F;}
        ...
        for (int y = 0; y < 17; ++y) {
          ...
          if (var38 < 0.0D) {output = var34;} else if (var38 > 1.0D) {output = var36;} else {output = var34 + (var36 - var34) * var38;}
  ...
  private void replaceBlocksForBiomeHigh(int chunkX, int chunkZ, ChunkPrimer inp, CustomChunkPrimer outp) {
    ...
    for (int x = 0; x < 16; ++x) {
      for (int z = 0; z < 16; ++z) {
        ...
        if (BiomeHelper.isBeach(getBiomeOffset(x - 1, z)) || BiomeHelper.isBeach(getBiomeOffset(x + 1, z))
            || BiomeHelper.isBeach(getBiomeOffset(x, z
                                                     + 1))
            || BiomeHelper.isBeach(getBiomeOffset(x, z - 1))) {
          if (!BiomeHelper.isBeach(getBiomeOffset(x, z))) {cliffMap[colIndex] = true;}
        }
        ...
        for (int y = 255 - yOffset; y >= 0; y--) {
          ...
          int beachCliffHeight =
            nonBeachTiles > 0 ? (int) ((highestStone - highestBeachTheoretical) * (nonBeachTiles) / 6.0 + highestBeachTheoretical) : highestBeachTheoretical;

          //Redo cliffs
          if (BiomeHelper.isBeach(biome) && y > seaLevel && outp.getBlockState(x, y + yOffset, z) != AIR && y >= beachCliffHeight) {
            inp.setBlockState(x, y, z, AIR);
            outp.setBlockState(x, y + yOffset, z, AIR);
          }
          //Ensure rivers can't get blocked
          if (BiomeHelper.isRiver(biome) && y >= seaLevel - 2 && outp.getBlockState(x, y + yOffset, z) != AIR) {
            ...
            //outp.setBlockState(x, y + yOffset, z, y >= seaLevel ? AIR : SALT_WATER);
          } else if (!BiomeHelper.isRiver(biome) && nonRiverTiles < 9 && outp.getBlockState(x, y + yOffset, z) == STONE && (
          ...
          if (outp.getBlockState(x, y + yOffset, z) == STONE) {
            if (seaLevelOffsetMap[colIndex] == 0 && y - seaLevel >= 0) {seaLevelOffsetMap[colIndex] = y - seaLevel;}

            if (chunkHeightMap[colIndex] == 0) {chunkHeightMap[colIndex] = y + yOffset;}
            ...
            } else {outp.setBlockState(x, y + yOffset, z, BlockRockVariant.get(rock1, Rock.Type.RAW).getDefaultState());}
            ...
            if (smooth == -1) {
              ...
              for (int c = 1; c < 3; c++) {
                ...
                if (current != surfaceBlock && current != subSurfaceBlock && !BlocksTFC.isWater(current)) {
                  ...
                  if (outp.getBlockState(x, yOffset + y + c + 1, z) == SALT_WATER) {outp.setBlockState(x, yOffset + y + c, z, subSurfaceBlock);}
                }
              }
              ...
              if (smooth > 0) {
                if (y >= seaLevel - 1 && y + 1 < yOffset && inp.getBlockState(x, y + 1, z) != SALT_WATER && dirtH > 0 && !(BiomeHelper.isBeach(biome)
                                                                                                                           && y >
          ...
          else if (inp.getBlockState(x, y, z) == SALT_WATER && !(BiomeHelper.isOceanic(biome) || BiomeHelper.isBeach(biome))) {
        ...
        for (int y = yOffset - 1; y >= 0; y--) // This cannot be optimized with the prev for loop, because the sealeveloffset won't be ready yet.
        {
          ...
          if (y < 1 + (s.flatBedrock ? 0 : rand.nextInt(3))) //  + (seaLevelOffsetMap[colIndex] / 3)
          {
            ...
            } else {outp.setBlockState(x, y, z, BlockRockVariant.get(rock1, Rock.Type.RAW).getDefaultState());}

            if (BiomeHelper.isBeach(biome) || BiomeHelper.isOceanic(biome)) {

...
public class BlockPlantFloatingWater extends BlockPlant implements IPlantBlock {
    ...
    public BlockPlantFloatingWater(PlantEnumVariant plantBlockVariant, PlantType plant) {
    ...
    public void onBlockAdded(World world, BlockPos pos, IBlockState state) {
        world.setBlockState(pos, state.withProperty(BlockPlant.DAYPERIOD, getDayPeriod()).withProperty(growthStageProperty, plant.getStageForMonth()));
    ...
    public boolean canBlockStay(World worldIn, BlockPos pos, IBlockState state) {
        if (pos.getY() >= 0 && pos.getY() < 256) {
            ...
            return (material == Material.WATER && stateDown.getValue(BlockLiquid.LEVEL) == 0 && stateDown == plant.getWaterType()) || material == Material.ICE || (material == Material.CORAL && !(state.getBlock() instanceof BlockPlantEmergentTallWater));
        } else {
            return false;
        }
    }
    ...
    protected BlockStateContainer createPlantBlockState() {
        return new BlockStateContainer(this, growthStageProperty, BlockPlant.DAYPERIOD, BlockPlant.AGE);
    }
}

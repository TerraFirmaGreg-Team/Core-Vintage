...
import static su.terrafirmagreg.modules.device.object.block.BlockSmelteryCauldron.LIT;

@SuppressWarnings("WeakerAccess")
@ParametersAreNonnullByDefault
public class TileSmelteryCauldron extends TEInventory implements ITickable, IFluidHandlerSidedCallback, IFluidTankCallback, ITileFields {
  ...
  public void update() {
    if (!world.isRemote) {
      if (++reload >= 10) {
        reload = 0;
        TileSmelteryFirebox firebox = Helpers.getTE(world, pos.down(), TileSmelteryFirebox.class);
        if (firebox != null) {
        ...
        if (recipe != null) {
          ...
          if (recipe.getMeltTemp() <= temp && tank.fill(output, false) >= output.amount) {
            ...
            temp -= ConfigTFC.Devices.HEAT.heatingModifier * 150;
            if (firebox != null) {

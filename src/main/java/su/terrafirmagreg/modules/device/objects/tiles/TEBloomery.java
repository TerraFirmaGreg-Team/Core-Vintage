...
public class TEBloomery extends TETickableInventory implements ITickable {
    ...
    public void update() {
        ...
        if (!world.isRemote && world.getTotalWorldTime() % 20 == 0) {
            ...
            if (state.getValue(LIT)) {
                if (this.getRemainingTicks() <= 0) {
                    ...
                    if (cachedRecipe != null) {
                        ...
                        TEBloom te = TileUtils.getTile(world, getInternalBlock(), TEBloom.class);
                        if (te != null) {
            ...
            while (maxOre < oreStacks.size()) {
                ...
                InventoryHelper.spawnItemStack(world, getExternalBlock().getX(), getExternalBlock().getY(), getExternalBlock().getZ(),
                        oreStacks.get(0));
                oreStacks.remove(0);
                markForSync();
            }
    ...
    protected void dumpItems() {
        ...
        oreStacks.forEach(
                i -> InventoryHelper.spawnItemStack(world, getExternalBlock().getX(), getExternalBlock().getY(), getExternalBlock().getZ(), i));
        fuelStacks.forEach(
                i -> InventoryHelper.spawnItemStack(world, getExternalBlock().getX(), getExternalBlock().getY(), getExternalBlock().getZ(), i));
    }
    ...
    protected void addItemsFromWorld() {
        ...
        for (EntityItem entityItem : world.getEntitiesWithinAABB(EntityItem.class,
                new AxisAlignedBB(getInternalBlock().up(), getInternalBlock().add(1, 4, 1)), EntitySelectors.IS_ALIVE)) {

...
public class GuiButtonKnapping extends GuiButton {
    ...
    public void onClick() {
        if (this.enabled) {
            this.visible = false;
            ModuleCoreOld.PACKET_SERVICE.sendToServer(new SCPacketGuiButton(this.id));
        }
    }
    ...
    public void playPressSoundBasedOnKnappingType(SoundHandler soundHandler, KnappingType knappingType) {
        ...
        switch (knappingType) {
            case KnappingType.STONE -> soundEvent = TFCSounds.KNAPPING_ROCK;
            case KnappingType.CLAY, KnappingType.FIRE_CLAY -> soundEvent = TFCSounds.KNAPPING_CLAY;
            case KnappingType.LEATHER -> soundEvent = TFCSounds.KNAPPING_LEATHER;
        }

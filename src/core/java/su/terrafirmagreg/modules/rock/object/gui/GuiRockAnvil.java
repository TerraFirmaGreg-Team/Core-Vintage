package su.terrafirmagreg.modules.rock.object.gui;
...
public class GuiRockAnvil
  ...
  private void updatePage() {
    ...
    for (int i = page * 18; i < (page + 1) * 18 && i < recipeList.size(); i++) {
      ...
      addButton(new GuiButtonAnvilPlanIcon((AnvilRecipe) recipeList.get(i), ++buttonID, guiLeft + posX, guiTop + posY));
    }
  ...
  protected void actionPerformed(GuiButton button) throws IOException {
    if (button instanceof GuiButtonAnvilPlanIcon) {
      ...
      new CSPacketGuiButton(button.id, new NBTBuilder().setString("recipe", recipeName.toString()).build()).sendToServer();

package su.terrafirmagreg.modules.device.object.render;
...
public class TESRSluice extends TileEntitySpecialRenderer<TileSluice> {

  private static final ItemStack GRAVEL = new ItemStack(Blocks.GRAVEL);
  private static final Random SOIL_NOISE = new Random();

  @Override
  public void render(TileSluice te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
    ...
    int soilBlocks = Math.min((int) Math.ceil(te.getSoil() * 5D / TileSluice.MAX_SOIL), 5);
  ...
  @Override
  public boolean isGlobalRenderer(TileSluice te) {
    return true;
  }

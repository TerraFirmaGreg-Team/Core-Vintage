        ...
        if (chunkGenerator instanceof ChunkGenTFC && world.provider.getDimension() == 0) {
            for (int x = 0; x < 16; x++) {
                for (int z = 0; z < 16; z++) {
                    ...
                    if (actualTemp < IceMeltHandler.ICE_MELT_THRESHOLD - 4 + 4 * (rand.nextFloat() - rand.nextFloat()) &&
                            stateAt.getBlock() == ChunkGenTFC.FRESH_WATER.getBlock()) {
                        world.setBlockState(posDown, ChunkGenTFC.FRESH_WATER_ICE);
                    }

                    if (world.isAirBlock(pos) && ChunkGenTFC.SNOW.getBlock()
                            .canPlaceBlockAt(world, pos) && actualTemp < -4 + 4 * (rand.nextFloat() - rand.nextFloat())) {

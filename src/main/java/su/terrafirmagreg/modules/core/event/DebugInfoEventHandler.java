...
public class DebugInfoEventHandler {
    ...
    private ChunkData getChunkData(Chunk chunk) {
        return chunk.getCapability(ChunkDataProvider.CHUNK_DATA_CAPABILITY, null);
    }

    private void addChunkData(List<String> list, ChunkData data, BlockPos blockPos) {
        ...
        var minTemp = String.format("%s%.1f" + DEGREE + "C", AQUA, ClimateHelper.monthFactor(data.getRegionalTemp(), Month.JANUARY.getTemperatureModifier(), blockPos.getZ()));
        var maxTemp = String.format("%s%.1f" + DEGREE + "C", AQUA, ClimateHelper.monthFactor(data.getRegionalTemp(), Month.JULY.getTemperatureModifier(), blockPos.getZ()));
    ...
    private void addTemperatureData(List<String> list, ChunkData data, BlockPos blockPos) {
    ...
    private void addRainfallData(List<String> list, ChunkData data) {
        list.add(String.format("%sRainfall: %s%.1f", YELLOW, AQUA, data.getRainfall()));
    }

    private void addFloraData(List<String> list, ChunkData data) {
        list.add(String.format("%sFlora Density: %s%s", YELLOW, AQUA, data.getFloraDensity()));
        list.add(String.format("%sFlora Diversity: %s%s", YELLOW, AQUA, data.getFloraDiversity()));
    }

    private void addSpawnProtectionData(List<String> list, ChunkData data) {

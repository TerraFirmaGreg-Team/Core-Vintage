...
public class BlockQuern extends Block implements ICapabilitySize, IHighlightHandler {
  ...
  private static SelectionPlace getPlayerSelection(World world, BlockPos pos, EntityPlayer player) {
    ...
    TileQuern teQuern = Helpers.getTE(world, pos, TileQuern.class);
    ...
    if (teQuern != null) {
      ...
      if (!teQuern.isGrinding() && teQuern.hasHandstone() && HANDLE_AABB.offset(pos).calculateIntercept(eyePos, lookingPos) != null) {
        return SelectionPlace.HANDLE;
                                                                                                                           ...
                                                                                                                           && !inventory.getStackInSlot(TileQuern.SLOT_INPUT)
        .isEmpty()))
  ...
  public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
    TileQuern teQuern = Helpers.getTE(source, pos, TileQuern.class);
      ...
      if (teQuern != null && !teQuern.isGrinding()) {
        ...
        if (inventory != null) {
          if (selection == SelectionPlace.HANDLE) {
            ...
            return true;
          } else if (selection == SelectionPlace.BASE && !inventory.getStackInSlot(TileQuern.SLOT_OUTPUT).isEmpty()) {
            ItemHandlerHelper.giveItemToPlayer(playerIn, inventory.extractItem(TileQuern.SLOT_OUTPUT, inventory.getStackInSlot(TileQuern.SLOT_OUTPUT)
              .getCount(), false));

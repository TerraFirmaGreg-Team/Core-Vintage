package net.dries007.horsepower.items;
...
public class ModItems {
  ...
  public static final Item FLOUR = new Item().setRegistryName(Constants.FLOUR_ITEM).setTranslationKey(Constants.FLOUR_ITEM)
    .setCreativeTab(CreativeTabsTFC.CT_FOOD);
  public static final Item DOUGH = new Item().setRegistryName(Constants.DOUGH_ITEM).setTranslationKey(Constants.DOUGH_ITEM)
    .setCreativeTab(CreativeTabsTFC.CT_FOOD);
  ...
  public static void registerRecipes() {
    ...
    if (Configs.general.enableFlour) {OreDictionary.registerOre("foodFlour", FLOUR);}
  }
  ...
  public static void registerRecipes(RegistryEvent.Register<IRecipe> ev) throws NoSuchFieldException, IllegalAccessException {
    ...
    ev.getRegistry()
      .register(new ShapedChoppingRecipe(loc, OreDictionary.getOres("logWood"), new ItemStack(ModBlocks.BLOCK_CHOPPER), "LSL", "SFS", "SWS", 'S', "stickWood", 'L', Ingredient.fromStacks(new ItemStack(Items.LEAD)), 'F', "saw", 'W', "logWood").setRegistryName(loc));
    if (Configs.general.enableHandChoppingBlock) {
      loc = new ResourceLocation("horsepower:manual_chopping");
      ev.getRegistry()
        .register(new ShapelessChoppingRecipe(loc, OreDictionary.getOres("logWood"), new ItemStack(ModBlocks.BLOCK_MANUAL_CHOPPER, 2), "logWood", "saw").setRegistryName(loc));
    }

    if (Configs.general.removeVanillaRecipes) {removeRecipes();}
  }
  ...
  private static void removeRecipe(ResourceLocation recipe) {
    if (recipe != null) {recipesToRemove.add(recipe);}
  }
  ...
  public static class RegistrationHandler {
    ...
    public static void registerItems(RegistryEvent.Register<Item> event) {
      ...
      for (final Item item : items) {
        if (!Configs.general.enableFlour && item == FLOUR) {continue;}
        if (!Configs.general.enableDough && item == DOUGH) {continue;}

...
public class ItemWateringCan extends ItemMisc {
        ...
        for (int z = -2; z <= 2; z++) {
          ...
          if (te instanceof IWaterable) {
            ...
            if (farmland.getBlock() instanceof BlockFarmlandTFC) {
              ...
              if (current < BlockFarmlandTFC.MAX_MOISTURE) {world.setBlockState(addPos, farmland.withProperty(BlockFarmlandTFC.MOISTURE, current + 1));}
            }
          }
        }
      }
  ...
  public void onUsingTick(ItemStack stack, EntityLivingBase entity, int countLeft) {
    ...
    if (world.isRemote && entity instanceof EntityPlayer && countLeft % 2 == 0) {
      ...
      double x = pos.getX() + MathUtils.RNG.nextFloat();
      double y = pos.getY() + 1.5f;
      double z = pos.getZ() + MathUtils.RNG.nextFloat();
      world.spawnParticle(EnumParticleTypes.WATER_SPLASH, x, y, z, MathUtils.RNG.nextFloat(), -0.2f, MathUtils.RNG.nextFloat());
    }
  }
}

...
public class FallingBlockManager {
    ...
    public static boolean hasSupportingSideBlock(IBlockState state) {
        return state.isNormalCube() || SIDE_SUPPORTS.contains(state) || state.getBlock() instanceof ISoilBlock &&
                (((ISoilBlock) state.getBlock()).getBlockVariant() == SoilBlockVariants.FARMLAND || ((ISoilBlock) state.getBlock()).getBlockVariant() == SoilBlockVariants.GRASS_PATH);
    }
    ...
    public static boolean shouldFall(World world, BlockPos posToFallFrom, BlockPos originalPos, IBlockState originalState, boolean ignoreSupportChecks) {
        return ConfigTFC.General.FALLABLE.enable && canFallThrough(world, posToFallFrom.down(), originalState.getMaterial()) && (ignoreSupportChecks || !BlockWoodSupport.isBeingSupported(world, originalPos));
    }
    ...
    public static boolean checkCollapsingArea(World world, BlockPos pos) {
        ...
        if (Constants.RNG.nextDouble() < ConfigTFC.General.FALLABLE.collapseChance) // Then, we check rng if a collapse should trigger
        {
            ...
            for (BlockPos checking : BlockWoodSupport.getAllUnsupportedBlocksIn(world, pos.add(-radX, -radY, -radZ), pos.add(radX, radY, radZ))) //9x5x9 max
            {
    ...
    public static void collapseArea(World world, BlockPos centerPoint) {
        ...
        for (BlockPos pos : BlockPos.getAllInBoxMutable(centerPoint.add(-radius, -4, -radius), centerPoint.add(radius, -4, radius))) {
            ...
            for (int y = 0; y <= 8; y++) {
                ...
                if (foundEmpty && (specAt = getSpecification(stateAt)) != null && specAt.collapsable && specAt.collapseChecker.canCollapse(world, posAt) && !BlockWoodSupport.isBeingSupported(world, posAt)) {

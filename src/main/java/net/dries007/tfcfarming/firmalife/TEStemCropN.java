...
public class TEStemCropN extends TEStemCrop {
  ...
  private double getDeltaAt(int i) {
    if (i == 0) {
      return factorList.get(1);
    } else {
      return factorList.get(i * 2 + 1) - factorList.get((i - 1) * 2 + 1);
    }
  }
  ...
  public void readFromNBT(@NotNull NBTTagCompound compound) {
    ...
    try {
      factorList = gson.fromJson(compound.getString("factorList"), new TypeToken<List<Double>>() {
      }.getType());
    } catch (Exception e) {
      e.printStackTrace();
    }

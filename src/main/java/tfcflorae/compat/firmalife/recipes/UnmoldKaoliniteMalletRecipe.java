...
public class UnmoldKaoliniteMalletRecipe extends IForgeRegistryEntry.Impl<IRecipe> implements IRecipe {
  ...
  private UnmoldKaoliniteMalletRecipe(@Nullable ResourceLocation group, NonNullList<Ingredient> input, @Nonnull String type, float chance) {
    this.group = group;
    this.input = input;
    this.type = type;
    this.chance = chance;
  ...
  public boolean matches(@Nonnull InventoryCrafting inv, @Nonnull World world) {
    ...
    for (int slot = 0; slot < inv.getSizeInventory(); ++slot) {
      ...
      if (!stack.isEmpty()) {
        if (!(stack.getItem() instanceof ItemKaoliniteMalletMoldFL moldItemKaolinite)) {
          return false;
        }

...
public class UnmoldRecipeEarthenware extends IForgeRegistryEntry.Impl<IRecipe> implements IRecipe {
    private final NonNullList<Ingredient> input;
    private final ResourceLocation group;
    private final ItemMetalTFCE.ItemType type;
    private final float chance; // Return chance

    private UnmoldRecipeEarthenware(@Nullable ResourceLocation group, NonNullList<Ingredient> input, @Nonnull ItemMetalTFCE.ItemType type, float chance) {
        this.group = group;
        this.input = input;
        this.type = type;
        this.chance = chance;
    ...
    public boolean matches(@Nonnull InventoryCrafting inv, @Nonnull World world) {
        ...
        for (int slot = 0; slot < inv.getSizeInventory(); slot++) {
            ...
            if (!stack.isEmpty()) {
                if (stack.getItem() instanceof ItemEarthenwareMoldTFCE) {
                    ItemEarthenwareMoldTFCE moldItem = ((ItemEarthenwareMoldTFCE) stack.getItem());
    ...
    public ItemStack getCraftingResult(InventoryCrafting inv) {
        ...
        for (int slot = 0; slot < inv.getSizeInventory(); slot++) {
            ...
            if (!stack.isEmpty()) {
                if (stack.getItem() instanceof ItemEarthenwareMoldTFCE) {
                    ItemEarthenwareMoldTFCE tmp = ((ItemEarthenwareMoldTFCE) stack.getItem());
        ...
        if (moldStack != null) {
            ...
            if (moldCap instanceof IMoldHandler) {
                ...
                if (!moldHandler.isMolten() && moldHandler.getAmount() == ((ItemEarthenwareMoldTFCE) moldStack.getItem()).getFluidCapacity()) {
    ...
    public static class Factory implements IRecipeFactory {
        ...
        public IRecipe parse(final JsonContext context, final JsonObject json) {
            ...
            return new UnmoldRecipeEarthenware(group.isEmpty() ? new ResourceLocation(result) : new ResourceLocation(group), ingredients, type, chance);
        }
    }
}

...
public final class FluidTransferHelper {
     ...
     * {@link FluidUtil#tryPickUpFluid(ItemStack, EntityPlayer, World, BlockPos, EnumFacing)} with one key difference: this will always pick up the
     * block if any amount can be filled. Used for the ceramic jug, as it has < 1 B of storage, but still needs to be able to pick up some fluid
     *
     * @param emptyContainer The empty container to fill. Will not be modified directly, if modifications are necessary a modified copy is returned in
     *                       the result.
    ...
    /**
     * Tries to empty a fluid container item into the fluid handler, then stores the remainder in the given inventory. If the fluid can't be
     * transferred into the tank or the remainder can't fit into the return inventory, the action will be aborted.
     *
     * @param container        The filled fluid container ItemStack to empty. Will not be modified directly, if modifications are necessary a modified
    ...
    /**
     * Tries to fill a fluid container item from the fluid handler, then stores the result in the given inventory. If the fluid can't be transferred
     * into the container or the result can't fit into the return inventory, the action will be aborted.
     *
     * @param container       The fluid container ItemStack to fill. Will not be modified directly, if modifications are necessary a modified copy is
     ...
     *
     * @param container        The filled container. Will not be modified. Separate handling must be done to reduce the stack size, stow containers,
     *                         etc, on success.
     ...
     * @param world            World to play the sound in. Null for no sound.
     * @param pos              BlockPos to play the sound at. Null for no sound.
     * @return A {@link FluidActionResult} holding the empty container if the fluid handler was filled. NOTE If the container is consumable, the empty
     * container will be null on success.
     */
    @NotNull
     ...
     * Fill a container from the given fluidSource.
     *
     * @param container   The container to be filled. Will not be modified. Separate handling must be done to reduce the stack size, stow containers,
     *                    etc, on success.
     * @param fluidSource The fluid handler to be drained.
     * @param maxAmount   The largest amount of fluid that should be transferred.
     * @param doFill      true if the container should actually be filled, false if it should be simulated.
    ...
    /**
     * Fill a destination fluid handler from a source fluid handler with a max amount. To transfer as much as possible, use {@link Integer#MAX_VALUE}
     * for maxAmount.
     *
    ...
    /**
     * Internal method for filling a destination fluid handler from a source fluid handler using a specific fluid. Assumes that "drainable" can be
     * drained from "fluidSource". or {@link #tryFluidTransfer(IFluidHandler, IFluidHandler, int, boolean)}.
     */
    @Nullable

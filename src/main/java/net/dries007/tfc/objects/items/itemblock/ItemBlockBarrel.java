		...
		if (barrelCap instanceof ItemBarrelFluidHandler && ((ItemBarrelFluidHandler) barrelCap).getBarrelContents() != null) // Have either fluid or items stored
		{
			...
			if (fluidStack == null || fluidStack.amount == 0) {
				if (inventory.isEmpty()) {
					...
					tooltip.add(TextFormatting.BLUE + I18n.format(Constants.MODID_TFC + ".tooltip.barrel_item", inventory.getCount(), inventory.getItem()
					                                                                                                                           .getItemStackDisplayName(inventory)));
				}
			} else {
				tooltip.add(TextFormatting.BLUE + I18n.format(Constants.MODID_TFC + ".tooltip.barrel_fluid", fluidStack.amount, fluidStack.getLocalizedName()));

				if (!inventory.isEmpty()) {
					tooltip.add(TextFormatting.BLUE + I18n.format(Constants.MODID_TFC + ".tooltip.barrel_item_in_fluid", inventory.getCount(), inventory.getItem()
					                                                                                                                                    .getItemStackDisplayName(inventory)));
				}
			}

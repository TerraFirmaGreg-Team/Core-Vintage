...
public class GeneratorRockSpeleothem
  ...
  public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
    if (chunkGenerator instanceof ChunkGenClassic && world.provider.getDimension() == 0) {
      ...
      for (int i = 0; i < tries; i++) {
        ...
        var target = new BlockPos(
          x + random.nextInt(spread),
          random.nextInt(upperBound) + offset,
  ...
  private void placeSpeleothem(Random random, World world, BlockPos pos, Block block, boolean up) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (block instanceof IRockBlock rockBlock) {
        ...
        IBlockState targetBlock = BlocksRock.SPELEOTHEM.get(rockBlock.getType())
                                                       .getDefaultState()

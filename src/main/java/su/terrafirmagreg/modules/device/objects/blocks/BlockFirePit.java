...
public class BlockFirePit extends BlockBase implements IBellowsConsumerBlock, ITEBlock {
    ...
    public void randomTick(World worldIn, BlockPos pos, IBlockState state, Random random) {
        TEFirePit te = TileUtils.getTile(worldIn, pos, TEFirePit.class);
    ...
    public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random rng) {
        ...
        if (state.getValue(ATTACHMENT) == FirePitAttachment.COOKING_POT) {
            TEFirePit tile = TileUtils.getTile(world, pos, TEFirePit.class);
            ...
            if (tile != null) {
                ...
                if (cap != null) {
                    ...
                    if (state.getValue(LIT) && anythingInInv) {
                        world.playSound(x, y + 0.425F, z, SoundEvents.BLOCK_LAVA_EXTINGUISH, SoundCategory.BLOCKS, 0.25F,
                                rng.nextFloat() * 0.7F + 0.4F, false);
                        world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, x + rng.nextFloat() / 2 - 0.25, y + 0.6, z + rng.nextFloat() / 2 - 0.25,
                                0.0D, 0.1D, 0.0D);
                    }
                }
            }
    ...
    public void onAirIntake(World world, BlockPos pos, int airAmount) {
        TEFirePit teFirePit = TileUtils.getTile(world, pos, TEFirePit.class);
        if (teFirePit != null) {

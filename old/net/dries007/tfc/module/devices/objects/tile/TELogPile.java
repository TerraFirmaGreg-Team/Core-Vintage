...
import net.dries007.tfc.module.devices.objects.blocks.BlockLogPile;
...
public class TELogPile extends TEInventory implements ITickable {
    ...
    private void tryLightNearby() {
        for (EnumFacing side : EnumFacing.values()) {
            ...
            if (state.getBlock() instanceof BlockLogPile) {
                if (state.getValue(ILightableBlock.LIT)) continue;
                world.setBlockState(pos.offset(side), state.withProperty(ILightableBlock.LIT, true));
    ...
    private void createCharcoal() {
        ...
        if (j == 1) {
            ...
            world.setBlockState(pos, BlocksDevice.CHARCOAL_PILE.getDefaultState().withProperty(BlockCharcoalPile.LAYERS, charcoal));
            return;
        }
        for (int k = j - 1; k >= 0; k--) {
            ...
            if (state.getBlock() == Blocks.AIR) {
                ...
                world.setBlockState(pos.down(k), BlocksDevice.CHARCOAL_PILE.getDefaultState().withProperty(BlockCharcoalPile.LAYERS, charcoal));
                world.setBlockState(pos, Blocks.AIR.getDefaultState());
                return;
            }
            ...
            if (state.getBlock() instanceof BlockCharcoalPile) {
                ...
                charcoal += state.getValue(BlockCharcoalPile.LAYERS);
                int toCreate = Math.min(charcoal, 8);
                world.setBlockState(pos.down(k), BlocksDevice.CHARCOAL_PILE.getDefaultState().withProperty(BlockCharcoalPile.LAYERS, toCreate));
                charcoal -= toCreate;
            }

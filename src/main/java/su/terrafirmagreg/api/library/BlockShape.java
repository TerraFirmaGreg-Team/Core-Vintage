...
public class BlockShape {
  ...
  public BlockShape(List<AxisAlignedBB> shapes) {
    if (shapes == null) {shapes = Collections.emptyList();}

    List<AxisAlignedBB> boxes = new ArrayList<>(shapes.size());
    loop:
    for (int i = 0; i < shapes.size(); i++) {
      AxisAlignedBB shape1 = shapes.get(i);
      if (shape1.maxX - shape1.minX == 0 || shape1.maxY - shape1.minY == 0 || shape1.maxZ - shape1.minZ == 0) {continue;}
      for (AxisAlignedBB shape2 : boxes) {
        if (shape1.minX >= shape2.minX && shape1.minY >= shape2.minY && shape1.minZ >= shape2.minZ &&
            shape1.maxX <= shape2.maxX && shape1.maxY <= shape2.maxY && shape1.maxZ <= shape2.maxZ) {continue loop;}
      }
    ...
    if (this.boxes.isEmpty()) {this.simplified = new AxisAlignedBB(0, 0, 0, 0, 0, 0);} else {
  ...
  public BlockShape offset(EnumFacing facing) {
    return this.offset(facing.getXOffset(), facing.getYOffset(), facing.getZOffset());
  }
  ...
  public boolean intersects(BlockShape shape) {
    if (this.isEmpty() || shape.isEmpty()) {return false;}
  ...
  public BlockShape rotate(EnumFacing.Axis axis) {
    ...
    if (axis == EnumFacing.Axis.X) {return this.transformBoxes(box -> new AxisAlignedBB(box.minX, box.minZ, -box.minY + 1, box.maxX, box.maxZ, -box.maxY + 1));}
    if (axis == EnumFacing.Axis.Y) {return this.transformBoxes(box -> new AxisAlignedBB(-box.minZ + 1, box.minY, box.minX, -box.maxZ + 1, box.maxY, box.maxX));}
    if (axis == EnumFacing.Axis.Z) {return this.transformBoxes(box -> new AxisAlignedBB(box.minY, -box.minX + 1, box.minZ, box.maxY, -box.maxX + 1, box.maxZ));}
    return null;
  }

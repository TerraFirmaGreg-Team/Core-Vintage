    ...
    for (float r = rarity; r < (5 + rarity); r++) {
      ...
      if (data.isInitialized() && data.getRainfall() >= RAINFALL_SAND_SANDY_MIX) {
        ...
        for (int i = random.nextInt(Math.round(1 + floraDiversity)); i < (mossyCount + floraDensity) * 10; i++) {
          ...
          if (BlocksTFC.isRawStone(world.getBlockState(blockPos)) && (world.isAirBlock(blockPos.up()) || world.isAirBlock(blockPos.down())
                                                                      || world.isAirBlock(blockPos.north()) || world.isAirBlock(blockPos.south())
                                                                      || world.isAirBlock(blockPos.east()) || world.isAirBlock(blockPos.west())) &&

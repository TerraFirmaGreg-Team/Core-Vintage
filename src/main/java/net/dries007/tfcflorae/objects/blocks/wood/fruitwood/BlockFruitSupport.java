...
public class BlockFruitSupport extends Block {
  ...
  public static Set<BlockPos> getAllUnsupportedBlocksIn(World worldIn, BlockPos from, BlockPos to) {
    ...
    for (BlockPos.MutableBlockPos searchingPoint : BlockPos.getAllInBoxMutable(minPoint.add(sRangeHorNeg, sRangeVertNeg, sRangeHorNeg),
      ...
      if (st.getBlock() instanceof BlockFruitSupport) {
        if (((BlockFruitSupport) st.getBlock()).canSupportBlocks(worldIn, searchingPoint)) {
  ...
  public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
    ...
    if (player.isSneaking() && heldStack.getItem() instanceof ItemBlock) {
      ...
      if (itemBlock instanceof BlockFruitSupport) {
        for (BlockPos.MutableBlockPos mutablePos = new BlockPos.MutableBlockPos(pos); mutablePos.getY() <= pos.getY() + 5;
  ...
  private boolean isConnectable(IBlockAccess world, BlockPos pos, EnumFacing facing) {
    ...
    return state.getBlock() instanceof BlockFruitSupport;
  }
  ...
  private boolean isThreeTall(IBlockAccess world, BlockPos pos) {
    ...
    if (!(world.getBlockState(pos.down()).getBlock() instanceof BlockFruitSupport)) {return false;}
    return world.getBlockState(pos.down().down()).getBlock() instanceof BlockFruitSupport;
  }
  ...
  private boolean canBlockStay(IBlockAccess world, BlockPos pos) {
    IBlockState state = world.getBlockState(pos);
    if (!(state.getBlock() instanceof BlockFruitSupport)) {return false;}
    EnumFacing.Axis axis = state.getValue(AXIS);
    if (axis == EnumFacing.Axis.Y) {
      return world.getBlockState(pos.down()).isNormalCube() || isConnectable(world, pos, EnumFacing.DOWN);
    } else {
  ...
  private int getHorizontalDistance(EnumFacing face, IBlockAccess worldIn, BlockPos pos) {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      if (!(worldIn.getBlockState(offsetPos).getBlock() instanceof BlockFruitSupport) && !worldIn.isAirBlock(offsetPos)) {
        return 0;
      }
      IBlockState state = worldIn.getBlockState(pos.offset(face, i + 1));
      if (state.getBlock() instanceof BlockFruitSupport && state.getValue(AXIS) == EnumFacing.Axis.Y) {
        distance = i;
        break;
      }
    }

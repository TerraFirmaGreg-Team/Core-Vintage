    ...
    protected void actionPerformed(GuiButton button) {
        if (button instanceof GuiButtonPlayerInventoryTab tabButton && ((GuiButtonPlayerInventoryTab) button).isActive()) {
            if (tabButton.isActive()) {
                ...
                ModuleCoreOld.PACKET_SERVICE.sendToServer(new SCPacketSwitchPlayerInventoryTab(tabButton.getGuiType()));
            }
    ...
    private void updateSkillValues() {
        ...
        if (skills != null) {
            ...
            if (startSkill >= totalSkills || startSkill < 0) {
                ModuleCoreOld.LOGGER.warn("Invalid skill page! Page: {}, Start at: {}, Skill Order is: {}", currentPage, startSkill, skillOrder);
                return;
            }

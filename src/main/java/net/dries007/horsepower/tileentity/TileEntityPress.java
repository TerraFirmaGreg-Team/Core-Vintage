...
public class TileEntityPress extends TileEntityHPHorseBase {
  ...
  public void setInventorySlotContents(int index, ItemStack stack) {
    ...
    if (index == 0 && !flag) {currentPressStatus = 0;}
  ...
  public int getInventoryStackLimit(ItemStack stack) {
    ...
    if (recipe == null) {return getInventoryStackLimit();}
    return recipe.getInput().getCount();
  }

  @Override
  public void markDirty() {
    if (getStackInSlot(0).isEmpty()) {currentPressStatus = 0;}
  ...
  public boolean hasCapability(Capability<?> capability, @Nullable EnumFacing facing) {
    return ((capability == CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY && (facing == null || facing == EnumFacing.DOWN))
  ...
  public <T> T getCapability(Capability<T> capability, @Nullable EnumFacing facing) {
    if (capability == CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY) {
      if (facing == null || facing == EnumFacing.DOWN) {return (T) tank;}
    }
  ...
  public boolean canWork() {
    if (getStackInSlot(0).isEmpty()) {
      ...
      if (getStackInSlot(0).getCount() < input.getCount()) {return false;}
      if (itemstack.isEmpty() && !recipe.isLiquidRecipe()) {return false;}
      ...
      if (recipe.isLiquidRecipe()) {
        ...
        return tank.getFluidAmount() == 0 && (output.isEmpty()
                                              || output.isItemEqual(itemstack) && output.getCount() + itemstack.getCount() <= output.getMaxStackSize());
      }
    }
  }

...
public enum VeinRegistry {
   ...
   * @param subfolder    the current subfolder, relative to TFC config directory (used to differentiate veins in case of more than one file registers
   *                     a vein with the same name)
   */
  public void readFile(Set<Map.Entry<String, JsonElement>> jsonElements, String subfolder) {
    for (Map.Entry<String, JsonElement> entry : jsonElements) {
      try {
        ...
        if ("#loader".equals(properVeinName)) {

...
public class ItemSizeHandler implements ICapabilityProvider, IItemSizeAndWeight {
    ...
    /**
     * Получает экземпляр ItemSizeHandler на основе указанного размера, веса и Capability складирования в стопках.
     *
     * @param size     Размер предмета.
     * @param weight   Вес предмета.
     * @param canStack Флаг, указывающий, может ли предмет быть складирован в стопках.
    ...
    /**
     * Возвращает экземпляр указанной Capability для объекта, если доступно.
     *
     * @param capability Возможность.
     * @param facing     Направление.
     * @param <T>        Тип Capability.
     * @return Экземпляр указанной Capability для объекта, если доступно, в противном случае - null.
     */
    @Nullable
    @Override

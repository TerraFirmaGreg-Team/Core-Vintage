			...
			if (rock == null) {
				...
				if (category == null) {
					TerraFirmaCraft.getLog()
					               .warn("Problem parsing ore entry '{}'. Rock / Rock Category '{}' is not defined. Skipping.", name, e);
				} else {
					blocks.addAll(TFCRegistries.ROCKS.getValuesCollection()
					                                 .stream()
					                                 .filter(e1 -> e1.getRockCategory() == category)

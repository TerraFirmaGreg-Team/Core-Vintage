...
public class TileFridge extends TEInventory implements ITickable, IEnergySink {
  ...
  public boolean hasCapability(Capability<?> capability, @Nullable EnumFacing facing) {
    if (!isMainBlock()) {
      TileFridge te = Helpers.getTE(world, pos.up(), TileFridge.class);
      if (te != null) {
  ...
  public void onBreakBlock(World world, BlockPos pos, IBlockState state) {
    if (Loader.isModLoaded("ic2")) {
      ic2Unload();
      if (isMainBlock()) {
        TileFridge child = Helpers.getTE(world, pos.down(), TileFridge.class);
        if (child != null) {
          child.ic2Unload();
        }
      } else {
        TileFridge main = Helpers.getTE(world, pos.up(), TileFridge.class);
        if (main != null) {
          main.ic2Unload();
        }
      }

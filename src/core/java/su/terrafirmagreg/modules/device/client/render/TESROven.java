...
import static net.dries007.tfc.objects.te.TEOven.SLOT_FUEL_1;
import static net.dries007.tfc.objects.te.TEOven.SLOT_FUEL_2;
  ...
  public void render(TEOven te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
    ...
    if (te.hasWorld()) {
      ...
      if (cap != null) {
        ...
        if (state.getValue(LIT)) {rotation -= 4;}
        if (state.getValue(CURED)) {rotation -= 8;}

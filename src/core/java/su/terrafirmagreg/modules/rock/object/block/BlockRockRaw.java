...
public class BlockRockRaw extends BaseBlock implements IRockEntry {
  ...
  public BlockRockRaw(RockType type) {
    ...
    FallingBlockManager.registerFallable(this, Specification.COLLAPSABLE_ROCK, BlocksRock.COBBLE.get(type).getDefaultState());
  }
  ...
  public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
    ...
    if (ConfigRock.BLOCK.ANVIL.enableStoneAnvil && stack.getItem() == ToolItems.HARD_HAMMER.get() && !worldIn.isBlockNormalCube(pos.up(), true)) {
      if (!worldIn.isRemote) {
        ...
        worldIn.setBlockState(pos, BlocksRock.ANVIL.get(getType()).getDefaultState());
      }
      return true;
    }
    return false;

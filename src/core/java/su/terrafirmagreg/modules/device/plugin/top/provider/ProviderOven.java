          ...
          switch (ConfigDevice.BLOCK.OVEN.timeTooltipMode) {
            ...
            case TICKS:
              probeInfo.text(new TextComponentTranslation(
                ModUtils.localize("top", "devices.oven.remaining.ticks"), remainingTicks).getFormattedText());
            case MINECRAFT_HOURS:
              long remainingHours = Math.round(remainingTicks / (float) ICalendar.TICKS_IN_HOUR);
              probeInfo.text(new TextComponentTranslation(
                ModUtils.localize("top", "devices.oven.remaining.hours"), remainingHours).getFormattedText());
              break;
            case REAL_MINUTES:
              long remainingMinutes = Math.round(remainingTicks / 1200.0f);
              probeInfo.text(new TextComponentTranslation(
                ModUtils.localize("top", "devices.oven.remaining.minutes"), remainingMinutes).getFormattedText());
              break;
          }

...
public abstract class TileEntityHPBaseRenderer<T extends TileEntityHPBase> extends TileEntitySpecialRenderer<T> {
	...
	public boolean canShowAmount(TileEntityHPBase te) {
		return Configs.client.renderItemAmount && (!Configs.client.mustLookAtBlock || this.rendererDispatcher.cameraHitResult != null && (te.getPos()
		                                                                                                                                    .equals(this.rendererDispatcher.cameraHitResult.getBlockPos()) || (te.getWorld()
		                                                                                                                                                                                                         .getBlockState(te.getPos()
		                                                                                                                                                                                                                          .up())
		                                                                                                                                                                                                         .getBlock() instanceof BlockFiller && te.getPos()
		                                                                                                                                                                                                                                                 .up()
	...
	protected void renderBaseModelWithFacing(TileEntityHPBase te, IBlockState blockState, Tessellator tessellator, BufferBuilder buffer, double x, double y, double z, int destroyStage) {
		...
		if (destroyStage >= 0) {
			...
			dispatcher.getBlockModelRenderer()
			          .renderModel(te.getWorld(), model, blockState, te.getPos(), buffer, false);
	...
	protected void renderLeach(double x1, double y1, double z1, double ox, double oy, double oz, double x2, double y2, double z2) {
		...
		for (int j = 0; j <= 24; ++j) {
			...
			vertexbuffer.pos(ox + d13 * (double) f3 + 0.0D, oy + d14 * (double) (f3 * f3 + f3) * 0.5D + (double) ((24.0F - (float) j) / 18.0F + 0.125F), oz + d15 * (double) f3)
			            .color(f, f1, f2, 1.0F)
			            .endVertex();
			vertexbuffer.pos(ox + d13 * (double) f3 + 0.025D, oy + d14 * (double) (f3 * f3 + f3) * 0.5D + (double) ((24.0F - (float) j) / 18.0F + 0.125F) + 0.025D, oz + d15 * (double) f3)
			            .color(f, f1, f2, 1.0F)
			            .endVertex();
		}
		...
		for (int k = 0; k <= 24; ++k) {
			...
			vertexbuffer.pos(ox + d13 * (double) f7 + 0.0D, oy + d14 * (double) (f7 * f7 + f7) * 0.5D + (double) ((24.0F - (float) k) / 18.0F + 0.125F) + 0.025D, oz + d15 * (double) f7)
			            .color(f4, f5, f6, 1.0F)
			            .endVertex();
			vertexbuffer.pos(ox + d13 * (double) f7 + 0.025D, oy + d14 * (double) (f7 * f7 + f7) * 0.5D + (double) ((24.0F - (float) k) / 18.0F + 0.125F), oz + d15 * (double) f7 + 0.025D)
			            .color(f4, f5, f6, 1.0F)
			            .endVertex();
		}

...
public class MMCQ {
  ...
  /**
   * http://www.compuphase.com/cmetric.htm http://stackoverflow.com/a/6334454 Returns 0 for equal colors, nonzero for colors that look different. The return
   * value is farther from 0 the more different the colors look.
   * <p>
  ...
  public static class CMap {
    ...
    public int[] nearest(int[] color) {
      ...
      for (int i = 0; i < numVBoxes; i++) {
        ...
        d2 = fastPerceptualColorDistanceSquared(color, vbColor);
        if (d2 < d1) {
          d1 = d2;
          pColor = vbColor;
        }

...
public class TESRFruitLoomTFCF extends TESRBase<TEFruitLoom> {
  ...
  private static final Map<IFruitTree, ResourceLocation> PLANKS_TEXTURES = new HashMap<>();
  private static final Map<Tree, ResourceLocation> PLANKS_TEXTURES_TREE = new HashMap<>();
  ...
  static {
    ...
    for (IFruitTree fruitTree : FruitTree.values()) {
      ...
      //noinspection ConstantConditions
      PLANKS_TEXTURES.put(fruitTree, new ResourceLocation(TFCF, "textures/blocks/wood/planks/" + name + ".png"));
    }

    for (int i = 0; i < BlocksTFCF.bamboo.length; i++) {
      PLANKS_TEXTURES_TREE.put(BlocksTFCF.bambooTrees[i], new ResourceLocation(TFC, "textures/blocks/wood/planks/" + BlocksTFCF.bamboo[i] + ".png"));
    }

    PLANKS_TEXTURES_TREE.put(TreesTFCF.CASSIA_CINNAMON_TREE, new ResourceLocation(TFCF, "textures/blocks/wood/planks/cassia_cinnamon.png"));

    PLANKS_TEXTURES_TREE.put(TreesTFCF.CEYLON_CINNAMON_TREE, new ResourceLocation(TFCF, "textures/blocks/wood/planks/ceylon_cinnamon.png"));
  }

  @Override
  public void render(TEFruitLoom te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
    ...
    try {
      ...
      if (te.getWood() != null) {this.bindTexture(PLANKS_TEXTURES.get(te.getWood()));} else if (te.getTree() != null) {
        this.bindTexture(PLANKS_TEXTURES_TREE.get(te.getTree()));
      }
  ...
  @Override
  public boolean isGlobalRenderer(TEFruitLoom te) {
    return false;
  }
  ...
  private void drawMaterial(BufferBuilder b, int maxPieces, int pieces, int maxProgress, int progress, double Z1, double Z2) {
    ...
    for (int i = 0; i < pieces; i++) {
      ...
      double[][] sidesZ = getPlaneVertices(
        0.1875 + (0.625 / maxPieces) * i, y1, z1 - 0.001, 0.1875 + (0.625 / maxPieces) * (i + 1), y2, z2 - 0.001, texX1, texY1, texX2, texY2);
      ...
      sidesZ = getPlaneVertices(
        0.1875 + (0.625 / maxPieces) * i, 0, z1 - 0.001, 0.1875 + (0.625 / maxPieces) * (i + 1), y2, z2 - 0.001, texX1, texY1, texX2, texY2);

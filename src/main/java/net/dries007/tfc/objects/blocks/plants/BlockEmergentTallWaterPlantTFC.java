...
public class BlockEmergentTallWaterPlantTFC extends BlockTallWaterPlantTFC implements ITallPlant {
  ...
  public boolean canGrow(World worldIn, BlockPos pos, IBlockState state, boolean isClient) {
    ...
    if (water == SALT_WATER) {
      ...
      return i < plant.getMaxHeight() && (worldIn.isAirBlock(pos.up()) || BlocksTFC.isFreshWater(worldIn.getBlockState(pos.up())))
             && canBlockStay(worldIn, pos.up(), state);
    }
  }
  ...
  public void shrink(World worldIn, BlockPos pos) {
    ...
    if (flag) {worldIn.setBlockState(pos, plant.getWaterType());} else {worldIn.setBlockToAir(pos);}
  ...
  public boolean removedByPlayer(IBlockState state, World world, BlockPos pos, EntityPlayer player, boolean willHarvest) {
    ...
    if (flag) {return world.setBlockState(pos, plant.getWaterType(), world.isRemote ? 11 : 3);} else {

...
public class TilePitKiln extends TEPlacedItem implements ITickable {
  ...
  public static final Vec3i[] DIAGONALS = new Vec3i[]{
    new Vec3i(1, 0, 1),
    new Vec3i(-1, 0, 1),
    new Vec3i(1, 0, -1),
    new Vec3i(-1, 0, -1)
  ...
  public static void convertPlacedItemToPitKiln(World world, BlockPos pos, ItemStack strawStack) {
    ...
    TileUtils.getTile(world, pos, TEPlacedItem.class).ifPresent(tileOld -> {
      ...
      TileUtils.getTile(world, pos, TilePitKiln.class).ifPresent(tileNew -> {
        ...
        // Copy misc data
        tileNew.isHoldingLargeItem = tileOld.holdingLargeItem();
  ...
  public void update() {
    if (isLit) {
      ...
      // Check if complete
      long remainingTicks = ConfigDevice.BLOCK.PIT_KILN.ticks - (Calendar.PLAYER_TIME.getTicks() - litTick);
      if (remainingTicks <= 0) {
        // Empty ingredients
        emptyFuelContents();

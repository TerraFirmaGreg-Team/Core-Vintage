...
public class BlockJoshuaTreeFlower extends Block {
      ...
      if (worldIn.isAirBlock(blockpos) && blockpos.getY() < 256) {
        ...
        if (i < 5 && net.minecraftforge.common.ForgeHooks.onCropsGrowPre(worldIn, blockpos, state, rand.nextInt(1) == 0)) {
          ...
          if (BlocksTFC.isSand(iblockstate) || BlocksTFC.isSoilOrGravel(iblockstate) || BlocksTFCF.isSand(iblockstate) || BlocksTFCF.isSoilOrGravel(iblockstate)
              || iblockstate == Blocks.HARDENED_CLAY || iblockstate == Blocks.STAINED_HARDENED_CLAY) {
            flag = true;
            ...
            for (int k = 0; k < 4; ++k) {
              ...
              if (block1 != BlockJoshuaTreeLog.get(wood)) {
                if (BlocksTFC.isSand(worldIn.getBlockState(pos.down(j + 1))) || BlocksTFC.isSoilOrGravel(worldIn.getBlockState(pos.down(j + 1)))
                    || BlocksTFCF.isSand(worldIn.getBlockState(pos.down(j + 1))) || BlocksTFCF.isSoilOrGravel(worldIn.getBlockState(pos.down(j + 1)))
      ...
      if (age < 5) {
        ...
        if (BlocksTFC.isSand(iblockstate) || BlocksTFC.isSoilOrGravel(iblockstate) || BlocksTFCF.isSand(iblockstate) || BlocksTFCF.isSoilOrGravel(iblockstate)
          ...
          for (int k = 0; k < 4; ++k) {
            ...
            if (block1 != BlockJoshuaTreeLog.get(wood)) {
              if (BlocksTFC.isSand(worldIn.getBlockState(currentBlock.down(j + 1))) || BlocksTFC.isSoilOrGravel(worldIn.getBlockState(currentBlock.down(j + 1)))
                  || BlocksTFCF.isSand(worldIn.getBlockState(currentBlock.down(j + 1))) || BlocksTFCF.isSoilOrGravel(worldIn.getBlockState(currentBlock.down(
                j + 1))) || block1 == Blocks.HARDENED_CLAY || block1 == Blocks.STAINED_HARDENED_CLAY) {
                flag1 = true;
              }
  ...
  public boolean canPlaceBlockAt(World worldIn, BlockPos pos) {
    ...
    return ((super.canPlaceBlockAt(worldIn, pos) && this.canSurvive(worldIn, pos)) || (BlocksTFC.isSand(worldIn.getBlockState(pos.down()))
  ...
  public boolean canSurvive(World worldIn, BlockPos pos) {
    ...
    if (block != BlockJoshuaTreeLog.get(wood) && !(BlocksTFC.isSand(iblockstate) || BlocksTFC.isSoilOrGravel(iblockstate) || BlocksTFCF.isSand(iblockstate)

...
public class TEOven extends TEInventory implements ITickable {
  ...
  public void update() {
    if (!world.isRemote) {
      if (isBurning) {
        ...
        if (inventory.getStackInSlot(SLOT_MAIN).isEmpty()) {turnOff();}
      } else {
        turnOff();
      }
    }
  ...
  public void light() {
    if (world != null) {
      ...
      if (block instanceof BlockOven) {world.scheduleBlockUpdate(pos, BlocksFL.OVEN, 1, 1);}
    }
  ...
  private void cureSelfWallsAndChimney() {
    ...
    if (!isCuredBlock(state)) {world.setBlockState(pos, state.withProperty(CURED, true));}

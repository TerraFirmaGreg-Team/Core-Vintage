...
public class Plant extends IForgeRegistryEntry.Impl<Plant> {
  ...
  public boolean isValidFloatingWaterDepth(World world, BlockPos pos, IBlockState water) {
    ...
    for (int i = 1; i <= depthCounter; ++i) {
      if (world.getBlockState(pos.down(i)) != water && world.getBlockState(pos.down(i)).getMaterial() != Material.CORAL) {return false;}
    }
  ...
  public boolean canBePotted() {
    return plantType == PlantType.STANDARD || plantType == PlantType.CACTUS || plantType == PlantType.CREEPING || plantType == PlantType.TALL_PLANT
           || plantType == PlantType.DRY || plantType == PlantType.DESERT || plantType == PlantType.MUSHROOM;
  }
  ...
  public final EnumPlantTypeTFC getEnumPlantTypeTFC() {
    switch (plantType) {
      ...
      case DESERT_TALL_PLANT:
        if (isClayMarking) {return EnumPlantTypeTFC.DESERT_CLAY;} else {return EnumPlantTypeTFC.NONE;}
      case DRY:
      case DRY_TALL_PLANT:
        if (isClayMarking) {return EnumPlantTypeTFC.DRY_CLAY;} else {return EnumPlantTypeTFC.DRY;}
      case REED:
      case TALL_REED:
      ...
      default:
        if (isClayMarking) {return EnumPlantTypeTFC.CLAY;} else {return EnumPlantTypeTFC.NONE;}
    }
  }

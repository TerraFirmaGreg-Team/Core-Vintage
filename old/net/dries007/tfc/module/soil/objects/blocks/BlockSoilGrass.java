        ...
        if (!worldIn.isRemote) {
            ...
            if (block instanceof ISoilBlock) {
                ...
                if (worldIn.getLightFromNeighbors(pos.up()) < 4 && worldIn.getBlockState(pos.up()).getLightOpacity(worldIn, pos.up()) > 2) {
                    worldIn.setBlockState(pos, StorageSoil.getSoilBlock(SoilBlockVariants.DIRT, soil).getDefaultState());
                } else {
                    if (worldIn.getLightFromNeighbors(pos.up()) >= 9) {
                        for (int i = 0; i < 4; ++i) {
                            ...
                            if (iblockstate1.getBlock() == StorageSoil.getSoilBlock(SoilBlockVariants.DIRT, soil) && worldIn.getLightFromNeighbors(blockpos.up()) >= 4 && iblockstate.getLightOpacity(worldIn, pos.up()) <= 2) {
                                worldIn.setBlockState(blockpos, StorageSoil.getSoilBlock(SoilBlockVariants.GRASS, soil).getDefaultState());
                            }
                        }
                    }
                }

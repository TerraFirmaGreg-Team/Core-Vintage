    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
        ...
        for (int x = 0; x < 16; ++x) {
            for (int z = 0; z < 16; ++z) {
                ...
                final float rainfall = ChunkData.getRainfall(world, strataLayer);
                ...
                if (rainfall < +1.3 * random.nextGaussian() + RAINFALL_DRY_GRASS && avgTemperature >= 15f) {
                    ...
                    if (BiomeUtils.isMesaBiome(b)) {
                        for (int y = WorldTypeClassic.SEALEVEL; y < world.getTopSolidOrLiquidBlock(strataLayer).getY(); ++y) {
                            ...
                            if ((y <= WorldTypeClassic.SEALEVEL + 5 &&
                                    (BlocksTFC.isRawStone(currentBlockState) || BlocksTFC.isGround(currentBlockState) ||
                                            BlocksTFCF.isGround(currentBlockState) || BlocksTFC.isSoil(currentBlockState) ||
                                            BlocksTFCF.isSoil(currentBlockState)) &&
                                    !(BlocksTFC.isGrass(currentBlockState) || BlocksTFC.isSand(currentBlockState) ||
                                            BlocksTFC.isClay(currentBlockState) || BlocksTFCF.isGrass(currentBlockState) ||
                                            BlocksTFCF.isSand(currentBlockState) || BlocksTFCF.isClay(currentBlockState))) ||
                                ...
                                if (y >= strataLayer.getY() + 13 && y <= strataLayer.getY() + 15) {
                                    world.setBlockState(currentBlock, STAINED_HARDENED_CLAY.withProperty(BlockColored.COLOR, EnumDyeColor.MAGENTA),
                                            2);
                                }
                                ...
                                if (y >= strataLayer.getY() + 39 && y <= strataLayer.getY() + 46) {
                                    world.setBlockState(currentBlock, STAINED_HARDENED_CLAY.withProperty(BlockColored.COLOR, EnumDyeColor.LIGHT_BLUE),
                                            2);
                                }
                            ...
                            /*if (y + 1 >= WorldTypeClassic.SEALEVEL - 1 && (currentBlockStateTop instanceof BlockPlantTFC || currentBlockStateTop instanceof BlockPlantTFCF))
                            {

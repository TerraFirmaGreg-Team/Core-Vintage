...
public class TEIceBunker extends TileEntityLockableLoot implements IInventory, ITickable, IContainerProvider<ContainerIceBunker, GuiIceBunker> {
	...
	private int getBlockType(int x, int y, int z) {
		...
		if (block instanceof BlockCellarWall) {
			...
			return 1;
				...
				block instanceof BlockRedstoneLight || world.isAirBlock(new BlockPos(getPos().getX() + x, getPos().getY() + y, getPos().getZ() + z))) {
			return 2;
		}

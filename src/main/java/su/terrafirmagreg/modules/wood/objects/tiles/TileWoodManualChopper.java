...
public class TileWoodManualChopper extends TileEntityHPBase {
  ...
  public boolean isItemValidForSlot(int index, ItemStack stack) {
    return index == 0 && HPRecipes.instance()
        .hasChopperRecipe(stack, true) && getStackInSlot(1).isEmpty() && getStackInSlot(
        0).isEmpty();
  }
  ...
  public void setInventorySlotContents(int index, ItemStack stack) {
    ...
    boolean flag =
        !stack.isEmpty() && stack.isItemEqual(itemstack) && ItemStack.areItemStackTagsEqual(stack,
            itemstack);
    if (index == 0 && !flag) {
  ...
  public boolean hasCapability(Capability<?> capability, @Nullable EnumFacing facing) {
    return super.hasCapability(capability, facing) || (
        capability == CapabilityItemHandler.ITEM_HANDLER_CAPABILITY && facing != null);
  }
  ...
  public <T> T getCapability(Capability<T> capability, @Nullable EnumFacing facing) {
    ...
    return cap != null ? cap
        : (facing != null && capability == CapabilityItemHandler.ITEM_HANDLER_CAPABILITY)
            ? (T) handlerSide : null;
  }
    ...
    if (canWork()) {
      ...
      if (!getWorld().isRemote) {
        ...
        if (Configs.general.choppingBlockDrop) {
          InventoryHelper.spawnItemStack(getWorld(), getPos().getX(), getPos().getY() + 0.5,
              getPos().getZ(), result);
        } else {
          if (output.isEmpty()) {
      ...
      getWorld().playSound(player, getPos(), SoundEvents.BLOCK_WOOD_BREAK, SoundCategory.BLOCKS,
          1.0f, 1.0f);
      input.shrink(1);
      markDirty();
    }

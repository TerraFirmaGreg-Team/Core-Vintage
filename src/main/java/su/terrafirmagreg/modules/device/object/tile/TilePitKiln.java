...
import static su.terrafirmagreg.modules.device.object.block.BlockPitKiln.FULL;
...
public class TilePitKiln extends TEPlacedItem implements ITickable {
  ...
  public static void convertPlacedItemToPitKiln(World world, BlockPos pos, ItemStack strawStack) {
    ...
    if (teOld != null) {
      ...
      // Copy TE data
      TilePitKiln teNew = Helpers.getTE(world, pos, TilePitKiln.class);
      if (teNew != null) {
        // Copy inventory
        ...
        // Copy misc data
        teNew.isHoldingLargeItem = teOld.holdingLargeItem();
    ...
    if (hasFuel() && isValid() && !isLit()) {
      ...
      if (Blocks.FIRE.canPlaceBlockAt(world, above)) {
        ...
        for (Vec3i diagonal : DIAGONALS) {
          ...
          TilePitKiln pitKiln = Helpers.getTE(world, pitPos, TilePitKiln.class);
          if (pitKiln != null) {
            pitKiln.tryLight();
          }
        }

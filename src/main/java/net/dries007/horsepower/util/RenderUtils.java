    ...
    for (int xo = -3; xo <= 3; xo++) {
      for (int yo = yOffset; yo <= 1 + yOffset; yo++) {
        for (int zo = -3; zo <= 3; zo++) {
          ...
          if ((xo <= 1 && xo >= -1) && (zo <= 1 && zo >= -1)) {continue;}
          if (pos.getY() >= 0) {
            IBlockState state = world.getBlockState(pos);
            if (!state.getBlock().isReplaceable(world, pos)) {
              GlStateManager.color(1, 0, 0, invalidAplha);
              drawBoundingBoxOutline(new AxisAlignedBB(pos).offset(-Minecraft.getMinecraft().getRenderManager().viewerPosX, -Minecraft.getMinecraft()
                .getRenderManager().viewerPosY, -Minecraft.getMinecraft().getRenderManager().viewerPosZ));
              drawBoundingBox(state.getBoundingBox(world, pos).offset(pos)
                .offset(-Minecraft.getMinecraft().getRenderManager().viewerPosX, -Minecraft.getMinecraft()
                  .getRenderManager().viewerPosY, -Minecraft.getMinecraft().getRenderManager().viewerPosZ));
            } else {

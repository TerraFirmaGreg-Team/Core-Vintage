...
public class TileWoodPress extends TileEntityHPHorseBase {
  ...
  public boolean canWork() {
    if (getStackInSlot(0).isEmpty()) {
      ...
      if (recipe.isLiquidRecipe()) {
        return output.isEmpty() && (tank.getFluidAmount() == 0
            || tank.fillInternal(fluidOutput, false) >= fluidOutput.amount);
      } else {
        return tank.getFluidAmount() == 0 &&
            (output.isEmpty() || output.isItemEqual(itemstack)
  ...
  public ITextComponent getDisplayName() {
    if (valid) {
      ...
      return new TextComponentTranslation(Localization.INFO.PRESS_INVALID.key()).setStyle(

...
public class SimpleCraftMatrix {
  ...
  public SimpleCraftMatrix(boolean outsideSlotRequired, String... pattern) {
    if (pattern.length == 0 || pattern.length > MAX_HEIGHT) {throw new IllegalArgumentException("Pattern height is invalid");}
    ...
    if (width > MAX_WIDTH) {throw new IllegalArgumentException("Pattern width is invalid");}
  ...
  public void setAll(boolean value) {
    for (int i = 0; i < width * height; i++) {set(i, value);}
  }
  ...
  public boolean isEqual(SimpleCraftMatrix other) {
    ...
    for (int i = 0; i < width * height; i++) {
      if (other.matrix[i] != this.matrix[i]) {return false;}
    }
    return true;
  }
    ...
    for (int x = 0; x < this.width; x++) {
      for (int y = 0; y < this.height; y++) {
        ...
        if (x < startX || y < startY || x - startX >= other.width || y - startY >= other.height) {
          ...
          if (matrix[patternIdx] != other.outsideSlot) {return false;}
        } else {
          // Otherwise, the value must equal the value in the pattern
          int otherIdx;
          if (isMirrored) {otherIdx = (y - startY) * other.width + (other.width - 1 - (x - startX));} else {
            otherIdx = (y - startY) * other.width + (x - startX);
          }

          if (matrix[patternIdx] != other.matrix[otherIdx]) {return false;}
        }
      }
    }
    return true;

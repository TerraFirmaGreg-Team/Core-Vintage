...
public enum JsonConfigRegistry {
            ...
            try {
                ...
                if (defaultFile.exists()) {
                    ...
                    FileUtils.copyInputStreamToFile(Objects.requireNonNull(JsonConfigRegistry.class.getClassLoader()
                            .getResourceAsStream(DEFAULT_ORE_SPAWN)), defaultFile);
    ...
    public void postInit() {
        ...
        try {
            recursivePathList = Files.walk(tfcConfigDir.toPath())
                    .filter(f -> Files.isRegularFile(f) && f.toString().endsWith(".json"))
                    .toArray(Path[]::new);
        } catch (IOException e) {
            TerraFirmaCraft.getLog()
                    .error("Unable to read files in the config directory! TFC will not generate any ore veins or load any damage resitances!");

...
public class BlockBloomery extends BlockHorizontal implements IItemSizeAndWeight, ILightableBlock, IItemProvider, IHasModel {
    ...
    public IBlockState getStateFromMeta(int meta) {
        return this.getDefaultState()
                .withProperty(BlockHorizontal.FACING, EnumFacing.byHorizontalIndex(meta % 4))
                .withProperty(ILightableBlock.LIT, meta / 4 % 2 != 0)
                .withProperty(BlockTrapDoor.OPEN, meta / 8 != 0);
    }

    @Override
    public int getMetaFromState(IBlockState state) {
        return state.getValue(BlockHorizontal.FACING).getHorizontalIndex()
                + (state.getValue(ILightableBlock.LIT) ? 4 : 0)
                + (state.getValue(BlockTrapDoor.OPEN) ? 8 : 0);
    }
    ...
    public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
        return AABB[state.getValue(BlockHorizontal.FACING).getHorizontalIndex()][0];
    }
    ...
    public AxisAlignedBB getCollisionBoundingBox(IBlockState blockState, IBlockAccess worldIn, BlockPos pos) {
        ...
        return AABB[blockState.getValue(BlockHorizontal.FACING).getHorizontalIndex()][0];
    }
    ...
    public RayTraceResult collisionRayTrace(IBlockState blockState, World worldIn, BlockPos pos, Vec3d start, Vec3d end) {
        if (blockState.getValue(BlockTrapDoor.OPEN)) {
            int index = blockState.getValue(BlockHorizontal.FACING).getHorizontalIndex();
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if (!worldIn.isRemote) {
            if (!state.getValue(ILightableBlock.LIT)) {
                worldIn.setBlockState(pos, state.cycleProperty(BlockTrapDoor.OPEN));
                worldIn.playSound(null, pos, SoundEvents.BLOCK_FENCE_GATE_CLOSE, SoundCategory.BLOCKS, 1.0f, 1.0f);
            }
            TEBloomery te = Helpers.getTE(worldIn, pos, TEBloomery.class);
            if (te != null) {
                if (!state.getValue(ILightableBlock.LIT) && te.canIgnite()) {
                    ItemStack held = player.getHeldItem(hand);
                    if (ItemFireStarter.onIgnition(held)) {
                        worldIn.setBlockState(pos, state.withProperty(ILightableBlock.LIT, true).withProperty(BlockTrapDoor.OPEN, false));
                        te.onIgnite();
                        return true;
                    }
                }
    ...
    public IBlockState getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer) {
        ...
        return this.getDefaultState().withProperty(BlockHorizontal.FACING, placeDirection);
    }

    @Override
    @Nonnull
    protected BlockStateContainer createBlockState() {
        return new BlockStateContainer(this, BlockHorizontal.FACING, ILightableBlock.LIT, BlockTrapDoor.OPEN);
    }

...
public abstract class BlockCropSimple extends BlockCropTFC {
  ...
  public static BlockCropSimple create(ICrop crop, boolean isPickable) {
    ...
    if (property == null) {throw new IllegalStateException("Invalid growthstage property " + (crop.getMaxStage() + 1) + " for crop");}
    ...
    if (isPickable) {
      ...
      if (!foodDrop.isEmpty()) {
        ...
        if (!worldIn.isRemote) {
          ...
          if (!seedDrop.isEmpty()) {ItemHandlerHelper.giveItemToPlayer(playerIn, seedDrop);}
        }
        return true;
      }
    }

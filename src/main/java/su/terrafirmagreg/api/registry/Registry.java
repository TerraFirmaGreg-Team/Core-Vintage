...
import su.terrafirmagreg.api.registry.strategy.IClientModelRegistrationStrategy;
import su.terrafirmagreg.api.registry.strategy.IForgeRegistryEventRegistrationStrategy;
import su.terrafirmagreg.api.registry.strategy.ITileEntityRegistrationStrategy;
...
public class Registry {
	...
	@Getter
	private final String modId = Tags.MOD_ID;
	...
	public Registry(@Nullable CreativeTabs creativeTabs) {
	...
	public <B extends Block> B registerBlock(B block, String name) {
		String resourcePath = name.toLowerCase().replaceAll("/", ".");
		block.setRegistryName(new ResourceLocation(this.modId, resourcePath));
		block.setTranslationKey(this.modId + "." + resourcePath);
	...
	public <B extends Block, I extends ItemBlock> B registerBlock(B block, I itemBlock) {
		...
		if (itemBlock != null) this.registerItem(itemBlock);
	...
	public Item registerItem(Item item, ResourceLocation registryName, boolean noCreativeTab) {
		...
		String resourcePath = registryName.getPath().toLowerCase().replace("/", ".");
		item.setRegistryName(registryName);
		item.setTranslationKey(resourceDomain + "." + resourcePath);

		return this.registerItem(item, noCreativeTab);
	}

package su.terrafirmagreg.modules.core.feature.climate.spi;
...
public class ClimateHelper {
  ...
  public static float monthlyTemp(float regionalTemp, int z, long timeOffset) {
    ...
    final float delta = (float) ICalendarFormatted.getDayOfMonth(time, Calendar.CALENDAR_TIME.getDaysInMonth()) / Calendar.CALENDAR_TIME.getDaysInMonth();
    // Affine combination to smooth temperature transition
    return currentMonthFactor * (1 - delta) + nextMonthFactor * delta;
  }

  /**
   * Internationally accepted average lapse time is 6.49 K / 1000 m, for the first 11 km of the atmosphere. Our temperature is scales the 110 m against 2750 m, so that gives us a change of 1.6225 / 10 blocks. The amount to reduce
   * temperature by after applying height transformations
   *
   * @param y the y level
